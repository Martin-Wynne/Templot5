...
type
  ...
  Talert_box = class(TForm)
    ...
    procedure left_panbuttonMouseDown(Sender:TObject; Button:TMouseButton; Shift:TShiftState; X,Y:Integer);
    procedure left_panbuttonMouseUp(Sender:TObject; Button:TMouseButton; Shift:TShiftState; X,Y:Integer);
    procedure right_panbuttonMouseDown(Sender:TObject; Button:TMouseButton; Shift:TShiftState; X,Y:Integer);
    procedure down_panbuttonMouseDown(Sender:TObject; Button:TMouseButton; Shift:TShiftState; X,Y:Integer);
    procedure up_panbuttonMouseDown(Sender:TObject; Button:TMouseButton; Shift:TShiftState; X,Y:Integer);
...
var
  ...
  alert_colour:array[0..8] of integer;     //  0-7 actually used
...
var
  ...
  online_url_str:string='https://85a.uk/templot/companion/info.php?ref=000';
  ref_str:string='000';
...
begin
  ...
  if Copy(php_str,1,4)='php/'       // marker exists
     then begin
            ...
            alert_box.ref_label.Caption:=ref_str;
            ...
            online_url_str:='https://85a.uk/templot/companion/info.php?ref='+ref_str;
          end
     else begin
            ref_str:='';
  ...
  with alert_box
       do begin
            ...
            if caption_str<>'' then Caption:=caption_str
            ...
            current_load_str:=convert_tagged_string_to_html(0,0,message_str,True,False);    // 0.97.d
               ...
               then begin
                      ...
                      if Pos('_',panel1_str)<>1   // if begins with underscore, show option boxes instead   205d
                         ...
                         then begin   // show normal bar
                                ...
                                alert_panel1.Caption:=Trim(panel1_str);
                                ...
                                if n>0               // 2 checkboxes...
                                   then begin
                                          option1a_checkbox.Caption:=Copy(panel1_str,11,(n-11));
                                          option1b_checkbox.Caption:=Copy(panel1_str,n+10,100);
                                          ...
                                          option1a_checkbox.Caption:=Copy(panel1_str,11,100);
               ...
               then begin
                      ...
                      if Pos('_',panel2_str)<>1   // if begins with underscore, show option boxes instead   205d
                         ...
                         then begin   // show normal bar
                                ...
                                alert_panel2.Caption:=Trim(panel2_str);
                                ...
                                if n>0               // 2 buttons...
                                   then begin
                                          option2a_button.Caption:=Copy(panel2_str,5,(n-5));
                                          option2b_button.Caption:=Copy(panel2_str,n+4,100);
                                          ...
                                          option2a_button.Caption:=Copy(panel2_str,5,100);
            ...
            if panel5_str<>''
               then begin
                      ...
                      alert_panel5.Caption:=Trim(panel5_str);
                      alert_panel5.Show;
                    end
               else alert_panel5.Hide;
            ...
            if panel6_str<>''
               then begin
                      ...
                      alert_panel6.Caption:=Trim(panel6_str);
                      alert_panel6.Show;
                    end
               else alert_panel6.Hide;
            ...
            whats_this_label.Visible:=(preferences_checkbox.Visible) and (preferences_checkbox.Tag=0);

            if whats_this_label.Visible=True
               then preferences_checkbox.Left:=whats_this_label.Left+whats_this_label.Width
               else preferences_checkbox.Left:=whats_this_label.Left;
            ...
            if Showing=False then do_show_modal(alert_box);  // show the form and wait for a click.
...
procedure Talert_box.HotSpotClick(Sender: TObject; const SRC: ThtString; var Handled: Boolean);
...
begin
  ...
  if help(-1,'Templot needs a response from you.||Click a coloured bar to make your selected response,'
          ...
          +' <SPAN STYLE="color:#800000; font-style:italic;">If you are a new Templot user it is recommended that you don''t do this until you are familiar with Templot.</SPAN>'
          +' If you save your program preferences, the message will also not be shown in subsequent Templot sessions.'
          +' To restore seeing messages, re-start Templot without using your preferences.'
...
begin
  help(-6,'      `0Add  to  Preferences`9'
         ...
         +'||If you save your program preferences, the message will also not be shown in subsequent Templot sessions. To restore seeing messages, re-start Templot without using your preferences.'
...
begin
  ...
  more_url_str:=StringReplace(more_url_str,'https://','',[rfReplaceAll, rfIgnoreCase]);
...
begin
  ...
  if num_files>1
     then ShowMessage('error - attempt to drop more than one image file')
     else begin
            DragQueryFile(msg.Drop,0,@buffer,SizeOf(buffer));

            buffer_str:=change_jpeg_filename(buffer);   // if nec

            try
              drop_image.Picture.LoadFromFile(buffer_str);
            except
              on EInvalidGraphic do begin
                ShowMessage('error - the '+ExtractFileExt(buffer_str)+' file format is not supported');
                EXIT;
              end;//on
            end;//try
...
initialization
  ...
  alert_colour[2]:=$00F0F8F8;   //  information  off-white
  alert_colour[3]:=$00FFFFF8;   //  handy hint   off-green
  alert_colour[4]:=$00F0F8F0;   //  question     pale sage green
  alert_colour[5]:=$00F0FFD0;   //  error        ice blue
  alert_colour[6]:=$00F8FFE8;   //  invalid      frost
