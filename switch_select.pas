...
const
  ...
  +' For more information and diagrams, please refer to the Templot Companion pages on the Templot web site at  85a.uk/templot/companion .'
  +'|-----------------------------------------'
  ...
  +'||Group 9. Custom switches used by Templot in creating tandem turnouts. These are suitable for bullhead or flat-bottom rail.'
  ...
  +' construction - dummy rail joints can be added to represent the shorter switch blade. Templot is not concerned with the construction of switches, only their dimensions'
  ...
  +'||For diagrams and more prototype information about switches, see "Real Track" in the Templot Companion pages on the Templot web site at  85a.uk/templot/companion .';
...
uses pad_unit, control_room, bgkeeps_unit, entry_sheet, colour_unit, alert_unit, help_sheet, chat_unit,
...
const
   custom_help_str:string='Custom  Switch'
    ...
    +' If you enter incompatible dimensions Templot may decline to draw your switch.'
  ...
  joint_help_str:string='This is the switch front distance from the stock rail joint at the left end of the turnout to the "toe" (blade tips). For GWR switches this dimension is normally 64 inches, elsewhere it'
                       +' is usually 65 inches. There are normally 2 standard-length timbers occupying this space.'
                       +'||Handy Hint:'
  ...
  +'||If you are simply experimenting with a custom switch, Templot can fill in the timber positions for you at a constant spacing.'
  ...
  +'||For a diagram and further notes explaining the use and dimensioning of joggles, see also the "real track" page on the Templot web site at  85a.uk/templot/companion .'
  ...
  +'||Templot puts a guide mark across the rail at this position.';
  ...
  +'||In most cases there are 2 timbers in the switch front, but Templot can generate up to 5 of them if needed (J1-J5). If more than this are needed they can be added on each individual template as BONUS timbers.';
  ...
  with switch_selector_listbox do begin
    ...
    with Tswitch(Items.Objects[ItemIndex]) do begin
      ...
      if list_switch_info.valid_data=True
         then begin
                temp_str:=list_switch_info.sw_name_str;   // local string needed
      ...
      if i=4              // adopt control template switch (now in csi)...
         then begin
                ...
                ItemIndex:=item_index;               // show new string selected.
      ...
      if n<>0
         then begin
                alert(6,'    invalid  switch',
                       ...
                       +'||Please refer to the Templot documentation for information about this switch error code.',
      ...
      repeat
        i:=alert(4,'    custom  switch  timbering ?',
                  ...
                  +'||( All dimensions should be entered in FULL-SIZE inches, and to the centre of the timber.)',
                   '','','','?  help','no  -  let  Templot  use  fixed  spacings','yes  -  let  me  enter  the  spacings',4);
...
const
  ...
  +'||Please also consider sharing your custom switch with other users via the Templot Club forum - details are on the Templot web site at 85a.uk/templot';
...
begin
  ...
  ClientWidth:=672;    // 208a see also data_child_form.Close
  ClientHeight:=440;   // 208a see also data_child_form.Close
end;
//______________________________________________________________________________
...
var
  ...
  vert_spacing,vert_joint,vert_front,vert_tips:extended;
                      ...
                      begin
                        ...
                        sw_init_info.num_block_slide_chairs_in_T:=0;     // byte;  // 241a
                        sw_init_info.num_block_heel_chairs_in_T:=0;      // byte;  // 241a
...
begin
  ...
  sw_init_info.num_block_heel_chairs:=1;               // byte 214a
  sw_init_info.num_bridge_chairs_main_rail:=1;         // byte 214a
  sw_init_info.num_bridge_chairs_turnout_rail:=2;      // byte 214a

     // 241a  2 "T" timbers have switch chairs..

  sw_init_info.num_block_slide_chairs_in_T:=1;     // byte;  // 241a
  sw_init_info.num_block_heel_chairs_in_T:=1;      // byte;  // 241a
  ...
  sw_init_info.num_block_slide_chairs:=3;              // byte 214a
  ...
  sw_init_info.num_bridge_chairs_turnout_rail:=3;      // byte 214a
       ...
       // 241a  0 "T" timbers have switch chairs..
  ...
  sw_init_info.num_block_slide_chairs_in_T:=0;     // byte;  // 241a
  sw_init_info.num_block_heel_chairs_in_T:=2;      // byte;  // 241a
  ...
  sw_init_info.num_block_slide_chairs:=2;              // byte 214a     E and F switches inside-keyed on these chairs -- omitted on models for wheel flange clearance and increased opening
  sw_init_info.num_block_heel_chairs:=7;               // byte 214a
  sw_init_info.num_bridge_chairs_main_rail:=3;         // byte 214a
