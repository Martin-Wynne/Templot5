...
{$ALIGN OFF}
   ...
   // create EMF metafiles for external PDF creator

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  StdCtrls, ExtCtrls, ComCtrls, BegaPreviewPanel;
...
(* OT2024
  laz_metafile:TMetafile;           // 245a
  output_canvas:TMetafileCanvas;    // 245a
*)
  ...
  procedure pdf_rotate_metafile(i:integer);forward;   // rotate metafile supplied 90degs clockwise.   213b
...
begin
  with pdf_laz_form do begin
    ...
    //banner_fill_checkbox.Enabled:=False; //  0.91.d pdf  // can't click this once printing under way.

    page_listbox.Enabled:=False;      // 227a

    //cancel_panel.Enabled:=False;
...
begin
  with pdf_laz_form do begin
    ...
    //if (banner_paper=True) and (printer_printing=False) then pdf_laz_form.banner_fill_checkbox.Enabled:=True;   // might have been disabled if this is a change.
...
function pdf_laz_draw:integer;     // draw control template or entire pad on an EMF output canvas.
     ...
     // and relies on the API to crop it to the current page rectangle.
  ...
  (* OT2024
              ...
              begin
                ...
                with data_node do begin
                  ...
                  WriteString('pdf_file_name',pdf_file_name,'');
              ...
              procedure begin_doc;    // start a fresh page
              ...
              begin
                ...
                try
                  ...
                  output_canvas:=TMetaFileCanvas.Create(laz_metafile,0);     // don't need reference device, size already set

                  output_canvas.Font.PixelsPerInch:=pdf_width_dpi;   // match PDF
              ...
              procedure end_doc;    // end current page and save EMF
              ...
              begin
                try
                  output_canvas.Free;   // creates the metafile record when the metafile canvas is freed

                  emf_file_str:=exe_str+'internal\pdf\emf_for_pdf_page_'+IntToStr(page_counter)+'.emf';

                  laz_metafile.SaveToFile(emf_file_str);
              ...
              begin
                ...
                with output_canvas do begin
                  ...
                  //showmessage('font x '+inttostr(font.size));
              ...
              end;
{
              ...
              procedure draw_outline_railedge(aq:integer);
              ...
              begin
                if ( (plain_track=False) or (aq=0) or (aq=8) or (aq=3) or (aq=11) or ((aq>15) and (aq<24)) ) and (aqyn[aq]=True)
                        ...
                        // stock rails and adjacent tracks only if plain track, and data available ?
              ...
              begin
                {if bgnd_form.output_shapes_in_front_of_sb_checkbox.Checked=True
                   then pdf_sketchboard_items(output_canvas,grid_left,grid_top); } // 206e
...
(* OT2024
  ...
  laz_metafile:=TMetafile.Create;    // init..

  laz_metafile.Width:=pdf_width_dots;            // must set the size before using it for a metafilecanvas..
  laz_metafile.Height:=pdf_height_dots;

  laz_metafile.Enhanced:=True;
  ...
  with pdf_laz_form.pdf_save_dialog do begin
    ...
    {pdf_laz_form.pdf_printer.FileName} pdf_file_name:=file_str;

  end;//with

  //if pdf_laz_form.pdf_setup.Execute=False then EXIT;
...
{ 227a
  ...
  pdf_laz_form.info_scrollbox.VertScrollBar.Position:=0;
}
  ...
  with pdf_laz_form do begin
    ...
    //227a printer_info_label.Caption:=insert_crlf_str(info_str);
                  ...
                  case pdf_laz_form.ModalResult of
                    ...
                    mrIgnore: begin
                     ...
                     mrRetry: begin
                        ...
                        mrOk: begin
                                ...
                                if output_in_progress=True then begin
                                                                  end_doc;
                                                                  RESULT:=RESULT+1;

                                                                //{pdf_laz_form.pdf_printer}.EndPage;
                                                                //{pdf_laz_form.pdf_printer}.StartPage(pdf_width_dots,pdf_height_dots,pdf_width_dpi,pdf_height_dpi,0);    // 0.91.d
                                                                end;
                                begin_doc;
                                          //               else begin
                    ...
                    mrCancel: begin
                                if output_in_progress=True
                                   then begin
                                          end_doc;              // finish page and omit all remaining
                                          RESULT:=RESULT+1;
                                        end;
                                EXIT;
                              end;
          ...
          Brush.Style:=bsSolid;
                 ...
                 then begin
                        ...
                        repeat
                          ...
                          {if banner_paper=False
                             then} text_out(grid_now_dots-(TextWidth(grid_label_str) div 2),page_top_dots-(printmargin_wide div 2)-halfmm_dots-TextHeight('A'),grid_label_str); //  add labels.
                             ...
                             {else begin
                                    if (sheet_down mod 2)=0     // alternate pages only for banners.
                                       then begin
                                              ...
                                              text_out(grid_now_dots-(TextWidth(grid_label_str) div 2),bangrid_dots,grid_label_str);
                                            end;
                                  end;}
                        ...
                        {if banner_paper=False then} text_out(left_blanking_dots,page_top_dots-(printmargin_wide div 2)-halfmm_dots-TextHeight('A'),grid_str);  // add the units string.
                                              {else begin
                                                    if (sheet_down mod 2)=0     // alternate pages only for banners.
                                                       then begin
                                                              if staggered_pages=True
                                                              ...
                                                              text_out(left_blanking_dots,bangrid_dots,grid_str);
                                                            end;
                                                   end;}
              ...
              and (turnoutx>0)                  // not if invalidated
                 ...
                 then begin
                        ...
                        pdf_laz_draw_symbols(output_canvas,current_symbols,0,1,grid_left,grid_top,ypd);  // 227c draw the 0=droppers over timbers   1=first layer
                        ...
                        if (print_settings_form.output_rails_checkbox.Checked=True) or ( (detail_mode_form.thickcl_radio.Checked=True) and (detail_mode_form.no_platforms_radio.Checked=False) )    // 226e
                           ...
                           then begin
                                  ...
                                  pdf_laz_draw_symbols(output_canvas,current_symbols,0,2,grid_left,grid_top,ypd);  // 227c
                                  pdf_laz_draw_symbols(output_canvas,current_symbols,1,2,grid_left,grid_top,ypd);  // 227a draw the 1=gaps           2=second layer
                                  pdf_laz_draw_symbols(output_canvas,current_symbols,2,2,grid_left,grid_top,ypd);  // 227a draw the 2=stickers       2=second layer
              ...
              {if banner_paper=False
                 then} text_out(left_blanking_dots,top_blanking_dots,top_str);  // name and "who for?" string at topleft.
                 {else begin
                        if banner_top_done=False  //sheet_down=0     // only once for banners.
                           then begin
                                  ...
                                  text_out(left_blanking_dots,stry,top_str);    // name and "who for?" string at top left.

                                  stry:=0-Font.Height*7;
                                  Font.Assign(print_labels_font);                              // font for bottom string.
                                  Font.Color:=calc_intensity(clBlack);                                         // but in black.
                                  if print_entire_pad_flag=True then text_out(left_blanking_dots,stry,' start'+bottom_str)  // add the bottom string also.
                                                                else text_out(left_blanking_dots,stry,' '+bottom_str);
                                end;
                      end;}

{
              ...
              TextOut(page_left_dots+printmargin_wide+3, page_top_dots+printmargin_wide+2, DateToStr(Date)+' '+TimeToStr(Time));  // date and time in top left corner.
              TextOut(page_left_dots+printmargin_wide+3, page_bottom_dots+Font.Height-printmargin_wide-4, page_num_str);           // repeat page number in bottom left corner.

              TextOut(page_right_dots-printmargin_wide-TextWidth(page_num_str)-3,page_top_dots+printmargin_wide+2,page_num_str); // repeat page number in top right corner.

              if box_project_title_str<>''
                 then TextOut(page_right_dots-printmargin_wide-TextWidth(box_project_title_str)-3,page_bottom_dots+Font.Height-printmargin_wide-4,box_project_title_str);  // project title in bottom right corner.
            ...
            //Font.Assign(set_font('Arial',6,[],calc_intensity(clBlack)));
              ...
              { pdf out if (cal_ok=False) and (pdf_laz_form.warnings_checkbox.Checked=True)     //  no calibration done for this printer.
                 then begin
                        Font.Assign(set_font('Arial',7,[],printmargin_colour));

                        text_out(page_left_dots+printmargin_wide,page_top_dots+printmargin_wide-(Font.Height*3),
              ...
              if distortions<>0  // ) and (pdf_laz_form.warnings_checkbox.Checked=True)
                 then begin
                        Font.Assign(set_font('Arial',7,[],printmargin_colour));
              ...
              //if banner_paper=False then TextOut(left_blanking_dots,printer_length_indexmax_dots-halfmm_dots*2-TextHeight('A'),page_str+bottom_str); // add the bottom string last
    ...
    if Assigned(laz_metafile) then laz_metafile.Free;
    print_busy:=False;
    enable_buttons(True);        // ?? not really needed.
    pdf_laz_form.Close;//Hide;
...
begin
{
  if (stay_visible_form.Showing=True) and ( stay_visible_form.Left<(Left+Width+4) )
     then begin
            Left:=4;
  ...
  // 227a info_scrollbox.VertScrollBar.Position:=0;
  // 227ainfo_scrollbox.HorzScrollBar.Position:=0;
...
{
  if (print_entire_pad_flag=True) and (classic_templot=True)
     then begin
            if pad_form.include_current_template_menu_entry.Checked=False
               then Caption:=Caption+'  ( omitting  the  control  template )'
               else Caption:=Caption+'  ( including  the  control  template )';
          end;
}


{
  if pdf_black_white=True then no_intensity_panel.Visible:=True
                      else no_intensity_panel.Visible:=False;
}

  pad_form.top_toolbar_panel.Hide;
  pad_form.second_toolbar_panel.Hide;   // 217a
  //pad_form.third_toolbar_panel.Hide;    // 233a
  ...
  //pad_form.blue_corner_panel.Hide;
  //pad_form.sketchboard_button.Hide;
...
begin
{
  ...
  //creator_canv:=pdf_printer.Canvas;
end;
//______________________________________________________________________________
...
begin
  ...
  //pad_form.third_toolbar_panel.Show;    // 233a
  ...
  //pad_form.blue_corner_panel.Show;
  //pad_form.sketchboard_button.Show;
end;
//______________________________________________________________________________
...
var
  ...
  ////metafile_stream:TMemoryStream;

// OT2024  rotated_emf:TMetafile;
...
begin
  if {pad_form.print_shapes_on_menu_entry.Checked=False}  // 0.82.b
     print_settings_form.output_bgnd_shapes_checkbox.Checked=False then EXIT;
  ...
  with output_canvas do begin        // metafile canvas
                      ...
                      then begin
                             case shape_code of
                                    ...
                                    -1: begin     // picture = bitmap image.  !!! needs 90 deg rotate. 9-2-01.
                                          ...
                                          {if pdf_laz_form.picture_stretch_radio.Checked=True
                                             then begin}
                                                    ...
                                                    if Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]).bgnd_shape.picture_is_metafile=True
                                                       then begin
                                                              ...
                                                              with dummy_bitmap.Canvas do begin
                                                                Brush.Color:=output_canvas.Brush.Color;
                                                                Brush.Style:=bsSolid;
                                                                FillRect(dummy_rect);
                                                              ...
                                                              try
                                                                ...
                                                                //if Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]).bgimage.image_shape.draw_trans=True  // transparent option.
                                          ...
                                          //if pdf_laz_form.picture_dots_radio.Checked=True
                                          //   then pdf_rotate_bitmap_by_dots(i,p1,p2,grid_left,grid_top, Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]).bgimage.image_shape.image_bitmap.Canvas, output_canvas);
  ...
  with output_canvas do begin     // metafile canvas
    ...
    for n:=0 to maxbg_index do begin
      ...
      with now_keep do begin
        ...
        {]]]
        keep_index:=find_listkeep(timestamp);                                  // find it in the keeps box.
        if (keep_index<0) or (keep_index>(keeps_list.Count-1)) then CONTINUE;  //??? not in box ???
        }
                              ...
                              if (check_limits(infill_points[0],infill_points[1])=True) and (check_limits(infill_points[2],infill_points[3])=True)
                                 then begin
                                        ...
                                        case print_timb_infill_style of
                                                        ...
                                                        3: if {(impact>0) or} (pdf_black_white=True) or (mapping_colours_print<0)
                                                              then Brush.Style:=bsBDiagonal    // impact printer or plotter, or printing black and white or in a single colour.
                                                              else Brush.Style:=bsSolid;
...
var
  ...
  //drawn_full_aq1,drawn_full_aq2:boolean;
                  ...
                  begin
                    with now_keep do begin
                      ...
                      with output_canvas do begin
                        ...
                        {if (single_colour_flag=True) and (pdf_laz_form.black_edges_checkbox.Checked=False)
...
(*
  ...
  with output_canvas do begin
    ...
    for n:=0 to max_list_index do begin
      ...
      if kludge=False
         then pdf_laz_draw_symbols(output_canvas,now_ti.file_blocks.keep_symbols,0,1,grid_left,grid_top,bgnd_y_datum);  // 227a draw the 0=droppers over timbers  1=polygon only
      ...
      if kludge=False
         then begin
                pdf_laz_draw_symbols(output_canvas,now_ti.file_blocks.keep_symbols,0,2,grid_left,grid_top,bgnd_y_datum);  // 227c
                pdf_laz_draw_symbols(output_canvas,now_ti.file_blocks.keep_symbols,1,2,grid_left,grid_top,bgnd_y_datum);  // 227a draw the 1=gaps
                pdf_laz_draw_symbols(output_canvas,now_ti.file_blocks.keep_symbols,2,2,grid_left,grid_top,bgnd_y_datum);  // 227a draw the 2=stickers
              end;
...
begin
  //help_form.info_label.Height:=help_form.info_label.Height*2;       // out 0.91 ensure room for this long text.
  if help(-1,pdf_help_str,'about  mapping  colours')=1 then pad_form.marker_and_mapping_colours_help_menu_entry.Click;
  //help_form.info_label.Height:=help_form.info_label.Height div 2;   // out 0.91 he might have re-scaled the help form !! This way should be ok.
end;
...
//_______________________________________________________________________________________
{
procedure Tpdf_laz_form.page_map_buttonClick(Sender: TObject);

begin
  if (pdf_laz_form.pdf_printer.Printing=True) or (printer_printing=True)
     then index_sheet_wanted:=True                          // wanted later.
     else begin
            index_sheet_wanted:=False;                      // print immediately.
            print_index_sheet(True);
          end;
end;
//___________________________________________________________________________________________
}
...
// OT2024  if output_in_progress=False then print_preview(True,True,0);  // redraw in new font.
end;
//__________________________________________________________________________________________
...
{
procedure Tpdf_laz_form.print_intensity_trackbarChange(Sender: TObject);
...
procedure Tpdf_laz_form.reset_buttonClick(Sender: TObject);
...
procedure Tpdf_laz_form.dark_labelClick(Sender: TObject);
...
procedure Tpdf_laz_form.light_labelClick(Sender: TObject);
...
//________________________________________________________________________________________
{
procedure pdf_rotate_bitmap_by_dots(i:integer; p1,p2:Tpex; grid_left,grid_top:extended; source_canv,dest_canv:TCanvas);
...
begin
  ...
  try
    ...
    if height_pixels<32767 then pdf_laz_form.row_progressbar.Max:=height_pixels    // Delphi 16-bit limits..
                           else pdf_laz_form.row_progressbar.Max:=32767;
    pdf_laz_form.row_progressbar.Position:=0;

    saved_caption_str:=pdf_laz_form.Caption;
    ...
    for inrow:=0 to (height_pixels-1) do begin
      for incol:=0 to (width_pixels-1) do begin
        ...
        if pdf_check_if_on_page(out1,out2,True)=False then CONTINUE;  // blob not on this page or outside limits.

        pdf_laz_form.Caption:='  '+name_str+'  row: '+IntToStr(inrow)+'  col: '+InttoStr(incol);
    ...
    pdf_laz_form.row_progressbar.Position:=0;
    pdf_laz_form.Caption:=saved_caption_str;
    Screen.Cursor:=saved_screen_cursor;
  end;//try
end;
//_________________________________________________________________________________________

procedure Tpdf_laz_form.picture_dots_radioClick(Sender: TObject);
...
begin
  repeat
    i:=alert(3,'    print  picture  shapes  ( dots  method )',
               'The DOTS method of printing background picture shapes is EXTREMELY slow.',
               '','','more  information','O K  -  print  pictures  using  dots  method','cancel  -  print  pictures  as  outlines  only','print  pictures  using  stretch  method  instead',3);
    case i of
          3: if help(0,picture_help_str,'more  about  picture  shapes')=1 then bgnd_form.help_button.Click;          // !!! don't use alert_help, pdf_laz_form won't permit being deactivated.
...
procedure Tpdf_laz_form.picture_help_buttonClick(Sender: TObject);

begin
  if help(0,picture_help_str,'more  about  picture  shapes')=1 then bgnd_form.help_button.Click;
end;
//__________________________________________________________________________________________
...
function pdf_check_if_on_page(corner1,corner2:TPoint; check_within_limits:boolean):boolean;
...
//______________________________________________________________________________
(*
procedure Tpdf_laz_form.include_sketchboard_items_checkboxClick(Sender: TObject);   // 206e
