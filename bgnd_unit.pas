...
uses
  ...
  map_loader_unit, action_unit, map_clarity_unit, brick_unit, dxf_unit;  // 555a
...
begin
  with bgnd_form.bgnd_shapes_listbox do begin
    ...
    if Tbgshape(Items.Objects[ItemIndex]).bgnd_shape.shape_code<>4 then EXIT;   // not a target/clip
    if Tbgshape(Items.Objects[ItemIndex]).bgnd_shape.is_brick=False then EXIT;  // not a brick clip

    m:=find_paired_clip(ItemIndex);

    if m<0 then EXIT;  // selected clip not paired

    Tbgshape(Items.Objects[m]).bgnd_shape.p1:=Tbgshape(Items.Objects[ItemIndex]).bgnd_shape.p1;  // same position
    Tbgshape(Items.Objects[m]).bgnd_shape.p2:=Tbgshape(Items.Objects[ItemIndex]).bgnd_shape.p2;  // same size/direction,  twist angle

    Tbgshape(Items.Objects[m]).bgnd_shape.show_transparent:= NOT Tbgshape(Items.Objects[ItemIndex]).bgnd_shape.show_transparent;   // opposite gender   tommy bar / claws
...
procedure shapes_current_state;     // set the current state of the form to match the selected shape.
   ...
   // 234c ensure selected paired clips match size and position
...
begin
  ensure_selected_clip_pairs_match;  // 234c  after any changes to a paired clip

  with bgnd_form do begin

    if (line_radio_button.Checked=True) and (by_drawing_radiobutton.Checked=True)
       then add_shape_button.Caption:='add  line  shapes'
    ...
    with bgnd_shapes_listbox do begin
      ...
      allow_shape_sync_checkbox.Enabled:=(Items.Count>0);     // 226b
      ...
      if Items.Count<1                              // list empty...
         then begin
                ...
                save_all_brick_shapes_menu_entry.Enabled:=False;
                save_current_brick_shapes_menu_entry.Enabled:=False;  // 234d
                ...
                size_label.Caption:='';       // 226e

                disable_style_box;

                trans_checkbox.Checked:=False;

                allow_shape_sync_checkbox.Checked:=True;  // 226b default

                set_as_brick_checkbox.Checked:=False; // 234b
                brick_label_checkbox.Checked:=False;  // 234a

                brick_wall_checkbox.Checked:=False;  // 234e
      ...
      for n:=0 to (Items.Count-1) do Items.Strings[n]:=Tbgshape(Items.Objects[n]).bgnd_shape.shape_name;  // update list  234c
    ...
    with now_shape do begin
      ...
      allow_shape_sync_checkbox.Checked:=((option_bits AND $01)=0);   // 226b

      brick_label_checkbox.Checked:=((option_bits AND $04)<>0);   // 234a

      brick_wall_checkbox.Checked:=is_brick_wall;  // 234e

      set_as_brick_checkbox.Checked:=is_brick;  // 234b
      ...
      case shape_code of
                  ...
                  -1: begin                      // picture.
                        ...
                        brick_label_checkbox.Enabled:=False;
                        brick_wall_checkbox.Enabled:=False;
                        set_as_brick_checkbox.Enabled:=False;
                        ...
                        convert_24bit_button.Enabled:=True;         // 215b
                        convert_24bit_menu_entry.Enabled:=True;     // 215b

                        auto_fit_picture_button.Enabled:=True;
                        copy_image_button.Enabled:=True;

                        map_clarity_button.Enabled:=True;
                        ...
                        twist_rectangle_button.Enabled:=False;     // 234d
                   ...
                   0: begin                      // line.
                        ...
                        brick_label_checkbox.Enabled:=True;
                        brick_wall_checkbox.Enabled:=True;
                        set_as_brick_checkbox.Enabled:=True;

                        reload_picture_button.Enabled:=False;
                        save_picture_button.Enabled:=False;
                        twist_picture_button.Enabled:=False;

                        convert_24bit_button.Enabled:=False;        // 215b
                        convert_24bit_menu_entry.Enabled:=False;    // 215b

                        auto_fit_picture_button.Enabled:=False;
                        copy_image_button.Enabled:=False;

                        map_clarity_button.Enabled:=False;
                   ...
                   1: begin                      // rectangle.
                        ...
                        if rect_rot=0
                           then crop_rectangle_button.Enabled:=True     // 214a
                           else crop_rectangle_button.Enabled:=False;   // 234b
                        ...
                        twist_rectangle_button.Enabled:=True;     // 234d
                   ...
                   4: begin                      // target mark / brick clip
                        ...
                        if is_brick=True                     // 233c
                           then code_str:='brick  clip'
                           else code_str:='target  mark';
      ...
      improve_map_clarity_menu_entry.Enabled:=map_clarity_button.Enabled;
      ...
      if shape_code<3
         then begin
                ...
                if shape_code=0  // line
                   then size_label.Caption:='                 line length =  '+round_str(SQRT(SQR(p2.x-p1.x)+SQR(p2.y-p1.y)),2)+' mm'
                   else size_label.Caption:=insert_crlf_str('width =  '+round_str(ABS(p2.x-p1.x),2)+' mm          depth =  '+round_str(ABS(p2.y-p1.y),2)+' mm'
                                                          +'|                   diagonal =  '+round_str(SQRT(SQR(p2.x-p1.x)+SQR(p2.y-p1.y)),2)+' mm');
              end
    ...
    save_all_brick_shapes_menu_entry.Enabled:=(any_bricks>0);                // 234b
    save_current_brick_shapes_menu_entry.Enabled:=(any_current_brick>0);     // 234d
...
function get_shape_dims(modifying:boolean; code:integer; var p1,p2:Tpex; var rect_rot:extended):boolean;

const
  clip_twist_str:string='      Brick Clip Twist Angle'

  +'||Set the twist angle of this brick clip by entering the angle in degrees.'
  +'||Angles should be entered positive in the anti-clockwise direction from horizontal, and negative in the clockwise direction.';

  slab_twist_str:string='      Rectangle/Brick Slab Twist Angle'

  +'||Set the twist angle of this rectangle/slab by entering the angle in degrees.'
  +'||Angles should be entered positive in the anti-clockwise direction from horizontal, and negative in the clockwise direction.';

  target_str:string='      Target Mark Arm Length / Brick Size'

  +'||Set the size of this target mark or brick clip by entering the length (each way from the centre) of the horizontal and vertical arms.'
  +'||Entering a negative size will swap the direction of the clip.'
  ...
  +'||For more information about using conversion factors click the ? HELP button.'
  +'||The pre-set target arm length (enter a slash / ) is 10mm.';
     ...
     then begin
            case type_code of
                 ...
                 1: begin                                            // rectangle
                      ...
                      putdim(y_str,1,str1+'second'+str2+'Y  dimension  Y2 ',p2.y,False,False,False,False);  // ditto.

                   i:=putdim(slab_twist_str,3,'rectangle / slab  twist  angle',rect_rot*180/Pi,False,True,False,False);          // neg ok, no preset, zero ok, don't terminate on zero.
                      ...
                      if getdims('background  '+str1,get_str,bgnd_form,i,od)=True
                         then begin
                                ...
                                p2.x:=od[2];
                                p2.y:=od[3];
                                rect_rot:=od[4]*Pi/180 // twist angle for slab
                              end
                         else EXIT;
                    end;
                 ...
                 4: begin
                       putdim(x_str,1,'target  mark / clip  centre  X  dimension',p1.x,False,True,False,False);   // neg ok, no preset, allow zero, don't terminate on zero.
                       putdim(y_str,1,'target  mark / clip  centre  Y  dimension',p1.y,False,True,False,False);   // ditto.
                       putdim(target_str,1,'target  arm  length / clip  size',p2.x,False,False,False,False);      // neg ok, preset ok, zero ok, don't terminate on zero.
                    i:=putdim(clip_twist_str,3,'clip  twist  angle',p2.y*180/Pi,False,True,False,False);          // neg ok, no preset, zero ok, don't terminate on zero.
                      ...
                      if getdims('background  target  mark',get_str,bgnd_form,i,od)=True
                         then begin
                                ...
                                p2.y:=od[3]*Pi/180;                     // twist angle for clip
                                target_arm:=p2.x;                       // so can show the same for the next target.
                              end
                         else EXIT;
                    end;

               else begin            // -1, 0      picture, line
                      case code of
         ...
         then begin
                ...
                if ok_flag=True
                   then begin
                          ...
                          if img_bmp.PixelFormat<>pf8bit then img_bmp.PixelFormat:=pf24bit;    // 215b  down from 32bit for deep zooming (also workaround for TPngImage on lower than 8bit)
              ...
              with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[n]) do begin
                ...
                if bgimage<>nil       // clear any existing (this may be a reload).
                   then begin
                          ...
                          bgimage.image_shape.undo_bitmap.Free;   // 227a
(* OT2024
...
// OT2024                image_metafile:=TMetafile.Create;
// OT2024                rotated_metafile:=TMetafile.Create;

                rotated_picture:=TPicture.Create;

                try
 (* OT2024                  if dropped_picture.Graphic is TMetafile
                     then begin
...
(* OT2024
              ...
              if do_twain(new_bmp)=True    // get scan
                   then begin
                          ...
                          if new_bmp.PixelFormat<>pf8bit then new_bmp.PixelFormat:=pf24bit;    // 215b  down from 32bit for deep zooming (also workaround for TPngImage on lower than 8bit)
    ...
    if was_cancelled=False     // 211b
       then begin
              ...
              with bgnd_form.picture_load_dialog do begin  // 0.93.a
                ...
                if meta=True
                   then begin
                          show_modal_message('The file dialog will display EMF/WMF metafiles as requested.'
...
*)
      ...
      image_bitmap.PixelFormat:=pf24bit;    // 215b    was 8bit
...
function get_sk8_image(n:integer; old_bmp_file_str,sk8_str:string; var img_width,img_height:integer):string;    // 3-2-01.
         ...
         // used only for old-format BGS files prior to BGS3 files in version 225
...
*)
      ...
      if sk8_exists=True     // png format, 0.93.a
         then begin
// OT2024       create_png:=TPNGObject.Create;

                try

                  try
// OT2024           create_png.LoadFromFile(sk8_str);
                  except
                    sk8_exists:=False;
                    bgfile_error(sk8_str);
                  end;//try

//  OT2024        if sk8_exists=True then image_bitmap.Assign(create_png);

                finally
// OT2024         create_png.Free;
                end;//try
...
const
  ...
  +'||Enter the required width of this picture shape on your plan. Templot will scale the bitmap image to fit.';
  ...
  picture_sizing_help_str:string='    `0Sizing  Method`9'
    ...
    +' Templot will then scale the picture shape correctly to match your drawing.'
...
begin
  ...
  dpi_str:='rp.gif  The width-wise and height-wise DPI settings are entered separately. In the vast majority of cases the setting is the same for both, for example from a scanner.'
          ...
          +'||green_panel_begin tree.gif Do not set very high resolutions as this may severely slow down the screen refresh and zooming in Templot.'
     ...
     then begin
            ...
            repeat
              ...
              i:=alert(2,'        auto - fit  picture  shape',
                      ...
                      +'||If you answered yes to all of the above Templot can automatically set the picture shape to the'
  ...
  repeat
    ...
    i:=alert(4,'php/401    sizing  method ?',
             ...
             +'||If you click one of the blue bars Templot can size the picture shape automatically, but you will need to enter some information about the scanner DPI setting and the real-world scale of the track plan image.'
...
function get_rgb_text(col:integer):string;  // 234a brick label - default=colour RGB
...
begin
  ...
  green:=(col AND $0000FF00) div $100;
  blue:=(col AND $00FF0000) div $10000;
...
begin
  with RESULT do begin
    ...
    shape_name:='init';    // name or text string.  205e

    wrap_offset:=0;        // 1/100th mm for picture shape wrapping  205e     also used for brick colour 229a

    show_transparent:=False; // 0.93.a  for bitmap image    234a also for direction of target clip

    shape_code:=0;           // 0=line, 1=rectangle, 2=circle, 3=text label, 4=target mark / brick clip, -1=picture (bitmap image or metafile)

    shape_style:=0;          // 0=transparent, 1=blank/solid, 2=cross-hatched/dotted line;  // 213b was integer (stored MSB (byte) first)

    picture_is_metafile:=False;      // 213b     // spare1:boolean;

    hide_bits:=0;           // 214a  0=normal,  1=hide on trackpad,  2=hide on output,  3=hide both // spare2:byte;

    option_bits:=0;         // 226b  0=normal, 1 bit = don't allow sync       219b.1 spare3:byte;
    ...
    p1.x:=0;                    // first corner/end
    p1.y:=0;

    p2.x:=0;                    // second corner/end or other data
    p2.y:=0;

    rect_rot:=0;         // added 234b  rotation angle for rectangles

    clip_pairing_code:=0;   // TDateTime;  234c  for brick clips  0=not paired

    is_brick:=False;     // added 234b

    is_kerf:=False;     // 244a

    is_brick_boundary_rect:=False;     // added 234d

    is_brick_wall:=False;   // 234e

    wall_width:=0;     // 234e
    wall_depth:=0;     // 234e

    brick_label_scaling:=1.0;  // 236c
...
begin
  ...
  with bgnd_form do begin
    ...
    with new_shape do begin
      ...
      wrap_offset:=0;   // keep compiler happy

      rect_rot:=0;  // 234b

      clip_pairing_code:=0;  // 234c not yet paired if clip

      show_transparent:=False;      // default (used only for images),   and for target clip/type
      ...
      case shape_code of
          0,1,4: is_brick:=add_brick_checkbox.Checked;  // line, rectangle, target only   234b

            else is_brick:=False;
      end;//case

      if shape_code=0                                   // line
         then is_kerf:=add_kerf_lines_checkbox.Checked  // 244a
         else is_kerf:=False;

      if is_brick=True                                                // 234c
      ...
      case shape_code of
                ...
                4: begin        // target mark
                     ...
                     p2.x:=8*inscale;   // size mm   8" default

                     p2.y:=0; // rotation angle
      ...
      case add_option of
         ...
         0,2: begin   // drawn rectangle or clicked corners
           ...
           1: begin      // add_option=1  enter dimensions...
                repeat
                  if get_shape_dims(False,shape_code,p1,p2,rect_rot)=False then EXIT;
         ...
         then begin
                ...
                case shape_code of
                 ...
                 -1: shape_name:=drawn_str+'picture';
                  ...
                  0: begin
                       if is_brick=True
                          then shape_name:=drawn_str+'brick splint'
                          else if is_kerf=True
                                  then shape_name:=drawn_str+'kerf line'
                                  else shape_name:=drawn_str+'line';
                     end;

                  1: begin
                       if is_brick=True
                          then shape_name:=drawn_str+'brick slab'
                          else shape_name:=drawn_str+'rectangle';
                     end;
                  ...
                  4: begin
                       if is_brick=True
                          then shape_name:=drawn_str+'brick clip'
                          else shape_name:=drawn_str+'target mark';
                     end;
                end;//case
              end;
      ...
      with bgnd_form.bgnd_shapes_listbox do begin
        with Items do begin
          ...
          Tbgshape(Objects[n]).bgimage:=nil;                  // no image yet
  ...
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    if Tbgshape(Objects[n]).bgimage<>nil
       then begin
              ...
              Tbgshape(Objects[n]).bgimage.image_shape.undo_bitmap.Free;   // 227a
(* OT2024
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with now_shape do begin
      ...
      if get_shape_dims(True,shape_code,p1,p2,rect_rot)=False then EXIT;       // get the data for this shape. True=modifying
         ...
         then begin                                           // so free the image (but not the shape object).
                if Tbgshape(Items.Objects[n]).bgimage<>nil    // ???
                     then begin
                            ...
                            Tbgshape(Items.Objects[n]).bgimage.image_shape.undo_bitmap.Free;   // 227a
(* OT2024
...
begin
  with bgnd_shapes_listbox do begin
    ...
    n:=ItemIndex;
    if n<0 then EXIT;                          // nothing was selected.
    if n>(Items.Count-1) then EXIT;            // ???
  ...
  if alert(7,'      convert  rectangle ?',
            ...
            +'||This will remove any hatched or solid filling.'
...
function any_bricks:integer;      // 234b
...
begin
  ...
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    for n:=0 to Count-1 do begin
      ...
      if Tbgshape(Objects[n]).bgnd_shape.is_brick=True then RESULT:=RESULT+1;
...
function any_current_brick:integer;  // 234d

 // current brick matches marker colour
...
begin
  ...
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    for n:=0 to Count-1 do begin
      ...
      if (Tbgshape(Objects[n]).bgnd_shape.is_brick=True) and (Tbgshape(Objects[n]).bgnd_shape.wrap_offset=brick_form.store_marker_colour_panel.Color) then RESULT:=RESULT+1;
...
function save_all_shapes_to_file(file_str:string; bricks_only:integer; readable:boolean):boolean;     // 225a  new BGS3 XML format  25-NOV-2019
  ...
  // bricks_only  0=no, 1=all brick shapes, 2=current brick only

var
  xml_doc:TNativeXml;
  shape_node,header_node:TXmlNode;

  n,saved_count:integer;
  met_size:integer;
  met_copied:integer;
  p:Pointer;
...
begin
  ...
  if (bricks_only=1) and (any_bricks=0) then EXIT;         // ???
  if (bricks_only=2) and (any_current_brick=0) then EXIT;  // ???
  ...
  xml_doc.UseFullNodes:=True;  // 225b
  ...
  try
    ...
    with bgnd_form.bgnd_shapes_listbox.Items do begin
      ...
      with header_node do begin
        ...
        case bricks_only of
             1: WriteInteger('shapes_count',any_bricks,0);
             2: WriteInteger('shapes_count',any_current_brick,0);
           else WriteInteger('shapes_count',Count,0);
        end;//case
      ...
      for n:=0 to Count-1 do begin
        ...
        if (bricks_only>0) and (Tbgshape(Objects[n]).bgnd_shape.is_brick=False) then CONTINUE;      // not a brick

        if (bricks_only=2) and (Tbgshape(Objects[n]).bgnd_shape.wrap_offset<>brick_form.store_marker_colour_panel.Color) then CONTINUE;     // not current brick

        INC(saved_count);

        shape_node:=create_named_node(xml_doc.Root,'BGND_SHAPE_'+IntToStr(saved_count));
        ...
        with Tbgshape(Objects[n]) do begin
          ...
          with bgnd_shape do begin
            ...
            case shape_code of    // 234b mods
                 ...
                 4: begin              // target mark / 3D brick clip
                      shape_style:=0;  // not used for target

                      try
                        show_transparent:=(integer(show_transparent)=1);     // clip  gender  claws or tommy bar
                      except
                        show_transparent:=False;
                      end;
            ...
            if hide_bits>3 then hide_bits:=0;    // other bits not used
            ...
            with shape_node do begin
              ...
              WriteInteger('wrap_offset',wrap_offset,clSilver);  // 1/100ths mm

              WriteBool('show_transparent',show_transparent,False);     // image, brick gender

              WriteInteger('shape_code',shape_code,0);    // 0=line, 1=rectangle, 2=circle, 3=text, 4=target mark / brick clip, -1=picture (bitmap image or metafile)
              ...
              WriteFloat('rect_rot',rect_rot,0);  // 234b

              WriteBool('is_brick',is_brick,False);  // 234b

              WriteBool('is_brick_boundary_rect',is_brick_boundary_rect,False);  // 234d

              WriteBool('is_brick_wall',is_brick_wall,False);   // 234e

              WriteFloat('wall_width',wall_width,0);     // 234e
              WriteFloat('wall_depth',wall_depth,0);     // 234e

              WriteFloat('brick_label_scaling',brick_label_scaling,1);    // 236c

              WriteBool('is_kerf',is_kerf,False);   // 234b

              clip_code:=clip_pairing_code;         // 234c
              WriteFloat('clip_code',clip_code,0);  // 234c
                 ...
                 then begin
                        with bgimage.image_shape do begin
                          ...
                          if picture_is_metafile=True   // metafile...
                             then begin
(* OT2024
                                    WriteFloat('emf_width_mm',image_metafile.MMWidth/100,0);    // EMF frame size in mm
                                    WriteFloat('emf_height_mm',image_metafile.MMHeight/100,0);
*)
                                    save_stream:=TMemoryStream.Create;
                                    save_stream.Position:=0;

                                    try
// OT2024                                     image_metafile.SaveToStream(save_stream);
                                    except
                                      failed:=True;
                                      EXIT;
                                    end;

                                    if save_stream.Size>0
                                       then begin
                                              save_stream.Position:=0;
                                              XmlWriteHCKStream(shape_node,'EMF',save_stream);
                                            end
                                       else begin
                                              failed:=True;
                                              EXIT;
...
(* OpenTemplot metafile code in Lazarus ...

                                    met_size:=GetEnhMetaFileBits(image_metafile.Handle,0,nil);  // get size of EMF data
                                    ...
                                    met_copied:=GetEnhMetaFileBits(image_metafile.Handle,met_size,p);   // get the EMF contents into buffer
...
begin
  ...
  bricks_only:=TMenuItem(Sender).Tag;              // 234b   tag code 1 or 2

  if bgnd_shapes_listbox.Items.Count<1 then EXIT;

  if (bricks_only=1) and (any_bricks=0)
     then begin
            show_modal_message('invalid: there are no brick shapes');
            EXIT;
          end;

  if (bricks_only=2) and (any_current_brick=0)
     then begin
            show_modal_message('invalid: there are no current brick shapes');
            EXIT;
          end;
  ...
  if user_save_shapes_path=''
    ...
    col_str:=StringReplace(get_rgb_text(brick_form.store_marker_colour_panel.Color),'#','-',[]);     // 234d

    case bricks_only of

        1: FileName:=remove_invalid_str(Copy(Trim('bricks__'+box_project_title_str),1,fname_trunc)+FormatDateTime(' yy_mm_dd hhmm ss',Date+Time))+'.bgs3';

        2: FileName:=remove_invalid_str(Copy(Trim('brick'+col_str+'__'+box_project_title_str),1,fname_trunc)+FormatDateTime(' yy_mm_dd hhmm ss',Date+Time))+'.bgs3';   // 234d

      else FileName:=remove_invalid_str(Copy(Trim(box_project_title_str),1,fname_trunc)+FormatDateTime(' yy_mm_dd hhmm ss',Date+Time))+'.bgs3';
  ...
  if filesave_dialog.Execute=True                // get her file name.
     then begin
            ...
            if save_all_shapes_to_file(bgs_str,bricks_only,control_room_form.bgs3_readable_menu_entry.Checked)=False    // readable/compact XML format
               then begin
...
function load_shapes_from_file(file_str:string; reload_limits:boolean):boolean;     // 225a  new BGS3 XML format  25-NOV-2019

var
  xml_doc:TNativeXml;
  shape_node,header_node:TXmlNode;

  i,n,shapes_count,saved_from_version:integer;

  next_shape:Tbgnd_shape;

  load_stream:TMemoryStream;

  clip_code:double;  // 234c
...
begin
  ...
  saved_from_version:=header_node.ReadInteger('program_version',0);

  next_shape:=init_bgnd_shape;  // 234b
    ...
    for n:=0 to shapes_count-1 do begin
      ...
      with next_shape do begin
        with shape_node do begin
          ...
          wrap_offset:=ReadInteger('wrap_offset',clSilver);  // 1/100ths mm
          ...
          option_bits:=ReadInteger('option_bits',0);  // byte

          if saved_from_version<227 then option_bits:=(option_bits OR $02); // pre_227a
          ...
          rect_rot:=ReadFloat('rect_rot',0);     // 234b

          is_brick:=ReadBool('is_brick',False);  // 234b

          is_brick_boundary_rect:=ReadBool('is_brick_boundary_rect',False);  // 234d


          is_brick_wall:=ReadBool('is_brick_wall',False);   // 234e

          wall_width:=ReadFloat('wall_width',0);     // 234e
          wall_depth:=ReadFloat('wall_depth',0);     // 234e

          brick_label_scaling:=ReadFloat('brick_label_scaling',1);    // 236c

          is_kerf:=ReadBool('is_kerf',False);      // 244a

          clip_code:=ReadFloat('clip_code',0);     // 234c
          clip_pairing_code:=clip_code;            // 234c
          ...
          if is_brick=True
             then brick_form.store_marker_colour_panel.Color:=wrap_offset;   // 234d
...
*)
                          ...
                          if picture_is_metafile=True
                             then begin
(* OT2024
                                    image_metafile.MMWidth:=Round(ReadFloat('emf_width_mm',0)*100);      // EMF frame size in mm
                                    image_metafile.MMHeight:=Round(ReadFloat('emf_height_mm',0)*100);
*)
                                    ...
                                    if load_stream.Size>0
                                       then begin
                                              load_stream.Position:=0;
// OT2024                                              image_metafile.LoadFromStream(load_stream); // image_metafile.Handle:=SetEnhMetaFileBits(load_stream.Size,load_stream.Memory);
                                            end;
                                    load_stream.Free;
                                  end
    ...
    Sleep(1);    // 227a allow showing it closes while images being drawn...
...
var
  ...
  i,n:integer;
  bgs_str:string;
  next_shape:Told_bgnd_shape;    // 234b mod
  dummy1,dummy2:integer;
  top_str:string;
    ...
    try
                ...
                if (mru=False) and (dropped=False)
                   then begin
                          with fileload_dialog do begin
                            ...
                            Filter:='background  shapes  ( .bgs3 ,  .bgs)|*.bgs3;*.bgs';
      ...
      if FileExists(bgs_str)=False
         then begin
                ...
                if LowerCase(ExtractFileExt(bgs_str))='.bgs3'   // 225a new XML format
                   then begin
                        ...
                        // load old BGS format ...
                          ...
                          if LowerCase(ExtractFileExt(bgs_str))<>'.bgs'     // ???
                             then begin
                                    bgfile_error(bgs_str);
                                    user_load_shapes_path:='';
                                    if append=False then clear_shapes;    // remove any added
                            ...
                            // mods for 234b load Told_bgnd_shape ...
                          ...
                          try
                            ...
                            While Eof(shape_file)=False do begin
                              ...
                              with bgnd_shapes_listbox do begin
                                ...
                                Tbgshape(Items.Objects[n]).bgnd_shape:=init_bgnd_shape;  // 234b      // 234b
                                ...
                                next_shape.option_bits:=$02;   // set pre 227a file

                                    // copy loaded data to list ...

                                with Tbgshape(Items.Objects[n]).bgnd_shape do begin   // 234b   put data in list, modified

                                           shape_name:=next_shape.shape_name;         // name or text string.  205e

                                          wrap_offset:=next_shape.wrap_offset;        // 1/100th mm for picture shape wrapping  205e     also used for brick colour 229a

                                     show_transparent:=next_shape.show_transparent;   // 0.93.a  for bitmap image    234a also for direction of target clip

                                           shape_code:=next_shape.shape_code;         // 0=line, 1=rectangle, 2=circle, 3=text label, 4=target mark / brick clip, -1=picture (bitmap image or metafile)

                                          shape_style:=next_shape.shape_style;        // 0=transparent, 1=blank/solid, 2=cross-hatched/dotted line;  // 213b was integer (stored MSB (byte) first)

                                  picture_is_metafile:=next_shape.picture_is_metafile;

                                            hide_bits:=next_shape.hide_bits;             // 214a  0=normal,  1=hide on trackpad,  2=hide on output,  3=hide both

                                          option_bits:=next_shape.option_bits;           // 226b  0=normal, 1 bit = don't allow sync

                                                              // msb  map tile, has_been_interpolated , has_been_2_toned , has_been_low_contrast , has_been_negative , line is brick label tab , pre_227a , don't allow sync    lsb
                                                              //        $80         $40                    $20                 $10                      $08                     $04                $02              $01

                                                   p1:=next_shape.p1;   // first corner/end
                                                   p2:=next_shape.p2;   // second corner/end or other data

                                  rect_rot:=0;         // added 234b  rotation angle for rectangles, not valid in old BGS files
...
begin
  ...
  if Screen.PixelsPerInch>120
     then begin
            bgnd_shapes_listbox.IntegralHeight:=False;   // 227a
...
begin
  ...
  redraw(True);  // 234c  to show updated paired clips
end;
...
begin
  ...
  if n<>1 then EXIT;
  if getdims('shift  all  background  shapes','',bgnd_form,n,od)=True then shift_all_shapes(False,od[0],od[1]);   // False=not sync  226b
      ...
      if shape_code<3                   // picture, line, rectangle, circle.
         then begin
                ...
                if shape_code=-1  // picture
                   then begin
                          if bgnd_form.picture_lock_to_notch_radiobutton.Checked=True
                             then begin
                                    p1.x:=notchx+(p1.x-notchx)*xfactor;      // lock to notch
                                    p1.y:=notchy+(p1.y-notchy)*yfactor;
                                  end;

                          if bgnd_form.picture_lock_to_ring_radiobutton.Checked=True
                             then begin
  ...
  with bgnd_form.bgnd_shapes_listbox do begin
    ...
    for n:=0 to (Items.Count-1) do begin
      ...
      with now_shape do begin
        ...
        shape_width:=0;     // keep compiler happy..
        shape_height:=0;

        if bgnd_form.lock_grid_radiobutton.Checked=False
           then begin

                  if shape_code<>4    // not target/clip
                     then begin
                            shape_width:=p2.x-p1.x;           // scale the size
                            shape_height:=p2.y-p1.y;
                          end;

                  if bgnd_form.lock_notch_radiobutton.Checked=True
                     then begin
                            p1.x:=notchx+(p1.x-notchx)*xfactor;      // lock to notch
                            p1.y:=notchy+(p1.y-notchy)*yfactor;
                          end;

                  if bgnd_form.lock_ring_radiobutton.Checked=True
                     then begin
                            p1.x:=rings[0,0]+(p1.x-rings[0,0])*xfactor;   // lock to spacing-ring
                            p1.y:=rings[0,1]+(p1.y-rings[0,1])*yfactor;
                          end;
           ...
           else begin                   // scale on grid origin
                  p1.x:=p1.x*xfactor;
                  p1.y:=p1.y*yfactor;
      ...
      if (shape_code=1) and ( (xfactor=-1) xor (yfactor=-1) )  // rectangle  mirror one axis    244a
         then rect_rot:=0-rect_rot;
...
procedure shift_all_shapes(sync:boolean; xshift,yshift:extended);
  ...
  with bgnd_form.bgnd_shapes_listbox do begin
    ...
    for n:=0 to (Items.Count-1) do begin
      ...
      with now_shape do begin
        ...
        if (sync=True) and ((now_shape.option_bits AND $01)<>0)   // 226b   sync not allowed on this one
           then CONTINUE;
  ...
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    with Tbgshape(Objects[n]).bgnd_shape do begin      // get existing data.
      ...
      case shape_code of
       ...
       1,2: begin                       // rectangle, circle shapes
              ...
              if shape_code=1                      // twist rectangle by same amount  234d
                 then rect_rot:=rect_rot+rot_k;
       ...
       3,4: begin                                   // label shapes, target marks.
              ...
              if (shape_code=4) and (is_brick=True)  // twist brick clip
                 then p2.y:=p2.y+rot_k;
...
begin
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    for n:=0 to (Count-1) do begin
      ...
      if (sync=True) and ((Tbgshape(Objects[n]).bgnd_shape.option_bits AND $01)<>0)   // 226b   sync not allowed on this one
         then CONTINUE;

      if (Tbgshape(Objects[n]).bgnd_shape.shape_code=-1) and (pictures=True)        // picture shape
         then begin
                twist_angle:=0-rot_k;
...
begin
  ...
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    with Tbgshape(Objects[n]).bgnd_shape do begin
      ...
      if shape_code<>4   // not a target mark / clip
         then EXIT;
...
procedure rotate_this_clip_slab(angle:extended);     // 229a  234b
...
begin
  ...
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    with Tbgshape(Objects[n]).bgnd_shape do begin
      ...
      if shape_code=1           // rectangle / slab
         then rect_rot:=angle;

      if shape_code=4           // target mark / brick clip
         then p2.y:=angle;
...
begin
  ...
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      if is_brick_boundary_rect=True
         then begin
                if alert(3,'    re-sizing  brick  boundary  rectangle',
                         'The currently selected background shape is a brick boundary rectangle.'
                        +'||If you re-size it, it may no longer match your 3D printer build plate.',
...
begin
  ...
  j:=bgnd_shapes_listbox.ItemIndex;

  if j<0 then EXIT;
  if j>(bgnd_shapes_listbox.Items.Count-1) then EXIT;
  ...
  n:=putdim(rotshap_str+rot_cen_str,3,'rotate  background  shape  around  '+cen_str+'  by',0,False,True,False,False);   // negative ok, no preset, zero ok, don't terminate on zero.
...
begin
  with bgnd_shapes_listbox do begin
    ...
    if Tbgshape(Items.Objects[n]).bgnd_shape.is_brick_boundary_rect=True
       then begin
              if alert(3,'    re-sizing  brick  boundary  rectangle',
                         'The currently selected background shape is a brick boundary rectangle.'
                        +'||If you re-size it, it may no longer match your 3D printer build plate.',
...
begin
  with bgnd_shapes_listbox do begin
    ...
    if Tbgshape(Items.Objects[n]).bgnd_shape.is_brick_boundary_rect=True
       then begin
              if alert(3,'    changing  brick  boundary  rectangle',
                         'The currently selected background shape is a brick boundary rectangle.'
                        +'||If you change the corner dimensions, it may no longer match your 3D printer build plate.',
                          '','','','','cancel','continue',0)=5 then EXIT;
            end;
  end;//with
...
begin
  ...
  n:=putdim(help_rotshap_str+rot_cen_str,3,'rotate  all  shapes  around  '+cen_str+'  by',0,False,True,False,False);   // negative ok, no preset, zero ok, don't terminate on zero.
...
begin
  ...
  try
    ...
    while k2<k1 do k2:=k2+Pi*2;   // bug-fix 227a  -  ensure aways anticlockwise
    if k2=k1 then EXIT;

    inc_k:=0.004;   // 0.004 rads = 1/4 degree steps (arbitrary).
...
procedure Tbgnd_form.allow_shape_sync_checkboxMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);  // 226b

              // .Click not used because called when ticked programmatically (in current_state).
...
begin
  ...
  with Tbgshape(bgnd_shapes_listbox.Items.Objects[n]).bgnd_shape do begin

    if allow_shape_sync_checkbox.Checked=True
       then option_bits:=(option_bits AND $FE)      // bit 0 = 0  default allow sync
       else option_bits:=(option_bits OR $01);      // bit 0 = 1 don't allow
  end;//with
...
procedure Tbgnd_form.set_as_brick_checkboxMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);    // 234b
...
begin
  ...
  with Tbgshape(bgnd_shapes_listbox.Items.Objects[n]).bgnd_shape do begin
    ...
    case shape_code of
      ...
      0,1,4: begin
               is_brick:=set_as_brick_checkbox.Checked;
               if is_brick=True //and (brick_form.store_using_marker_colour_checkbox.Checked=True)    // 234c
                 then wrap_offset:=brick_form.store_marker_colour_panel.Color
                 else begin    // cancel brick...

                        option_bits:=(option_bits AND $FB);  // 234e  bit 2 = 0  normal line
                        is_brick_wall:=False;                // 234e

                        if shape_code=0
                           then shape_name:='line'
                           else if shape_code=1
                                   then shape_name:='rectangle'
                                   else shape_name:='target mark';


                      end;
             end;

        else EXIT;  // ??? should be disabled if not line, rectangle, target mark
    end;//case
...
procedure Tbgnd_form.brick_label_checkboxMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);   // 234a

    // .Click not used because would be called when ticked programmatically (in current_state).
...
begin
  ...
  with Tbgshape(bgnd_shapes_listbox.Items.Objects[n]).bgnd_shape do begin
    ...
    if shape_code<>0 then EXIT;  // ??? should be disabled if not line

    if brick_label_checkbox.Checked=True
       then begin
              shape_name:=get_rgb_text(wrap_offset); // 234a brick label - default=colour RGB
              option_bits:=(option_bits OR $04);     // bit 2 = 1  line is brick label tab in 3-D exports  (no effect in 2-D)
            end
       else begin
              if POS('#',shape_name)=1 then shape_name:='line';  // was a brick label
              option_bits:=(option_bits AND $FB);                // bit 2 = 0  normal line
            end;
...
procedure Tbgnd_form.brick_wall_checkboxMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
begin
  ...
  with Tbgshape(bgnd_shapes_listbox.Items.Objects[n]).bgnd_shape do begin
    ...
    if brick_wall_checkbox.Checked=True
       then begin
              shape_name:='brick wall';
              is_brick_wall:=True;
            end
       else begin
              if is_brick=True
                 then shape_name:='brick splint'
                 else shape_name:='line';
              is_brick_wall:=False;
            end;
...
const
  ...
  +'||Please bear in mind that Templot is not fully-fledged graphics imaging software - this "bare-bones" function is intended primarily to permit the alignment of scanned track plans.'
...
begin
  ...
  if trans_checkbox.Checked=True
     then begin
            if (Tbgshape(bgnd_shapes_listbox.Items.Objects[n]).bgnd_shape.picture_is_metafile=False) and (trans_msg_pref=False)
               then begin
                      alert_box.preferences_checkbox.Checked:=False;       //%%%%
                      alert_box.preferences_checkbox.Show;
                      ...
                      repeat
                        ...
                        i:=alert(3,'    transparent  picture  shape  image',
                                   'You have selected the transparent option for this picture shape image.'
      ...
      with bgimage.image_shape do begin
        ...
        try
          ...
          with rotated_bitmap.Canvas do begin
            Brush.Color:=twist_corner_colour;   //clWhite;          //  this clears the new bitmap for starters
            Brush.Style:=bsSolid;
            FillRect(Rect(0, 0, rotated_bitmap.Width, rotated_bitmap.Height));
          ...
          for inrow:=0 to do_truncx(inheight-1) do begin
            for incol:=0 to do_truncx(inwidth-1) do begin
              ...
              with rotated_bitmap.Canvas do begin
                ...
                outcol:=Round(pout.x);     // slow method - makes better job of filling all pixels.
                outrow:=Round(pout.y);
...
begin
  ...
  repeat
    i:=alert(4,'    twist  picture  shape',
               ...
               +'||You can choose to enter a twist angle in degrees, or let Templot use the calculated twist angle shown below to align your marker lines.'
               +'||(You may want to make a note of this angle and use it in other graphics software to rotate this or other bitmap images.)'
               +'||Before continuing you may want to save a BGS3 file, so that you can return to the untwisted original if necessary.',
...
begin
  ...
  show_modal_message('The image has been copied to the Windows clipboard.'
              +#13+#13+'It can now be pasted into any graphics program,'
                  ...
                  +#13+'or back into this one if trial changes to it prove unsatisfactory.');
end;
//______________________________________________________________________________
      ...
      with Tbgshape(Objects[n]) do begin
        ...
        with picture_save_dialog do begin  // set up the save dialog.
          ...
          if user_save_img_path=''
             ...
             //PNG image ( .png)|*.png|GIF image ( .gif)|*.gif|JPEG image ( .jpg  .jp  .jpeg)|*.jpg;*.jp;*.jpeg|BMP image (  .bmp)|*.bmp|EMF metafile ( .emf)|*.emf

          if bgnd_shape.picture_is_metafile=True
             then FilterIndex:=5   // EMF
             else FilterIndex:=1;  // PNG
          ...
          if Execute=True         // get the file name.
             then begin
                    ...
                    try
                      if ( (LowerCase(ExtractFileExt(FileName))='.jpg') or (LowerCase(ExtractFileExt(FileName))='.jpeg') ) and (bgnd_shape.picture_is_metafile=False)
                          then begin
...
// OT2024                                   create_jpg.JPEGNeeded;
                      ...
                      if (LowerCase(ExtractFileExt(FileName))='.gif') and (bgnd_shape.picture_is_metafile=False)
                          then begin
                      ...
                      if (LowerCase(ExtractFileExt(FileName))='.png') and (bgnd_shape.picture_is_metafile=False)
                          then begin
// OT2024                                 create_png:=TPNGObject.Create;
                                 try
// OT2024                                   create_png.Assign(bgimage.image_shape.image_bitmap);
// OT2024                                   create_png.SaveToFile(FileName);
                                 finally
// OT2024                                   create_png.Free;
                                 end;//try
                                 img_saved:=True;
...
(* OT2024
                      if (LowerCase(ExtractFileExt(FileName))='.bmp') and (bgnd_shape.picture_is_metafile=False)
                         then begin
                                bgimage.image_shape.image_bitmap.SaveToFile(FileName);
                                img_saved:=True;
                              end;

                      if (LowerCase(ExtractFileExt(FileName))='.emf') and (bgnd_shape.picture_is_metafile=True)
                         then begin
                                bgimage.image_shape.image_metafile.SaveToFile(FileName);
                                img_saved:=True;
                              end;
*)
...
function check_if_tile(n:integer; map_str:string):boolean;     // 227a
  ...
  // is shape a tile of current map?
...
begin
  ...
  with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[n]) do begin
    ...
    and (POS('map::',map_str)=1)                // a map
    and (POS(map_str,bgnd_shape.shape_name)=1)  // this map
       then begin
              if ((bgnd_shape.option_bits AND $80)<>0)                                                 // msb=1  a tile
              or ( ((bgnd_shape.option_bits AND $02)<>0) and (Copy(bgnd_shape.shape_name,22,1)='T') )  // pre_227 file loaded tile
...
//______________________________________________________________________________
              ...
              // save all map tiles in georeferenced format...

procedure Tbgnd_form.save_map_tiles_menu_entryClick(Sender: TObject);

var
  n,tile_count:integer;
  create_png:TPortableNetworkGraphic;   // OT2024

  file_str,geo_str:string;
...
begin
  ...
  with bgnd_shapes_listbox do begin
    ...
    if (Items.Count<1) or (ItemIndex<0)
       then begin
              show_modal_message('error : no background shape selected');
              EXIT;
            end;

    n:=ItemIndex;

    map_str:=Copy(Tbgshape(Items.Objects[n]).bgnd_shape.shape_name,1,22);

    if check_if_tile(n,map_str)=False
       then begin
              show_modal_message('invalid : the selected background shape is not a map tile');
              EXIT;
            end;

    map_name_str:=Trim(Copy(map_str,6,15));

    if alert(7,'   save  map  tile  images',
               'You are about to save all tiles from the map:'
               +'||    '+map_name_str
               +'||as georeferenced image files.'
    ...
    for n:=0 to Items.Count-1 do begin
      ...
      if check_if_tile(n,map_str)=False then CONTINUE;  // not this one

      INC(tile_count);

      with Tbgshape(Items.Objects[n]) do begin

        geo_str:=StringReplace(bgnd_shape.shape_name,map_str,'',[]);
        geo_str:=StringReplace(geo_str,'#','_',[rfReplaceAll]);
        geo_str:=StringReplace(geo_str,' ','',[rfReplaceAll]);

        file_str:=exe_str+'SHAPE-FILES\'+map_name_str+'__'+Trim(geo_str)+'.png';
  ...
  show_modal_message(IntToStr(tile_count)+' tile images have been saved to'
                    +#13+#13+exe_str+'SHAPE-FILES\');
end;
//______________________________________________________________________________
...
procedure Tbgnd_form.lock_grid_radiobuttonClick(Sender: TObject);

begin
  if lock_grid_radiobutton.Checked=True then rotate_centre_code:=3;     // default
end;
//____________________________________________________________________

procedure Tbgnd_form.lock_notch_radiobuttonClick(Sender: TObject);

begin
  if lock_notch_radiobutton.Checked=True then rotate_centre_code:=1;
end;
//_____________________________________________________________________

procedure Tbgnd_form.lock_ring_radiobuttonClick(Sender: TObject);

begin
  if lock_ring_radiobutton.Checked=True then rotate_centre_code:=2;
end;
//_____________________________________________________________________________________
        ...
        try
          ...
          for inrow:=0 to do_truncx(inheight-1) do begin
            for incol:=0 to do_truncx(inwidth-1) do begin
              ...
              with destination_bitmap.Canvas do begin
                outcol:=do_truncx((xc-min_extent_x)*picture_scale_width);     // slow method - makes better job of filling all pixels.
                outrow:=destination_bitmap_height-do_truncx((yc_on_pad-min_extent_y)*picture_scale_height{-0.5});
                Pen.Color:=pixel_colour;
                MoveTo(outcol,outrow);
                LineTo(outcol+1,outrow);        // most of this will get overwritten later.
                LineTo(outcol+1,outrow+1);
...
begin
  ...
  with bgnd_shapes_listbox do begin
    ...
    with Items do begin
      ...
      new_bitmap.PixelFormat:=pf24bit;      // 227a
      ...
      with new_bitmap.Canvas do begin
        Brush.Color:=combined_patch_colour;   //clWhite;          //  this clears the new bitmap for starters
        Brush.Style:=bsSolid;
        FillRect(Rect(0, 0, new_bitmap.Width, new_bitmap.Height));
      end;//with
      ...
      with Tbgshape(Objects[j]).bgimage.image_shape do begin
        ...
        image_bitmap.PixelFormat:=pf24bit;      // 227a
...
begin
  with bgnd_shapes_listbox do begin
    ...
    if (n<0) or (n>(Items.Count-1))
       then begin
              show_modal_message('invalid: No background shape is currently selected in the list.');
              EXIT;
            end;
    ...
    with now_shape do begin
      ...
      if (is_brick=True) and (shape_code=4)
         then begin
                show_modal_message('invalid: These changes cannot be applied to a brick connector clip.');
                EXIT;
              end;
      ...
      case shape_code of     // 234c

           -1: begin                // picture
                 is_brick:=False;
                 wrap_offset:=0;    // for image wrapping
               end;

        0,1,4: begin                                     // line, rectangle, target mark
                 is_brick:=add_brick_checkbox.Checked;
                 if is_brick=True // and (brick_form.store_using_marker_colour_checkbox.Checked=True)    // 234c
                   then wrap_offset:=brick_form.store_marker_colour_panel.Color
                   else wrap_offset:=clSilver;                                 // 229a  unused
               end;

          else begin                  // circle, text label
                 is_brick:=False;
                 wrap_offset:=clSilver;
               end;
     ...
     then begin
            ...
            repeat
              i:=alert(2,'php/401    background  images',
                        ...
                        +'||Or Templot can adjust it automatically for you.'
...
*)
  ...
  i:=alert(4,'php/401    load  image  for  picture  shape',
             ...
             +'||A picture shape is a container for an image, which is stretched to fit within it. Templot now needs the image which is to be contained within this one. You can provide the image in several ways.'
            ...
            +'||After the image has been loaded into this picture shape, Templot can help you adjust the size and position of this picture shape to match your track plan.'
            ...
            case i of
              ...
              2: begin  // scan
                   show_modal_message('If the image is a map or track plan for which you know the scale,'
                                     ...
                                     +#13+#13+'If you wish Templot will then be able to scale the image for you to match your model scale and gauge.');
    ...
    try
      ...
      with Tbgshape((Control as TListBox).Items.Objects[Index]).bgnd_shape do begin
        ...
        TextOut(Rect.Left,Rect.Top,mark_str+(Control as TListBox).Items.Strings[Index]);
                       ...
                       // 234d add the brick-colour marker...

        if is_brick=True
           then begin
                  brick_rect_left:=TextWidth('0000');
                  brick_rect_right:=TextWidth('0');

                  Brush.Color:=wrap_offset;

                  Rectangle(Rect.Right-brick_rect_left,Rect.Top+(bgnd_shapes_listbox.ItemHeight div 6),Rect.Right-brick_rect_right,Rect.Top+(bgnd_shapes_listbox.ItemHeight*5 div 6));

                  if (odSelected in State)=True                      // restore Brush
                     then Brush.Color:=$00FF7700                     // light blue
                     else Brush.Color:=bgnd_shapes_listbox.Color;
                end;

        if is_brick_boundary_rect=True         // and boundary marker...
           then begin
                  brick_rect_left:=TextWidth('000');
                  brick_rect_right:=TextWidth('000000');

                  Brush.Color:=wrap_offset;

                  Rectangle(brick_rect_left,Rect.Top+(bgnd_shapes_listbox.ItemHeight div 6),brick_rect_right,Rect.Top+(bgnd_shapes_listbox.ItemHeight*5 div 6));
      ...
      // do nothing   -- fixes strange bug see OLD Club message ref: 27596    program loops through here several times under condition mentioned there. 223c
    end;//try
  ...
  with bgnd_shapes_listbox do begin
    ...
    with Items do begin
      ...
      with Tbgshape(Objects[n]) do begin
        ...
        or (bgimage=nil)                              // no existing bitmap.
           then begin
                  show_modal_message(bgnd_shape.shape_name+' : no image content to convert.');
                  EXIT;
                end;

        if bgimage.image_shape.image_bitmap.PixelFormat=pf24bit
           then begin
                  show_modal_message(bgnd_shape.shape_name+' : image colour depth is already 24-bit.');
                  EXIT;
                end;
        ...
        try
          ...
          show_modal_message('Sorry, unable to convert image colour depth to 24-bit for '+bgnd_shape.shape_name);
          EXIT;
        end;//try
...
procedure Tbgnd_form.stop_lines_buttonClick(Sender: TObject);
...
procedure Tbgnd_form.improve_map_clarity_menu_entryClick(Sender: TObject);
...
procedure show_bgs3_progress;  // 227a  called from dtpNativeXml at TXmlNode.ReadFromStream(S: TStream);
  ...
  // also called when saving sketchboard

begin
  wait_form.wait_progressbar.StepIt;  // (if showing)

  Sleep(1);  // needed to allow repaint
end;
//______________________________________________________________________________

procedure Tbgnd_form.map_clarity_buttonClick(Sender: TObject);       // 227a
...
procedure Tbgnd_form.modified_3d_checkboxClick(Sender: TObject);
...
procedure Tbgnd_form.slab_adjust_angle_menu_entryClick(Sender: TObject);

begin
  pad_form.adjust_slab_angle_menu_entry.Click;  // 234b  pad_unit mouse action   slab angle
end;
//______________________________________________________________________________

procedure Tbgnd_form.clip_adjust_angle_menu_entryClick(Sender: TObject);

begin
  pad_form.adjust_clip_angle_menu_entry.Click;  // 229a  pad_unit mouse action   brick clip angle
end;
//______________________________________________________________________________

procedure Tbgnd_form.clip_adjust_size_menu_entryClick(Sender: TObject);

begin
  pad_form.adjust_clip_size_menu_entry.Click;   // 229a  pad_unit mouse action   brick clip size
end;
//______________________________________________________________________________

procedure Tbgnd_form.clip_swap_direction_menu_entryClick(Sender: TObject);
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      if (shape_code<>4) or (is_brick<>True)
       then begin
              show_modal_message('invalid: The currently selected background shape'+#13+'is not a brick connector clip.');
              EXIT;
            end;

      p2.x:=0-p2.x;   // sign of clip size is clip direction
  ...
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    for n:=0 to (Count-1) do begin
      ...
      with Tbgshape(Objects[n]).bgnd_shape do begin
        ...
        if (is_brick=True) and (shape_code=4) then RESULT:=RESULT+1;
...
function find_paired_clip(i:integer):integer;       // 234c
  ...
  // return index of paired clip, or -1 if not found

  // i is index of first clip
...
begin
  ...
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    for n:=0 to (Count-1) do begin
      ...
      if n=i then CONTINUE;  // ignore first clip

      with Tbgshape(Objects[n]).bgnd_shape do begin

        if (is_brick=False) or (shape_code<>4) or (clip_pairing_code=0) then CONTINUE;  // not a paired brick clip shape

        if ABS(clip_pairing_code-Tbgshape(Objects[i]).bgnd_shape.clip_pairing_code)<minfp
           then begin
                  RESULT:=n;   // found match
                  EXIT;
                end;
      end;//with
    end;//next
...
procedure Tbgnd_form.remove_pairing_menu_entryClick(Sender: TObject);
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      if clip_pairing_code=0
         then begin
                show_modal_message('invalid: The currently selected brick connector clip is not paired.');
                EXIT;
              end;

      m:=find_paired_clip(n);

      if (m<0) or (m>(Items.Count-1))
         then begin
                show_modal_message('error: Unable to find a paired clip.');
                EXIT;
              end;

      if alert(7,'      remove  clip  pairing ?',
                'You are about to remove the pairing on two brick connector clips.'
                +'||The clips will continue to exist but will no longer be paired.'
                +'||Any changes made to one clip will not be reflected in the other clip.',
                 '','','','','cancel','continue',0)=5 then EXIT;

           // cancel pairing on both...

      clip_pairing_code:=0;

      shape_name:=StringReplace(shape_name,'paired ','',[rfReplaceAll,rfIgnoreCase]);  // remove naming

      Tbgshape(Items.Objects[m]).bgnd_shape.clip_pairing_code:=0;

      Tbgshape(Items.Objects[m]).bgnd_shape.shape_name:=StringReplace(Tbgshape(Items.Objects[m]).bgnd_shape.shape_name,'paired ','',[rfReplaceAll,rfIgnoreCase]);  // remove naming
...
procedure Tbgnd_form.go_to_paired_clip_menu_entryClick(Sender: TObject);         // 234d
...
procedure Tbgnd_form.add_new_clip_menu_entryClick(Sender: TObject);

begin
  target_radio_button.Checked:=True;
  by_drawing_radiobutton.Checked:=True;
  add_brick_checkbox.Checked:=True;       // 234c
  add_kerf_lines_checkbox.Checked:=False;
  add_shape_button.Click;
end;
//______________________________________________________________________________

procedure Tbgnd_form.add_paired_clip_menu_entryClick(Sender: TObject);

       // add paired clip at existing clip location     234c
var
  new_shape:Tbgnd_shape;
  n,m:integer;
  clip_code:TDateTime;
...
begin
  if any_clips<1
     then begin
            show_modal_message('invalid: There are no brick clips in the background shapes.'
            +#13+#13+'To add a paired clip you must first select an existing clip with which it is to be paired.');
            EXIT;
          end;
  ...
  with bgnd_shapes_listbox do begin
    ...
    if n<0
       then begin
              show_modal_message('invalid: To add a paired clip you must first select an existing clip with which it is to be paired.');
              EXIT;
            end;

    with Items do begin

      if (Count<1) or (n>(Count-1)) then EXIT;  // ???
      ...
      with Tbgshape(Objects[n]).bgnd_shape do begin
        ...
        if clip_pairing_code<>0
           then begin
                  show_modal_message('invalid: The currently selected brick connector clip is already paired.'
                  +#13+#13+'To pair with it you must first remove the existing pairing.');
                  EXIT;
                end;

        clip_code:=Now;  // use current time as unique code

        clip_pairing_code:=clip_code;    // set code on first clip

        shape_name:=StringReplace(shape_name,'paired ','',[rfReplaceAll,rfIgnoreCase]);  // remove any previous

        shape_name:='paired '+shape_name;    // add again
      ...
      with new_shape do begin
        ...
        shape_name:='paired brick clip '+DateTimeToStr(Date+Time); // identify this shape with the time of creation.

        hide_bits:=0;       // normal visibility
        option_bits:=0;     // byte;

        shape_code:=4;      // 4=target mark/clip

        is_brick:=True;     // 234c

        rect_rot:=0;        // 234b   (used only for rectangles/slabs)

        clip_pairing_code:=clip_code;

        shape_style:=0;     // 0=transparent

        picture_is_metafile:=False;    // default (used only for images)

        {if brick_form.store_using_marker_colour_checkbox.Checked=True
           then}

           wrap_offset:=brick_form.store_marker_colour_panel.Color;   // (integer used for image wrapping, and for clip/brick splint colours)

           // else wrap_offset:=clSilver;                                // default

        show_transparent:= NOT Tbgshape(Objects[n]).bgnd_shape.show_transparent;      // swap tommy bar / claws

        p1.x:=Tbgshape(Objects[n]).bgnd_shape.p1.x;
        p1.y:=Tbgshape(Objects[n]).bgnd_shape.p1.y;

        p2.x:=Tbgshape(Objects[n]).bgnd_shape.p2.x;   // clip size/direction
        p2.y:=Tbgshape(Objects[n]).bgnd_shape.p2.y;   // clip rotation angle

      end;//with

      if n=(Count-1)                                              // bottom of list ?
         then m:=AddObject(new_shape.shape_name,Tbgshape.Create)  // create and add a new line in the shapes list.
         else begin
                m:=n+1;
                InsertObject(m,new_shape.shape_name,Tbgshape.Create);  // create and insert a new line in the shapes list.   234d
              end;

      Tbgshape(Objects[m]).bgnd_shape:=new_shape;          // put data in list.
      Tbgshape(Objects[m]).bgimage:=nil;                   // not bitmap image 3-2-01.
...
procedure Tbgnd_form.add_new_clip_at_notch_menu_entryClick(Sender: TObject);   // 234a

var                               // add clip at notch
  new_shape:Tbgnd_shape;
  n:integer;
...
begin
  ...
  with new_shape do begin
    ...
    shape_name:='ex-notch brick clip '+DateTimeToStr(Date+Time); // identify this shape with the time of creation.

    hide_bits:=0;       // normal visibility
    option_bits:=0;     // byte;

    shape_code:=4;      // 4=target mark

    is_brick:=True;     // 234c

    clip_pairing_code:=0;  // 234c not yet paired

    shape_style:=0;     // 0=transparent

    wrap_offset:=brick_form.store_marker_colour_panel.Color;
    ...
    p2.x:=8*inscale;             // target size mm   8" default
    p2.y:=notch_angle;           // clip rotation angle
    normalize_angle(p2.y);
...
procedure Tbgnd_form.mouse_move_ring_menu_entryClick(Sender: TObject);   // 233c
...
procedure Tbgnd_form.add_clip_at_ring_menu_entryClick(Sender: TObject);  // 233c

begin
  grid_form.make_clip_button.Click;      // 234c
end;
//______________________________________________________________________________

procedure Tbgnd_form.shift_clip_menu_entryClick(Sender: TObject);
...
procedure Tbgnd_form.clip_colour_menu_entryClick(Sender: TObject);
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      wrap_offset:=get_colour('set  this  clip  colour',wrap_offset);
...
procedure Tbgnd_form.clip_colour_as_marker_menu_entryClick(Sender: TObject);
...
procedure Tbgnd_form.target_clip_menuClick(Sender: TObject);     // 229a
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      if shape_code<>4 then EXIT;

      tommy_bar_menu_entry.Checked:= NOT show_transparent;      // radio items
      tommy_claws_menu_entry.Checked:=show_transparent;
...
procedure Tbgnd_form.tommy_bar_menu_entryClick(Sender: TObject);
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      if show_transparent=True then p2.x:=0-p2.x;   // 234c changing gender so also swap direction (sign of clip size is clip direction)
...
procedure Tbgnd_form.tommy_claws_menu_entryClick(Sender: TObject);
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      if show_transparent=False then p2.x:=0-p2.x;   // 234c changing gender so also swap direction (sign of clip size is clip direction)
...
procedure Tbgnd_form.boundary_colour_as_marker_menu_entryClick(Sender: TObject);
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      if (shape_code<>1) or (is_brick_boundary_rect=False)  // not boundary rectangle
         then begin
                show_modal_message('invalid: The currently selected background shape'+#13+'is not a brick boundary rectangle.');
                EXIT;
              end;
...
procedure Tbgnd_form.boundary_colour_menu_entryClick(Sender: TObject);
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      wrap_offset:=get_colour('set  this  brick  boundary  rectangle  colour',wrap_offset);
...
procedure Tbgnd_form.splint_colour_as_marker_menu_entryClick(Sender: TObject);
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      if shape_code>1  // not line or rectangle
         then begin
                show_modal_message('invalid: The currently selected background shape'+#13+'is not a line/splint or rectangle/slab.');
                EXIT;
              end;

      wrap_offset:=brick_form.store_marker_colour_panel.Color;

      if (label_text_as_RGB_menu_entry.Checked=True) and ((option_bits AND $04)<>0)  // 234a
         then shape_name:=get_rgb_text(wrap_offset);                                 // 234a brick label - default=colour RGB
...
procedure Tbgnd_form.splint_colour_menu_entryClick(Sender: TObject);
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      wrap_offset:=get_colour('set  this  splint / brick  label  / slab  colour',wrap_offset);

      if (shape_code=0) and (label_text_as_RGB_menu_entry.Checked=True) and ((option_bits AND $04)<>0)   // 234a
         then shape_name:=get_rgb_text(wrap_offset);                                                     // 234a brick label - change colour RGB
...
procedure Tbgnd_form.brick_wall_dims_menu_entryClick(Sender: TObject);   // 234e

const
  w_str:string='     `0brick  wall  width`9'
                  +'||Enter a dimension in millimetres for the width of this brick wall shape.'
                  +'||Enter zero 0 to set the same width as is currently set for brick splints.';

  d_str:string='    `0brick  wall  depth`9'
                  +'||Enter a dimension in millimetres for the depth of this brick wall shape.'
                  +'||Enter zero 0 to set the same depth as is currently set for the template timbers.';
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      if shape_code<>0  // not line
         then begin
                show_modal_message('invalid: The currently selected background shape'+#13+'is not a line/splint.');
                EXIT;
              end;

         putdim(w_str,1,'brick  wall  width',wall_width,True,True,False,False);   // no neg, no preset, zero ok, don't terminate on zero.
      i:=putdim(d_str,1,'brick  wall  depth',wall_depth,True,True,False,False);   // no neg, no preset, zero ok, don't terminate on zero.

      if i<>1 then EXIT;

      if getdims('brick  wall  dimemsions ...','',bgnd_form,i,od)=False then EXIT;
...
procedure set_all_sync(allow,all_shapes:boolean; code:integer);     // 229d ...

  // all_shapes = ignore shape code and set all shapes
  ...
  with bgnd_form.bgnd_shapes_listbox.Items do begin
    ...
    for n:=0 to Count-1 do begin
      ...
      with Tbgshape(Objects[n]).bgnd_shape do begin
        ...
        if (all_shapes=True) or (shape_code=code)
           then begin
                  if allow=True
                     then option_bits:=(option_bits AND $FE)      // bit 0 = 0  default allow sync
                     else option_bits:=(option_bits OR $01);      // bit 0 = 1 don't allow
...
procedure Tbgnd_form.all_sync_menu_entryClick(Sender: TObject);

begin
  set_all_sync(True,True,0);
end;
//____________________________________

procedure Tbgnd_form.none_sync_menu_entryClick(Sender: TObject);

begin
  set_all_sync(False,True,0);
end;
//______________________________________________________________________________

procedure Tbgnd_form.all_lines_sync_menu_entryClick(Sender: TObject);

begin
  set_all_sync(True,False,0);
end;
//____________________________________

procedure Tbgnd_form.none_lines_sync_menu_entryClick(Sender: TObject);

begin
  set_all_sync(False,False,0);
end;
//______________________________________________________________________________

procedure Tbgnd_form.all_rectangles_sync_menu_entryClick(Sender: TObject);

begin
  set_all_sync(True,False,1);
end;
//____________________________________

procedure Tbgnd_form.none_rectangles_sync_menu_entryClick(Sender: TObject);

begin
  set_all_sync(False,False,1);
end;
//______________________________________________________________________________

procedure Tbgnd_form.all_circles_sync_menu_entryClick(Sender: TObject);

begin
  set_all_sync(True,False,2);
end;
//____________________________________

procedure Tbgnd_form.none_circles_sync_menu_entryClick(Sender: TObject);

begin
  set_all_sync(False,False,2);
end;
//______________________________________________________________________________

procedure Tbgnd_form.all_labels_sync_menu_entryClick(Sender: TObject);

begin
  set_all_sync(True,False,3);
end;
//____________________________________

procedure Tbgnd_form.none_labels_sync_menu_entryClick(Sender: TObject);

begin
  set_all_sync(False,False,3);
end;
//______________________________________________________________________________

procedure Tbgnd_form.all_targets_sync_menu_entryClick(Sender: TObject);

begin
  set_all_sync(True,False,4);
end;
//____________________________________

procedure Tbgnd_form.none_targets_sync_menu_entryClick(Sender: TObject);

begin
  set_all_sync(False,False,4);
end;
//______________________________________________________________________________

procedure Tbgnd_form.all_pictures_sync_menu_entryClick(Sender: TObject);
...
procedure Tbgnd_form.none_pictures_sync_menu_entryClick(Sender: TObject);
...
procedure Tbgnd_form.twist_corner_colour_menu_entryClick(Sender: TObject);        // 232a

begin
  twist_corner_colour:=get_colour('choose  a  colour  for  corner  triangles  on  twisted  picture  shapes',twist_corner_colour);
end;
//______________________________________________________________________________

procedure Tbgnd_form.combined_patch_colour_menu_entryClick(Sender: TObject);          // 232a

begin
  combined_patch_colour:=get_colour('choose  a  colour  for  empty  patches  on  combined  picture  shapes',combined_patch_colour);
end;
//______________________________________________________________________________

procedure Tbgnd_form.label_text_as_RGB_menu_entryClick(Sender: TObject);     // 234a

begin
  label_text_as_RGB_menu_entry.Checked:= NOT label_text_as_RGB_menu_entry.Checked;
end;
//______________________________________________________________________________

procedure Tbgnd_form.twist_rectangle_buttonClick(Sender: TObject);
...
procedure Tbgnd_form.twist_one_by_buttonClick(Sender: TObject);     // 234d

const
  twist_str:string='     `0twist  background  shape`9'
    +'||Enter an angle in degrees for the amount by which to twist this background shape.'
    +'||Angles are measured positive in the anti-clockwise direction, and negative in the clockwise direction.'
    +'||This operation can be undone by twisting in the opposite direction by the same amount.';
...
begin
  ...
  with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[j]).bgnd_shape do begin

    if shape_code=-1      // picture shape not done here
       then begin
              twist_picture_button.Click;
              EXIT;
            end;

    if (shape_code=1)                            // rectangle
    or ( (shape_code=4) and (is_brick=True) )    // brick clip
       then begin
              if shape_code=1
                 then str:='rectangle'
                 else str:='brick  clip';

              n:=putdim(twist_str,3,'twist  background  '+str+'  by',0,False,True,False,False);   // negative ok, no preset, zero ok, don't terminate on zero.

              if n<>0 then EXIT;    // ???

              if getdims('twist  background  '+str,'',bgnd_form,n,od)=True
                 then begin
                        twist_k:=od[0]*Pi/180;

                        if shape_code=1
                           then rect_rot:=rect_rot+twist_k    // rectangle
                           else p2.y:=p2.y+twist_k;           // brick clip
                      end
                 else EXIT;

            end
       else begin
              show_modal_message('invalid: the selected background shape is not a twistable shape'
                                 +#13+#13+'twistable shapes are: picture shapes, rectangles/slabs, brick clips');
              EXIT;
            end;
  end;//with
...
procedure Tbgnd_form.twist_one_to_buttonClick(Sender: TObject);

const
  twist_str:string='     `0twist  background  shape`9'
    +'||Enter the required twisted angle in degrees for this background shape.'
    +'||Angles are measured positive in the anti-clockwise direction, and negative in the clockwise direction.';
  ...
  with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[j]).bgnd_shape do begin
    ...
    or ( (shape_code=4) and (is_brick=True) )    // brick clip
       then begin
              ...
              n:=putdim(twist_str,3,'twist  background  '+str+'  to',twist_k*180/Pi,False,True,False,False);   // negative ok, no preset, zero ok, don't terminate on zero.
              ...
              if getdims('twist  background  '+str,'',bgnd_form,n,od)=True
                 then begin
                        ...
                        if shape_code=1
                           then rect_rot:=twist_k    // rectangle
                           else p2.y:=twist_k;       // brick clip
                      end
                 else EXIT;
...
procedure Tbgnd_form.boundary_dimensions_menu_entryClick(Sender: TObject);
...
begin
     putdim('please refer to Templot Club forum',1,'brick  boundary  rectangle  corner  dimension  X',brick_boundary_rectangle_x,False,True,False,False);   // neg ok, no preset, allow zero, don't terminate on zero.
     putdim('please refer to Templot Club forum',1,'brick  boundary  rectangle  corner  dimension  Y',brick_boundary_rectangle_y,False,True,False,False);   // ditto.
     putdim('please refer to Templot Club forum',1,               'brick  boundary  rectangle  width',brick_boundary_rectangle_w,True,True,True,False);     // no neg, no preset, no zero, don't terminate on zero.
     putdim('please refer to Templot Club forum',1,               'brick  boundary  rectangle  depth',brick_boundary_rectangle_h,True,True,True,False);     // ditto.
  i:=putdim('please refer to Templot Club forum',3,        'brick  boundary  rectangle  twist  angle',brick_boundary_rectangle_k*180/Pi,False,True,False,False);   // neg ok, no preset, allow zero, don't terminate on zero.
  if i<>4 then EXIT;

  if getdims('brick  boundary  rectangle  pre-sets','',bgnd_form,i,od)=True
     then begin
            brick_boundary_rectangle_x:=od[0];
            brick_boundary_rectangle_y:=od[1];
            brick_boundary_rectangle_w:=od[2];
            brick_boundary_rectangle_h:=od[3];
            brick_boundary_rectangle_k:=od[4]*Pi/180;
          end;
end;
//______________________________________________________________________________

procedure Tbgnd_form.bgnd_boundary_menuClick(Sender: TObject);

begin
  show_pre_sets_menu_entry.Caption:=FormatFloat('0.##',brick_boundary_rectangle_w)+'  x  '+FormatFloat('0.##',brick_boundary_rectangle_h)+'  mm    '+FormatFloat('0.##',brick_boundary_rectangle_k*180/Pi)+'  degrees';
end;
//______________________________________________________________________________

procedure Tbgnd_form.add_new_boundary_menu_entryClick(Sender: TObject);     // 234d
...
begin
  ...
  with new_shape do begin
    ...
    shape_name:='           brick boundary rectangle';     // leave space for colour marker

    hide_bits:=0;       // normal visibility
    option_bits:=0;     // byte;

    shape_code:=1;      // rectangle
    ...
    is_brick_boundary_rect:=True;    // 234d
    ...
    p1.x:=brick_boundary_rectangle_x;
    p1.y:=brick_boundary_rectangle_y;

    p2.x:=brick_boundary_rectangle_x+ABS(brick_boundary_rectangle_w);
    p2.y:=brick_boundary_rectangle_y+ABS(brick_boundary_rectangle_h);

    rect_rot:=brick_boundary_rectangle_k;
...
procedure Tbgnd_form.show_hide_brick_shapes_colour_menu_entryClick(Sender: TObject);       // 235a
...
begin
  col:=brick_form.store_marker_colour_panel.Color;  // init

  col:=get_colour('show  brick  shapes  of  only  this  colour  on  trackpad',col);
    ...
    for n:=0 to (Count-1) do begin
      ...
      with Tbgshape(Objects[n]).bgnd_shape do begin
        ...
        case shape_code of
          ...
          0,1,4: begin
                   if (is_brick=True) and (wrap_offset=col)
                      then hide_bits:=hide_bits AND $FE  // toggle bit off, show this shape
                      else hide_bits:=hide_bits OR $01;  // toggle bit on, hide this shape
                 end;
          else CONTINUE;  // not brick shape
...
procedure Tbgnd_form.show_all_brick_shapes_menu_entryClick(Sender: TObject);      // 235a
...
procedure Tbgnd_form.brick_label_scaling_menu_entryClick(Sender: TObject);  // 236c

const
  str:string='     `0brick  label  text  size`9'
                  +'||Enter a % setting for the text size on the brick label, relative to the size of the label tab.';
...
begin
  with bgnd_shapes_listbox do begin
    ...
    with Tbgshape(Items.Objects[n]).bgnd_shape do begin
      ...
      if (shape_code<>0) or ((option_bits AND $04)=0)  // not label
         then begin
                show_modal_message('invalid: The currently selected background shape is not a brick label.');
                EXIT;
              end;

      i:=putdim(str,4,'text  size  on  brick  label',brick_label_scaling*100,True,True,True,False);  // no neg, no preset, no zero, don't etc.

      if i<>0 then EXIT;

      if getdims('brick  label  text','',bgnd_form,i,od)=False then EXIT;

      brick_label_scaling:=od[0]/100;
...
procedure Tbgnd_form.create_array_menu_entryClick(Sender: TObject);     // 236d
...
begin
  with bgnd_shapes_listbox do begin
    ...
    if Tbgshape(Items.Objects[n]).bgnd_shape.shape_code=-1
       then begin
              show_modal_message('invalid: The selected background shape is a picture shape.'
                        +#13+#13+'Picture shapes cannot be duplicated and must be added individually.');
              EXIT;
            end;
    ...
    putdim(str,1,'offset  to  next  new  shape  on  X',offx,False,True,False,False);   // neg ok, no preset, allow zero, don't terminate on zero.
    putdim(str,1,'offset  to  next  new  shape  on  Y',offy,False,True,False,False);   // ditto.
 i:=putdim(str,0,'number  of  new  shapes  to  create',numco,True,True,True,False);    // no neg, no preset, no zero, don't terminate on zero.

    if i<>2 then EXIT;

    if getdims('create  array  of  duplicate  shapes','',bgnd_form,i,od)=False then EXIT;

    offx:=od[0];
    offy:=od[1];
    numco:=Round(od[2]);

    for m:=1 to numco do begin

      new_shape:=Tbgshape(Items.Objects[n]).bgnd_shape;   // get it again as a copy

      with new_shape do begin

        shape_name:=shape_name+IntToStr(m);

        p1.x:=p1.x+m*offx;
        p1.y:=p1.y+m*offy;

        if shape_code<>4  // not for a target or clip
           then begin
                  p2.x:=p2.x+m*offx;
                  p2.y:=p2.y+m*offy;
                end;
      ...
      with Items do begin
        ...
        u:=AddObject(new_shape.shape_name,Tbgshape.Create);     // create and insert a new entry in the shapes list.

        Tbgshape(Objects[u]).bgimage:=nil;            // not for picture shapes
        Tbgshape(Objects[u]).bgnd_shape:=new_shape;   // put data in list.

      end;//with

      ItemIndex:=u;     // make it current.
...
begin
  if line_ends.valid=False then EXIT;
  ...
  with new_shape do begin
    ...
    shape_name:='added line '+time_str;
...
procedure Tbgnd_form.doclips1Click(Sender: TObject);

begin
  //adjusting_bgshapes:=True;

// OT2024  clips_form.Show;

  redraw(True);
end;
//______________________________________________________________________________

procedure Tbgnd_form.add_brick_checkboxMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);

begin
  if add_brick_checkbox.Checked=True then add_kerf_lines_checkbox.Checked:=False;
end;
//______________________________________________________________________________

procedure Tbgnd_form.add_kerf_lines_checkboxMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
