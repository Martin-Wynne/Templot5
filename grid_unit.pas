     ...
     then begin
            with canv do begin
              ...
              for i:=first_index to max_index do begin
                ...
                if (bgnd_form.Showing=True) and (i=bgnd_form.bgnd_shapes_listbox.ItemIndex)
                       then Pen.Color:=clRed                                                 // show current shape red
                ...
                try
                  ...
                  if bgnd_form.pad_shapes_linewidth_4_radiobutton.Checked=True then Pen.Width:=4;

                  brick_boundary_pen_width:=1;                                                              // 234d ...
                  if bgnd_form.pad_brick_boundary_2_radio.Checked=True then brick_boundary_pen_width:=2;
                  if bgnd_form.pad_brick_boundary_3_radio.Checked=True then brick_boundary_pen_width:=3;
                  if bgnd_form.pad_brick_boundary_4_radio.Checked=True then brick_boundary_pen_width:=4;
                  if bgnd_form.pad_brick_boundary_5_radio.Checked=True then brick_boundary_pen_width:=5;
                  if bgnd_form.pad_brick_boundary_6_radio.Checked=True then brick_boundary_pen_width:=6;
                  ...
                  with Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]).bgnd_shape do begin     // next shape.
                    ...
                    if (shape_code=0) and (is_kerf=True) then Pen.Color:=$00FF0080;  // 244a   violet
                    ...
                    if shape_code<>4    // not a target mark
                       then begin
                              ...
                              case shape_style of
                                          ...
                                          1: begin
                                               Brush.Color:=shapes_infill_colour;   // 239a  was timber_infill_colour;   // 0.93.a was paper_colour
                              ...
                              if shape_code=3      // label rectangle..
                                 then begin
                                        ...
                                        if ( NOT ( ( (corner1_mod=1) or (corner2_mod=1) or (oneshape_shift_mod=1) or (oneshape_scale_mod=1) or (clip_size_mod=1) or (clip_angle_mod=1) )
                              ...
                              if (paper_bunching=True) and (shape_code=-1)   // 237a  for bunched picture shape  -- bug-fix
                                 then begin
                                        unbunched_move_to:=move_to;
                                        unbunched_line_to:=line_to;

                                        unbunched_width:=line_to.X-move_to.X;
                                        unbunched_height:=move_to.Y-line_to.Y;        // Y from the top
                                      end;
                                 ...
                                 then begin
                                        case shape_code of
                                              ...
                                              -1: begin     // picture = bitmap image.
                                                    ...
                                                    omit_shape:=False;   // init

                                                    if (paper_bunching=True)           // 237a
                                                       then begin
                                                              bunch_end:=bunch_start+bunch_gap; //+bunch_jump_i;   //bunch_shear_i

                                                              if (line_to.X>=bunch_start) and (move_to.X<=bunch_end)  // picture crosses or touches bunch zone      // 237a
                                                                 then begin
                                                                        omit_shape:=True;  // don't draw it normally

                                                                        bunched_bitmap:=TBitmap.Create;     // draw to temporary bitmap

                                                                        bunched_bitmap.Width:=unbunched_width+1;
                                                                        bunched_bitmap.Height:=unbunched_height+1;

                                                                        bunched_raster_rect.Left:=0;
                                                                        bunched_raster_rect.Bottom:=unbunched_height;
                                                                        bunched_raster_rect.Right:=unbunched_width;
                                                                        bunched_raster_rect.Top:=0;

                                                                        with bunched_bitmap.Canvas do begin
...
*)
                                                                          StretchDraw(bunched_raster_rect,pad_form.bgnd_shape_image.Picture.Graphic);   // draw to temporary bitmap

                                                                        end;//with temporary canvas

                                                                             // copy to screen ...

                                                                        if move_to.X<bunch_start    // need left part of image
                                                                           then begin
                                                                                  bunched_raster_rect.Left:=0;
                                                                                  bunched_raster_rect.Bottom:=unbunched_height;
                                                                                  bunched_raster_rect.Right:=bunch_start-move_to.X;
                                                                                  bunched_raster_rect.Top:=0;

                                                                                  raster_rect.Left:=move_to.X;
                                                                                  raster_rect.Bottom:=move_to.Y;
                                                                                  raster_rect.Right:=bunch_start;
                                                                                  raster_rect.Top:=unbunched_line_to.Y;
(* OT2024
                                                                                  if Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]).bgnd_shape.picture_is_metafile=True
                                                                                     then begin
                                                                                            pad_form.bgnd_shape_image.Picture.Graphic:=Tbgshape(bgnd_form.bgnd_shapes_listbox.Items.Objects[i]).bgimage.image_shape.image_metafile;
...
*)
                                                                                  ...
                                                                                  if bgnd_form.picture_borders_checkbox.Checked=True
                                                                                     then begin
                                                                                            Brush.Color:=paper_colour;
                                                                                            Brush.Style:=bsClear;
                                                                                            Rectangle(raster_rect); //move_to.X, move_to.Y, line_to.X, line_to.Y);
                                                                                          end;


                                                                                end;// left part

                                                                        if line_to.X>bunch_end    // need right part of image
                                                                           then begin
                                                                                  bunched_raster_rect.Left:=unbunched_width-(line_to.x-bunch_end);
                                                                                  bunched_raster_rect.Bottom:=unbunched_height;
                                                                                  bunched_raster_rect.Right:=unbunched_width;
                                                                                  bunched_raster_rect.Top:=0;

                                                                                  raster_rect.Left:=bunch_end;
                                                                                  raster_rect.Bottom:=line_to.Y+unbunched_height;
...
*)
                                               ...
                                               0: begin                // line
                                                    if is_brick=True   //  229a   first add outline of 3-D DXF brick splint
                                                       then begin
                                                              if p1.x>p2.x
                                                                 then begin
                                                                        xlo:=p2.x;
                                                              ...
                                                              if (option_bits AND $04)<>0     // this is a brick label tab
                                                                 then spw:=label_tab_width*inscale/2
                                                                 else begin
                                                                        if (is_brick_wall=True) and (wall_width<>0)   // 234e
                                                                           then spw:=wall_width/2
                                                                           else spw:=splint_width*inscale/2;      // splint or default wall
                                                                      end;

                                                              splint_points_pex[0].x:=xlo-spw*SIN(spk);
                                                              splint_points_pex[0].y:=ylo+spw*COS(spk);

                                                              splint_points_pex[1].x:=xhi-spw*SIN(spk);
                                                              splint_points_pex[1].y:=yhi+spw*COS(spk);

                                                              splint_points_pex[2].x:=xhi+spw*SIN(spk);
                                                              splint_points_pex[2].y:=yhi-spw*COS(spk);

                                                              splint_points_pex[3].x:=xlo+spw*SIN(spk);
                                                              splint_points_pex[3].y:=ylo-spw*COS(spk);

                                                              for n:=0 to 3 do begin
                                                                splint_points[n].X:=Round(ex-gx+(splint_points_pex[n].x*100+re_org_x)*sx);   // to screen dims ..
                                                                splint_points[n].Y:=Round(by-gy+(splint_points_pex[n].y*100+re_org_y)*sy);
                                                              end;//next

                                                              Brush.Color:=wrap_offset;

                                                              if (is_brick_wall=True) or ((option_bits AND $04)<>0)     // wall or brick label
                                                                 then begin
                                                                        if Pen.Color<>clRed then Pen.Color:=wrap_offset;
                                                                        Brush.Style:=bsDiagCross;
                                                                      end
                                                                 else Brush.Style:=bsSolid;

                                                              if (check_limits(splint_points[0],splint_points[1])=True) and (check_limits(splint_points[2],splint_points[3])=True)
                                                                 then Polygon(splint_points);

                                                                 // add brick label characters ...

                                                              if ((option_bits AND $04)<>0) and (POS('#',shape_name)=1)  // this is a label tab, write 7-seg characters on it
                                                                 then begin                                              // # prefix required for brick label tab
                                                                        Pen.Width:=2;
                                                                        seven_seg(brick_label_scaling,xlo,ylo,shape_name);
                                                                      end;

                                                            end
                                                       else begin
                                                              MoveTo(move_to.X, move_to.Y);
                                                              LineTo(line_to.X, line_to.Y);     // normal line shape
                                                            end;
                                                  end;

                                               1: begin

                                                     //  twist rectangle  234b
                                                    ...
                                                    for n:=0 to 3 do begin
                                                      infill_points[n].X:=Round(ex-gx+(infill_points_pex[n].x*100+re_org_x)*sx);   // to screen dims ..
                                                      infill_points[n].Y:=Round(by-gy+(infill_points_pex[n].y*100+re_org_y)*sy);
                                                    end;//next


                                                    if (i=bgnd_form.bgnd_shapes_listbox.Itemindex) and ( (clip_size_mod=1) or (clip_angle_mod=1) or (oneshape_shift_mod=1) )  // adjusting this rectangle / slab
                                                       then begin
                                                              Brush.Style:=bsSolid;
                                                              Brush.Color:=clRed;
                                                            end  
                                                              ...
                                                              if is_brick=True
                                                                 then begin
                                                                        Brush.Style:=bsSolid;                    // otherwise as set from style options
                                                                        Brush.Color:=wrap_offset;
                                                                      end;

                                                              if is_brick_boundary_rect=True      // 234d rectangle fill as style options
                                                                 then begin
                                                                        Pen.Color:=wrap_offset;
                                                                        Pen.Style:=psSolid;
                                                                        Pen.Width:=brick_boundary_pen_width;

                                                                        if (bgnd_form.Showing=True) and (i=bgnd_form.bgnd_shapes_listbox.ItemIndex)    // selected
                                                                           then begin
                                                                                  Pen.Color:=clRed;
                                                                                  //Pen.Width:=1;
                                                                                  Brush.Style:=bsDiagCross;
                                                    ...
                                                    if (check_limits(infill_points[0],infill_points[1])=True) and (check_limits(infill_points[2],infill_points[3])=True)
                                                       then Polygon(Slice(infill_points,4));   // number of points, not index
                                               ...
                                               3: begin                                                     // label shape
                                                    ...
                                                    TextOut(move_to.X+2,move_to.Y+1,' '+shape_name);   // 0.93.a was X+1  // insert label text in rectangle box.
                                                  end;
                              ...
                              if is_brick=True
                                 then begin
                                           ...
                                           //  229a   first add outline of 3-D DXF brick connector clip

                                        clip_length:=ABS(p2.x);  // clip size mm
                                        clip_rot:=p2.y;          // clip rotation angle

                                        while (clip_rot>(Pi*2)) do clip_rot:=clip_rot-Pi*2;   // normalize
                                        while (clip_rot<(Pi*2)) do clip_rot:=clip_rot+Pi*2;

                                        if p2.x<0      // swap direction if size negative, rotate 180 degrees
                                           then begin
                                                  if clip_rot>Pi then clip_rot:=clip_rot-Pi
                                                                 else clip_rot:=clip_rot+Pi;
                                                end;
                                        ...
                                        with infill_points_pex[5] do begin x:=infill_points_pex[0].x;   y:=p1.y-clip_shaft_width*inscale/2; end;
                                        ...
                                        with infill_points_pex[11] do begin x:=p1.x;                    y:=infill_points_pex[0].y; end;


                                           // clip guide marker lines 241c ...

                                        claw_guide1.x:=infill_points_pex[0].x;    claw_guide1.y:=p1.y+30*inscale;    // 30" each side arbitrary
                                        claw_guide2.x:=infill_points_pex[0].x;    claw_guide2.y:=p1.y-30*inscale;

                                        tommy_guide1.x:=infill_points_pex[8].x;    tommy_guide1.y:=p1.y+30*inscale;
                                        tommy_guide2.x:=infill_points_pex[8].x;    tommy_guide2.y:=p1.y-30*inscale;

                                        dotransform(clip_rot,p1.x,p1.y,claw_guide1,claw_guide1);   // rotate to clip angle
                                        dotransform(clip_rot,p1.x,p1.y,claw_guide2,claw_guide2);

                                        dotransform(clip_rot,p1.x,p1.y,tommy_guide1,tommy_guide1);
                                        dotransform(clip_rot,p1.x,p1.y,tommy_guide2,tommy_guide2);

                                        was_colour:=Pen.Color; // draw guide lines ..

                                        if was_colour=clRed then Pen.Color:=clWhite;  // $00FF0080;   // violet
                                        
                                        MoveTo( Round(ex-gx+(claw_guide1.x*100+re_org_x)*sx), Round(by-gy+(claw_guide1.y*100+re_org_y)*sy) );    // draw guide line at claw ..
                                        lineTo( Round(ex-gx+(claw_guide2.x*100+re_org_x)*sx), Round(by-gy+(claw_guide2.y*100+re_org_y)*sy) );

                                        MoveTo( Round(ex-gx+(tommy_guide1.x*100+re_org_x)*sx), Round(by-gy+(tommy_guide1.y*100+re_org_y)*sy) );  // draw guide line at tommy bar ..
                                        lineTo( Round(ex-gx+(tommy_guide2.x*100+re_org_x)*sx), Round(by-gy+(tommy_guide2.y*100+re_org_y)*sy) );

                                        Pen.Color:=was_colour;   // restore


                                        for n:=0 to 11 do begin

                                          dotransform(clip_rot,p1.x,p1.y,infill_points_pex[n],infill_points_pex[n]);   // rotate to clip angle
                                        ...
                                        if (i=bgnd_form.bgnd_shapes_listbox.Itemindex) and ( (clip_size_mod=1) or (clip_angle_mod=1) or (oneshape_shift_mod=1) )  // adjusting this target / clip
                                           then Brush.Color:=clRed
                                           else Brush.Color:=wrap_offset;

                                        if show_transparent=True  //  True = clip claws
                                           then Brush.Style:=bsSolid
                                           else Brush.Style:=bsClear;

                                        if  (check_limits(infill_points[0],infill_points[1])=True) and (check_limits(infill_points[2],infill_points[3])=True)
                                        and (check_limits(infill_points[4],infill_points[5])=True) and (check_limits(infill_points[6],infill_points[7])=True)
                                        and (check_limits(infill_points[8],infill_points[9])=True) and (check_limits(infill_points[10],infill_points[11])=True)
                                            then Polygon(infill_points);

                                            // tommy bar part ...

                                        with infill_points_pex[0] do begin x:=p1.x-clip_length; y:=p1.y+clip_shaft_width*inscale/2; end;

                                        dotransform(clip_rot,p1.x,p1.y,infill_points_pex[0],infill_points_pex[0]);   // rotate to clip angle

                                        infill_points_pex[1]:=infill_points_pex[11];   // already rotated
                                        infill_points_pex[2]:=infill_points_pex[10];   // already rotated
                                        infill_points_pex[3]:=infill_points_pex[9];   // already rotated
                                        infill_points_pex[4]:=infill_points_pex[8];   // already rotated
                                        infill_points_pex[5]:=infill_points_pex[7];   // already rotated
                                      //infill_points_pex[6]  no change

                                        with infill_points_pex[7] do begin x:=p1.x-clip_length;   y:=p1.y-clip_shaft_width*inscale/2; end;

                                        dotransform(clip_rot,p1.x,p1.y,infill_points_pex[7],infill_points_pex[7]);   // rotate to clip angle
                                        ...
                                        if show_transparent=False  //  False = tommy bar
                                           then Brush.Style:=bsSolid
                                           else Brush.Style:=bsClear;

                                        if  (check_limits(infill_points[0],infill_points[1])=True) and (check_limits(infill_points[2],infill_points[3])=True)
                                        and (check_limits(infill_points[4],infill_points[5])=True) and (check_limits(infill_points[6],infill_points[7])=True)
                                            then Polygon(Slice(infill_points,8)); // number of points, not index

                                      end;

                              if is_brick=True
                                 then begin
                                        arm:=scale;           // cross arm length 12" scale each way for clip
                                        diamond:=4*inscale;   // 4" size of centre diamond
                                      end
                                 else begin
                                        arm:=30*inscale;        // cross arm length 30" scale each way for normal target mark
                                        diamond:=15*inscale;    // 15" size of centre diamond
                                      end;

                              x1:=((p1.x-arm)*100+re_org_x)*sx;   // arm length is each way
...
begin
  ...
  with pad_form do begin
    ...
    if (show_reminders_menu_entry.Checked=False) or (keeps_list.Count<1) or {show_bgnd_keeps_menu_entry.Checked} (show_bgnd_templates=False)
       then begin
    ...
    for n:=0 to (keeps_list.Count-1) do begin
      ...
      with Ttemplate(keeps_list.Objects[n]) do begin
        ...
        if (group_selected=True) and (show_group_templates=False) then CONTINUE;

        if (group_selected=False) and (tagged_hidden=True) then CONTINUE;    // 237a  hidden prefix tagged template

        if skip_show_brick_template(n)=True then CONTINUE;  // 234a
...
begin
  ...
  with Ttemplate(keeps_list.Objects[bgk]) do begin
    ...
    if (group_selected=True) and (show_group_templates=False) {(pad_form.hide_group_templates_menu_entry.Checked=True)} then EXIT;

    if skip_show_brick_template(bgk)=True then EXIT;  // 234a

    try
      now_keep:=template_info.bgnd_keep;   // next background keep.

      with now_keep do begin

        for aq:=29 downto 0 do begin   // 226c bug-fix   was 25    29-26 = K-crossing check rails
...
var
  ...
  template_id:integer;  // 228b

  is_timber_brick:integer;  // 228b


                              //////////////////////////////////////////////////

                              procedure draw_bgnd_symbols(symbol_type:integer; yd:extended);    // 227
                              ...
                              begin
                                if pad_form.show_template_symbols_menu_entry.Checked=False then EXIT;

                                with canv do begin

                                  with Ttemplate(keeps_list.Objects[bgk]).template_info.file_blocks do begin

                                    if Length(keep_symbols)<1 then EXIT;

                                    for n:=0 to Length(keep_symbols)-1 do begin

                                      with keep_symbols[n] do begin
                                               ...
                                               0: begin                  // it's a dropper...
                                                    ...
                                                    for i:=0 to (num_points-1) do begin
                                                      ...
                                                      infill_points[i].Y:=Round(check_inty);
                                                         ...
                                                         // approximations using screen co-ordinates during mouse actions ...

                                                      if (shift_keeps_mod=1) and (Ttemplate(keeps_list.Objects[bgk]).group_selected=True)
                                                         then begin
                                                                infill_points[i].X:=infill_points[i].X+Round(xshift_keeps*fx);
                                                                infill_points[i].Y:=infill_points[i].Y+Round(yshift_keeps*fy);
                                                              end;

                                                      if (twist_keeps_mod=1) and (Ttemplate(keeps_list.Objects[bgk]).group_selected=True)
                                                         then begin
                                                                infill_points[i]:=keep_transform(kform_keeps,infill_points[i]);
                                                              end;
                                                    ...
                                                    if (shift_keeps_mod=1) and (Ttemplate(keeps_list.Objects[bgk]).group_selected=True)
                                                       then begin
                                                              move_to.X:=move_to.X+Round(xshift_keeps*fx);
                                                              move_to.Y:=move_to.Y+Round(yshift_keeps*fy);

                                                              line_to.X:=line_to.X+Round(xshift_keeps*fx);
                                                              line_to.Y:=line_to.Y+Round(yshift_keeps*fy);
                                                            end;

                                                    if (twist_keeps_mod=1) and (Ttemplate(keeps_list.Objects[bgk]).group_selected=True)
                                                       then begin
                                                              move_to:=keep_transform(kform_keeps,move_to);
                                                              line_to:=keep_transform(kform_keeps,line_to);
                                                            end;
                              ...
                              procedure do_the_marks(chairs:boolean);   // 232a sub-proc   no chairs, or chairs only
                                ...
                                with canv do begin
                                  ...
                                  for bgk:=0 to (keeps_list.Count-1) do begin
                                    ...
                                    with Ttemplate(keeps_list.Objects[bgk]) do begin
                                      ...
                                      if (group_selected=True) and (show_group_templates=False) {(pad_form.hide_group_templates_menu_entry.Checked=True)} then CONTINUE;  // 209c

                                      if (group_selected=False) and (tagged_hidden=True) then CONTINUE;    // 237a  hide prefix tagged template


                                      if skip_show_brick_template(bgk)=True then CONTINUE;  // 234a

                                      keep_selected:=group_selected;

                                      template_id:=template_info.keep_dims.box_dims1.now_time;   // template ID  228b
                                      ...
                                      try
                                        ...
                                        template_id:=template_info.keep_dims.box_dims1.now_time;   // template ID
                                        ...
                                        with now_keep do begin
                                          ...
                                          for i:=0 to array_max do begin
                                            ...
                                            if (code>999) and (code<9996) then CONTINUE;  // 227a bug fix       9996-9999 is DXF stuff

                                            if chairs=True    // do chairs only
                                               then begin
                                                      case code of

                                        484,485,493,494,497: if (chair_outlines_checkbox.Checked=True) and (screenx<800*scale)
                                                                then begin
                                                                       if using_marker_colour=False then Pen.Color:=bgkeep_timber_colour  // chair outlines.
                                                                                                    else Pen.Color:=marker_colour;
                                                                     end
                                                                else CONTINUE;

                                                491,495,496: if (chair_outlines_checkbox.Checked=True) and (screenx<800*scale)
                                                                then begin
                                                                       if using_marker_colour=False then Pen.Color:=clGray           // chair plug, chair bolts
                                                                                                    else Pen.Color:=marker_colour;
                                                                     end
                                                                else CONTINUE;

                                                        498: if (chair_sockets_checkbox.Checked=True) and (screenx<800*scale)
                                                                then begin
                                                                       if using_marker_colour=False then Pen.Color:=clGray           // chair sockets for 2-D
                                                                                                    else Pen.Color:=marker_colour;
                                                                     end
                                                                else CONTINUE;

                                                        else CONTINUE;  // chairs (2-D) only for this run
                                                      end;//case
                                 ...
                                 -4,0,501..508,600..605,700..703: CONTINUE;    // ignore blank lines.   501..508 ignore check-rail labels
                                                                               // 206b  600..605, 700..703 ignore long marks and switch/xing labels on trackpad

                                                    480..483,492: CONTINUE;    // chair keys, 3-D chair sockets

                             484,485,491,493,494,495,496,497,498: CONTINUE;    // ignore chair outlines for this run

                                                  -3,-2: if radial_centre_marks_checkbox.Checked=True
                                                            then begin
                                           ...
                                           1,2,7,10,101: if guide_marks_checkbox.Checked=True
                                                            then begin
                                                                   if using_marker_colour=False then Pen.Color:=bgkeep_mark_colour     // guide marks, radial ends, transition ends. switch drive
                                                                                                else Pen.Color:=marker_colour;
                                                                 end
                                                            else CONTINUE;
                                            ...
                                            203,233,293: if (timber_infill_checkbox.Checked=True) and ( (screenx<200*scale) or (bgpad_timb_infill_style>2) )  // infill on pad if solid/blank fill or large enough to see hatching.
                                                    ...
                                                    479: if dropper_ridge_marks_checkbox.Checked=True  // dropper wire ridges  243b
                                                            then begin
                                                                   if using_marker_colour=False then Pen.Color:=bgkeep_timber_colour
                                                                                                else Pen.Color:=marker_colour;
                                                                 end
                                                    ...
                                                    490: if timber_outlines_flanges_checkbox.Checked=True      // timber outer flanges for 3-D
                                                            then begin
                                                                   if (template_info.keep_dims.box_dims1.timbering_brick_flag=False) and (bricks_only_timber_outlines_flanges_checkbox.Checked=True)   // only brick templates?
                                                                      then CONTINUE;   // 234c

                                                                   if using_marker_colour=True
                                                                      then Pen.Color:=marker_colour
                                                                      else Pen.Color:=bgkeep_timber_colour;
                                                                 end
                                                            else CONTINUE;


                                                   9996: if timber_outlines_kerf_checkbox.Checked=True
                                                            then begin
                                                                   if using_marker_colour=False then Pen.Color:=clBlack          // timber outline kerf lines for 2-D
                                                                                                else Pen.Color:=marker_colour;
                                                                 end
                                                            else CONTINUE;


                                                   9997: if (chair_sockets_kerf_checkbox.Checked=True) and (screenx<800*scale)
                                                            then begin
                                                                   if using_marker_colour=False then Pen.Color:=clBlue          // chair socket kerf lines for 2-D
                                                                                                else Pen.Color:=marker_colour;
                                                                 end
                                                            else CONTINUE;

                                              9998,9999: if timber_outlines_flanges_checkbox.Checked=True      // timber webs and sprue for 3-D
                                                            then begin
                                                                   if (template_info.keep_dims.box_dims1.timbering_brick_flag=False) and (bricks_only_timber_outlines_flanges_checkbox.Checked=True)   // only brick templates?
                                                                      then CONTINUE;  // 234c
                                            ...
                                            if ((code=203) or (code=233) or (code=293) or (code=484) or (code=485) or (code=493) or (code=494) or (code=497) {or (code=-233) or (code=-293)} ) and (i<array_max)    // timber infill,  chair outlines 221a
                                               then begin
                                                      ...
                                                      if (code=484) or (code=485) or (code=493) or (code=494) or (code=497)    // 221a  another 12 marks for radiused corners, and 2 marks for plinth rectangle ...
                                                         ...
                                                         then begin
                                                                ...
                                                                repeat
                                                                    p[nm].X:=intarray_get(list_bgnd_marks[0],i+nn);       // 1/100ths mm
                                                                    p[nm].Y:=intarray_get(list_bgnd_marks[1],i+nn);
                                                                  p[nm+1].X:=intarray_get(list_bgnd_marks[2],i+nn);
                                                                  p[nm+1].Y:=intarray_get(list_bgnd_marks[3],i+nn);
                                                                ...
                                                                for nm:=0 to 27 do begin       // was 23
                                                                  p[nm].X:=Round(limits(h_minint,h_maxint,p[nm].X*sx+ex-gx,dummy_i));
                                                                  p[nm].Y:=Round(limits(h_minint,h_maxint,p[nm].Y*sy+by-gy,dummy_i));
                                                                end;//next
                                            ...
                                            if (code>0) {or (code=-233) or (code=-293)}
                                               then begin
                                                      move_to.X:=Round(check_int1x);  move_to.Y:=Round(check_int1y);
                                                      line_to.X:=Round(check_int2x);  line_to.Y:=Round(check_int2y);
                                                      ...
                                                      if (code=203) or (code=233) or (code=293) or (code=484) or (code=485) or (code=493) or (code=494) or (code=497) {or (code=-233) or (code=-293)}     // timber infill, chair outlines ...
                                                         then begin
                                                                ...
                                                                if (code=484) or (code=485) or (code=493) or (code=494) or (code=497)         // 221a  chair outlines
                                                                   then begin
                                                                          for nm:=0 to 27 do begin               // 221a  was 23

                                                                            infill_points[nm+4].X:=p[nm].X;
                                                                            infill_points[nm+4].Y:=p[nm].Y;
                                                                          end;//next
                                                                        end;
                                                                ...
                                                                if (check_limits(infill_points[0],infill_points[1])=True) and (check_limits(infill_points[2],infill_points[3])=True)
                                                                   then begin
                                                                           ...
                                                                           if (code=484) or (code=485) or (code=493) or (code=494) or (code=497)   // chair outlines
                                                                              then begin
                                                                                     Pen.Width:=1;              // no bold outlines
                                                                                     Brush.Color:=$0090A0B0;
                                                                                     Brush.Style:=bsSolid;
                                                                                   end
                                                                              else begin                        // timber infill...
                                                                                     ...
                                                                                     // OT2024 or (clips_form.Visible=True)    // 241c
                                                                                        then Brush.Style:=bsDiagCross
                                                                                        else begin
                                                                ...
                                                                and (code<>491) and (code<>495) and (code<>496)  // chair plug,  bolt centres
                                                                   then Pen.Width:=3
                                                                   else Pen.Width:=1;
                                          ...
                                          with template_info.file_blocks do begin
                                            ...
                                            if (Length(keep_shoved_timbers)>0) and (template_info.keep_dims.turnout_info2.no_timbering_flag=False) and (bgnd_timber_fill_overdraw_generated=True)   // 227d
                                               then begin
                                                      for i:=0 to Length(keep_shoved_timbers)-1 do begin

                                                        with keep_shoved_timbers[i] do begin
                                                          ...
                                                          if (shove_data.sv_use_tcol=True) and (shove_data.sv_bgnd=True)  // overdraw previous infill on output
                                                             then begin

                                                                    check_int1x:=limits(h_minint,h_maxint,stored_shoved_corners.p1.X*sx+ex-gx,dummy_i);      // h_min, h_max 31 bit to give room for some arithmetic on the data (shift keeps, etc.)
                                                                    check_int1y:=limits(h_minint,h_maxint,stored_shoved_corners.p1.Y*sy+by-gy,dummy_i);

                                                                    check_int2x:=limits(h_minint,h_maxint,stored_shoved_corners.p2.X*sx+ex-gx,dummy_i);
                                                                    check_int2y:=limits(h_minint,h_maxint,stored_shoved_corners.p2.Y*sy+by-gy,dummy_i);

                                                                    check_int3x:=limits(h_minint,h_maxint,stored_shoved_corners.p3.X*sx+ex-gx,dummy_i);      // h_min, h_max 31 bit to give room for some arithmetic on the data (shift keeps, etc.)
                                                                    check_int3y:=limits(h_minint,h_maxint,stored_shoved_corners.p3.Y*sy+by-gy,dummy_i);

                                                                    check_int4x:=limits(h_minint,h_maxint,stored_shoved_corners.p4.X*sx+ex-gx,dummy_i);
                                                                    check_int4y:=limits(h_minint,h_maxint,stored_shoved_corners.p4.Y*sy+by-gy,dummy_i);
                                                                    ...
                                                                    if (shift_keeps_mod=1) and (Ttemplate(keeps_list.Objects[bgk]).group_selected=True)
                                                                       then begin
                                                                              for sk:=0 to 3 do begin
                                                                                infill_points[sk].X:=infill_points[sk].X+Round(xshift_keeps*fx);
                                                                                infill_points[sk].Y:=infill_points[sk].Y+Round(yshift_keeps*fy);
                                                                              end;//next
                                                                            end;

                                                                    if (twist_keeps_mod=1) and (Ttemplate(keeps_list.Objects[bgk]).group_selected=True)
                                                                       then begin
                                                                              for sk:=0 to 3 do infill_points[sk]:=keep_transform(kform_keeps,infill_points[sk]);
                                                                            end;
                                                                    ...
                                                                    if (check_limits(infill_points[0],infill_points[1])=True) and (check_limits(infill_points[2],infill_points[3])=True)    // 221a ignore checks on remaining points for chairs
                                                                       then begin
                                                                              ...
                                                                              if using_marker_colour=False
                                                                                 then Pen.Color:=bgkeep_timber_colour  // timber outlines.
                                                                                 else Pen.Color:=marker_colour;
                                          ...
                                          end;//with template_info

                                        end;//with now_keep data.

                                        template_info.bgnd_keep:=now_keep;   // save the label location.

                                      except
                                        CONTINUE;      // ignore this template if calc exception
                                      end;// try
                                    end;//with template
...
begin
  if (keeps_list.Count<1) or (show_bgnd_templates=False) then EXIT;
  ...
  with canv do begin
    ...
    do_the_marks(False);   // 232a False = no chairs on first run

    if brick_form.bgnd_chairs_checkbox.Checked=True then do_the_marks(True);   // 232a add chairs if wanted
    ...
    for bgk:=0 to (keeps_list.Count-1) do begin
      ...
      with Ttemplate(keeps_list.Objects[bgk]) do begin
        ...
        try
          ...
          now_keep:=template_info.bgnd_keep;
          ...
          with now_keep do begin
            ...
            // but first draw the dropper symbols ...  227a

            bgnd_y_datum:=template_info.keep_dims.box_dims1.transform_info.datum_y*100;

            if (bgkeeps_form.feed_droppers_checkbox.Checked=True) then draw_bgnd_symbols(0,bgnd_y_datum);  // 0=dropper
              ...
              with platform_trackbed_info do begin
                ...
                if (adjacent_edges_keep=True) and (bgkeeps_form.trackbed_edges_checkbox.Checked=True)   // trackbed edges
                   then begin
                          ...
                          if (using_marker_colour=False) or (marker_colours_pad<3)
                             then Pen.Color:=bgkeep_trackbed_colour
                             else Pen.Color:=marker_colour;
                          ...
                          aq:=18;
                          draw_bgnd_rail(True);  // 235b solid lines    was dotted lines on screen
                          aq:=19;
                          draw_bgnd_rail(True);
                          aq:=22;
                          draw_bgnd_rail(True);
            ...
            with template_info.keep_dims.box_dims1.platform_trackbed_info do begin    // 0.93.a ...
              ...
              // now draw gap symbols and stickers over rails ...  227a

            if (bgkeeps_form.isolation_gaps_checkbox.Checked=True) then draw_bgnd_symbols(1,bgnd_y_datum);   // 1=gap
            if (bgkeeps_form.stickers_checkbox.Checked=True) then draw_bgnd_symbols(2,bgnd_y_datum);         // 2=sticker
    ...
    for bgk:=0 to (keeps_list.Count-1) do begin
      ...
      with Ttemplate(keeps_list.Objects[bgk]) do begin
        ...
        try
          ...
          template_info.bgnd_keep:=now_keep;   // save the label extent.
...
procedure draw_dtp_items_on_pad(on_canvas:TCanvas);    // 0.93.a  add sketchboard items
  ...
  // called only while sketchbook not active
...
begin
  ...
  if dtp_settings_form.transfer_mode_low_res_radiobutton.Checked=True    // stretchdraw a bitmap onto pad
     ...
     then begin
            ...
            low_res_bitmap.PixelFormat:=pf24bit;    // this seems to allow maximum zoomimg in stretchdraw
            ...
            try
              ...
              show_modal_message('Low-res stretch-draw failed.');
            end;//try
...
// OT2024    draw_dtp_items_on_pad(backdrop_bmp.Canvas);    // 0.93.a  add sketchbook items
...
// OT2024    if add_sketchboard_items=True then draw_dtp_items_on_pad(offdraw_bmp.Canvas);    // 0.93.a  add sketchboard items
...
begin
  ...
  if show_bgnd_templates=False then EXIT;  // 209c bug-fix  can't click on invisible template
  ...
  for bgk:=0 to (keeps_list.Count-1) do begin
    ...
    with Ttemplate(keeps_list.Objects[bgk]) do begin
      ...
      if (group_selected=True) and (show_group_templates=False) then CONTINUE;  // 209c  can't click on invisible template

      if (group_selected=False) and (tagged_hidden=True) then CONTINUE;    // 237a  hidden prefix-tagged template   can't click on hidden template

      if skip_show_brick_template(bgk)=True then CONTINUE;  // 234a    can't click on invisible template
      ...
      try
        ...
        with now_keep do begin
          ...
          for aq:=29 downto 0 do begin    // 226c bug-fix       was 25        26-29 = k-crossing check rails

            if (aq>15) and (aq<24) then CONTINUE;   // 226c ignore platforms and trackbed edges
...
procedure Tgrid_form.make_circle_shapes_buttonClick(Sender: TObject);

var                        // convert the spacing ring to background circle shapes
...
begin
  ...
  with new_shape do begin
    ...
    if brick_form.store_using_marker_colour_checkbox.Checked=True
       ...
       else wrap_offset:=clSilver;     // default (used for image wrapping, and for target clip/brick splint colours)
    ...
    if rings[0,2]>0              // inner diameter.
       then begin
              ...
              wrap_offset:=clSilver;        // default (used for image wrapping, and for target clip/brick splint colours)
              show_transparent:=False;      // default (used only for images),   and for target clip/type
              picture_is_metafile:=False;   // default (used only for images)
...
procedure Tgrid_form.make_target_clip_buttonClick(Sender: TObject);       // 233c

var                               // add background target mark/clip at the spacing ring
  new_shape:Tbgnd_shape;
  n:integer;
  time_str:string;
...
begin
  ...
  with new_shape do begin
    ...
    p2.y:=0;           // clip rotation angle

    rect_rot:=0;  // 234b

    is_brick:=(TButton(Sender).Tag=1);  // make clip instead of target mark 234b

    clip_pairing_code:=0;  // 234c not yet paired   unused for target mark

    if is_brick=True
       then shape_name:='ex-ring brick clip '+time_str
       else shape_name:='ex-ring target mark '+time_str;

    if (is_brick=True) and (brick_form.store_using_marker_colour_checkbox.Checked=True)
       then wrap_offset:=brick_form.store_marker_colour_panel.Color
       else wrap_offset:=clSilver;     // default (used for image wrapping, and for target clip/brick splint colours)
...
begin
  ...
  show_dummy_vehicles_radio_button.Checked:=True;  // 227a
...
begin
  ...
  if dv_copies_index>(dv_copies_c-1)
     then begin
            show_modal_message('Dummy vehicle copies - limit reached.'+#13+#13+'To make another copy you must first delete one or more existing copies,'
...
begin
  if dv_envelopes_index<0
     then begin
            show_modal_message('There are no envelopes to delete.');
            EXIT;
          end;
...
function skip_show_brick_template(n:integer):boolean;     // 234a

   // return True if template is to be skipped

var
  is_brick,is_current_brick,show_bricks,show_only_bricks,all_bricks:boolean;

begin
  RESULT:=False;  // init default show all templates

  with Ttemplate(keeps_list.Objects[n]).template_info.keep_dims.box_dims1 do begin
    is_brick:=timbering_brick_flag;
    is_current_brick:=(is_brick=True) and (use_pad_marker_colour=True) and (pad_marker_colour=brick_form.store_marker_colour_panel.Color);
  end;//with

  show_bricks:=pad_form.show_brick_templates_menu_entry.Checked;            // radio item 1
  show_only_bricks:=pad_form.show_only_brick_templates_menu_entry.Checked;  // radio item 1

  if (show_only_bricks=True) and (is_brick=False)
     then begin
            RESULT:=True;  // don't show this one
            EXIT;
          end;

  all_bricks:=pad_form.all_brick_templates_menu_entry.Checked;              // radio item 2

  if (show_bricks=True) or (show_only_bricks=True)
     then begin
            if (all_bricks=True) or (is_current_brick=True) then EXIT;      // show these
          end;

  RESULT:=is_brick;  // hide this brick template
end;
//______________________________________________________________________________
