...
uses Printers, control_room, grid_unit, alert_unit, math_unit, calibration_unit,
  ...
  print_settings_unit, export_unit, pdf_laz_unit;
  ...
  if (include_bgnd=True) and (keeps_list.Count>0)
     then begin
            ...
            for n:=0 to (keeps_list.Count-1) do begin
              ...
              with Ttemplate(keeps_list.Objects[n]).template_info.bgnd_keep do begin
...
*)
            if (output_code=1) or (output_code=2)  // OT2024
               then begin
            ...
            try
              ...
              printer_width_dots:=Printer.PageWidth;    // printer page-width in dots
              printer_length_dots:=Printer.PageHeight;  // printer page-length in dots

              nom_width_dpi:=Printer.XDPI;  // dpi across width
              nom_length_dpi:=Printer.YDPI; // dpi down length
...
(* OT2024
                    ...
                    page_margin_top_mm:=0;    // 0.93.a  1-3-2009  no trim margins on sketchbook
...
*)
  ...
  sheet_co_wide:=do_truncx((max_wide-page_left_offset)/page_width_out);    // max sheet index (no of sheets -1) across (calc from left page margin).
  sheet_co_long:=do_truncx((max_long-page_top_offset)/page_length_out);
        ...
        if (print_entire_pad_flag=True) and (keeps_list.Count>0) { and (pad_form.print_keeps_on_menu_entry.Checked=True) }
           then begin
                  ...
                  for n:=0 to (keeps_list.Count-1) do begin
                    ...
                    bgndk:=Ttemplate(keeps_list.Objects[n]).template_info.bgnd_keep;
            ...
            if no_alerts=True
               then EXIT
                      ...
                      if print_entire_pad_flag=False     // mods 0.93.a
                         then begin
                                {i:=}alert(6,'  no  data  for  print  control  template  preview',
...
begin
  // OT2024 if go_sketchboard=False then EXIT;  // 205e   sketchboard not in use

  if print_settings_form.output_sketchboard_items_checkbox.Checked=False then EXIT;  // output not wanted

  if pad_form.page_map_sketchboard_items_menu_entry.Checked=False then EXIT;  // preview not wanted
  ...
  on_canvas.Pen.Color:=grid_colour;

  on_canvas.Brush.Style:=bsBDiagonal;
  on_canvas.Brush.Color:=grid_colour;
...
begin
  ...
  try
    ...
    prev_canvas:=offdraw_bmp.Canvas;     // the off-screen draw bitmap.
...
*)
      ...
      if  (pad_form.page_map_background_shapes_menu_entry.Checked=True) // 206e
                                   ...
                                   then begin
                                          case shape_code of
                                                ...
                                                -1: begin     // picture = bitmap image.
                                                 ...
                                                 1: begin   // 236d twisted rectangle

                                                       //  mods for twist rectangle
                                                      ...
                                                      for nn:=0 to 3 do begin
                                                        infill_points[nn].X:=  left_dots+Round((infill_points_pex[nn].x*100+re_org_x)*pvsx);
                                                        infill_points[nn].Y:=bottom_dots-Round((infill_points_pex[nn].y*100+re_org_y)*pvsy);
                                                      end;//next
      ...
      if (print_entire_pad_flag=True) and (keeps_list.Count>0)
         then begin
                ...
                for bgk:=0 to (keeps_list.Count-1) do begin  // next background keep.
                  ...
                  now_keep:=Ttemplate(keeps_list.Objects[bgk]).template_info.bgnd_keep;
                      ...
                      with now_keep do begin
                        ...
                        for aq:=0 to aq_max_c do begin
                          ...
                          if adjacent_edges_keep=True  // platforms and trackbed edges
                             then begin
                                    if (aq=18) or (aq=19) or (aq=22) or (aq=23) then Pen.Style:=psSolid;  // trackbed edges
                          ...
                          if (aq=16) or (aq=17) or (aq=20) or (aq=21) then Pen.Color:=bgkeep_platform_colour;  // platform front and rear edges.

                          if (aq=18) or (aq=19) or (aq=22) or (aq=23) then Pen.Color:=bgkeep_trackbed_colour;  // 236b
...
begin
  ...
  try
    ...
    with Printer.Canvas do begin
      ...
      top_str:=' '+DateToStr(Date)+'  '+TimeToStr(Time)+'      TEMPLOT V:'+round_str(program_version/100,2)+'  from  Martin  Wynne   85a.uk/templot';
      TextOut(0,0,top_str);
