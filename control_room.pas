...
try
  ...
  control_room_form.pad_maximized_radio.Checked:=(pad_form.WindowState=wsMaximized);        // 227a
  control_room_form.pad_downsized_radio.Checked:= NOT (pad_form.WindowState=wsMaximized);   // 227a

     // 223c ...

  shove_timber_form.rolled_in_sleepered_radio_button.Checked:=rolled_in_sleepered;         // radio item
  shove_timber_form.rolled_in_rails_only_radio_button.Checked:= NOT rolled_in_sleepered;   // radio item

  pad_form.rolled_in_sleepered_menu_entry.Checked:=rolled_in_sleepered;                    // radio item
  pad_form.rolled_in_rails_only_menu_entry.Checked:= NOT rolled_in_sleepered;              // radio item


  with pad_form do begin

    control_is_brick_menu_entry.Checked:=timbering_brick;   // 234a

    if timbering_brick=True
       then info_form.gauge_bang_label.Font.Color:=$0000A0FF   // orange
       else info_form.gauge_bang_label.Font.Color:=clYellow;

      // 223c ...

    if (plain_track=True) or (half_diamond=True)                    // 227a
       then facing_trailing_button.Caption:='swap  ends'
       else facing_trailing_button.Caption:='facing  -  trailing';

    insert_turnout_button.Enabled:=plain_track;
    convert_to_plain_track_button.Enabled:= NOT plain_track;

    insert_turnout_menu_entry.Enabled:=plain_track;
    convert_to_plain_track_menu_entry.Enabled:= NOT plain_track;

    if (plain_track=True) or (half_diamond=False) or (tradius_is_straight=True) or ((half_diamond=True) and (ABS(hdkn-k3n)<0.005))
    ...
    copy_for_brick_popup_entry.Enabled:=classic_templot;      // 241c
       ...
       then begin
              ...
              case slew_mode of
                              1: begin
                                   ...
                                   info_form.slew_caution_mode_label.Caption:='The control template contains a mode 1  SLEW . Smallest radius info is not available.';
                                 end;
                              2: begin
                                   ...
                                   info_form.slew_caution_mode_label.Caption:='The control template contains a mode 2  SLEW . Smallest radius info is not available.';
                                 end;
              end;//case
    ...
    switch_drive_menu_entry.Checked:=(switch_drive_flag=True) and (plain_track=False); // and (half_diamond=False);  227a   switch-diamond
    switch_drive_menu_entry.Enabled:=(plain_track=False);                              // and (half_diamond=False);  227a
    ...
    if plain_track=True
       then begin
              ...
              turnout_road_options_menu_entry.Caption:='turnout-road  e&xit  length';
            end
       else begin
              if half_diamond=False
                 then begin
                        ...
                        turnout_road_options_menu_entry.Caption:='diagonal-road  e&xit  length';
                      end;
            ...
            if abandon_calcs=True
               then begin
                      case alert(2,'    calculations abandoned',
                                   '||Sorry, your data confused Templot. The turnout calculations had to be abandoned.'
...
begin
  try
    ...
    ShowMessage('Sorry, your computer does not have enough free memory to run Templot.'
               +#13+#13
               +'Try restarting Templot with no other programs running.');  // 0.93.a
...
begin
  ...
  ShowWindow(GetConsoleWindow(),SW_MINIMIZE);    // for Templot5 compiled on Lazarus

  BorderIcons:=[];  // 205c prevent him clicking X, wanting only to hide splash, not quit.

  // 555a   if DirectoryExists('C:\Windows\ImmersiveControlPanel')=True      // 212a we are in Windows 8 or later.
  // 555a   then win7_menus_menu_entry.Click;                             // default to the latest menu style instead of XP style

  // 205c in case Enter key held down disable until after internet check...
  ...
  // OT2024 picture_panel.Visible:=True;

  Hide;  // hide the form until ready

(* OT2024
  if Screen.Width<1270           // make sketchboard fit the screen 1024 x 768
     then begin
            dtp_form.Left:=6;
            dtp_settings_form.Left:=560;

            dtp_form.Top:=12;
            dtp_settings_form.Top:=24;
          end
     else begin
            dtp_form.Left:=20;
            dtp_settings_form.Left:=820;

            if Screen.Height<1000     // old laptop
               then begin
                      dtp_form.Top:=12;
                      dtp_settings_form.Top:=60;
                    end
               else begin                // 1280 x 1024 or more
                      dtp_form.Top:=18;
                      dtp_settings_form.Top:=200;
                    end;
...
*)
  ...
  ForceDirectories(exe_str+'internal\fview');    // 208d fview needed for file viewer, in case not created on older installs
  ...
  if Win32Platform=VER_PLATFORM_WIN32_NT   // running under Windows NT/2000/XP , set 32-bit default...  0.93.a was 24-bit
     then begin
  ...
  if hi_color=False                   //  8-bit (or lower)
     then begin                       //  set the form colours for lo-colour...
            ...
            //data_entry_form.Color:=clGray;
            gauge_form.Color:=clGray;
            dxf_form.Color:=clBtnFace;
            //thick_form.Color:=clBtnFace;
            ...
            alert_colour[0]:=clBlack;         // program alert
  ...
  if (Screen.MonitorCount>1) and (multi_monitors_msg_pref=False)     //%%%%
     then begin
            ...
            if alert(7,'   Templot   -   Multiple  Monitors',
                       'Your computer is currently set to use multiple monitors.'
                      +'||Do you want the Templot trackpad to allow use of the full screen area across all monitors?'
                      +'||A small margin will be allowed initially at the right and bottom borders for easier resizing to a smaller area.'
                      +'||Be aware that if you answer yes, Templot will use more memory and the screen response may be slower on older systems.'
                      +' For the fastest screen response, arrange the primary monitor to be on the left.'
                      +'||If you answer no, the maximum usable size of the Templot trackpad will be restricted to match your primary monitor only.'
               ...
               else he_wants_multiple_monitors:=False;   // 223e
  ...
  if (GetKeyState(VK_CAPITAL) AND 1)<>0    // caps lock on at startup...
     then begin
            alert(3,'   Templot   -   Caps Lock  ON',
                     'Your computer currently has the Caps Lock ON (keyboard indicator light showing).'
                    +'||While the Caps Lock is on Templot will highlight any background templates on the drawing as the mouse pointer'
                    +' passes over their name labels.'
                    +'||If you have just started Templot for the first time you may find this confusing.'
  ...
  if (GetKeyState(VK_SCROLL) AND 1)<>0    // 244b scroll lock on at startup...
     then begin
            alert(3,'   Templot   -   Scroll Lock  ON',
                     'Your computer currently has the Scroll Lock ON (keyboard indicator light showing).'
                    +'||While the Scroll Lock is on you will not be able to select background templates by simply clicking on them.'
                    +'||If you have just started Templot for the first time you may find this confusing.'
                    +'||You can avoid this by switching the Scroll Lock OFF. To do this press the  `0SCROLL LOCK`2  key once so that the indicator light is no longer showing.'
                    +'||Please click the green OK bar to continue.',
                     '','','','','','O K',0);
          end;
  ...
  if (he_wants_multiple_monitors=True) or (running_under_wine=True)  // monitors 0.91.b,   Wine 212a to allow dragging to find hidden modal dialogs
     then begin
            ...
            pad_form.Width:=Screen.DesktopWidth-24;    // initial margins for easier resizing.
            pad_form.Height:=Screen.DesktopHeight-60;  // allow for taskbar.
          end
  ...
  info_form.Top:=104;  // 235b was 96     216a   deeper toolbar    was 60 0.91.d     //was as above. problems with offset multiple monitors.
  ...
  if FileExists(pbo_str)=False then first_time_here_label.Show  // 0.97.b  // beginner first few times (no prior-previous)
                               else trackpad_groupbox.Show;     // 227a    // not beginner now

  init_rollbacks;         // also inits notch rollback, parking bays and org_template.

  fill_kd(org_template);     // 224a     default for use on importing mecbox files
  ...
  Caption:='        TEMPLOT5       program  panel';

  if (ParamCount>0)    // command line parameters?
     then begin
            param_str:=LowerCase(ParamStr(ParamCount));
            if ExtractFileExt(param_str)='.box'
               then begin
...
(* OT2024                                             else begin
                                                    if ExtractFileExt(param_str)='.sk9'
                                                       then begin
                                                              if FileExists(param_str)=True then load_sketchboard_file(param_str);
                                                            end;
...
*)
  ...
  for i:=0 to parking_c do begin

   fill_kd(parking_bay[i]);   // ditto initial fill all parking bays per current pad.

   park_name_str[i]:='';                                            // init template names in parking bay.
   park_memo_str[i]:=' your memo notes for this template ...|';     // init template memo notes in parking bay.
  end;
     ...
     then begin
            ...
            if prefs_warn_msg_pref=False     //%%%%
               then begin
                      ...
                      if alert(3,'   Templot  started  using  saved  program  preferences',
                                 ' |Templot started using saved program preferences.'
                                +'||If you are a new or infrequent user of Templot please be aware that running Templot using saved program preferences may mean that you won''t see important help messages and hints.'
                                +'||If you experience difficulty you may prefer to quit Templot and restart without using saved preferences (by clicking the initial GO button).| ',
  ...
  with control_room_form do begin
    ...
    session_menu.Enabled:=True;
    file_menu.Enabled:=True;
    preferences_menu.Enabled:=True;
    background_menu.Enabled:=True;
    program_menu.Enabled:=True;
  ...
  default_file_str:=exe_str+'BOX-FILES\start.box';
...
begin
  about_str:='<P ALIGN="CENTER" STYLE="margin-top:0px;"><IMG SRC="'+exe_str+'internal\hlp\templot2_logo_shadow_262_74_8bit.png"></P>'
            ...
            +'<HR NOSHADE>'
            +'<P CLASS="mainheading" ALIGN="CENTER" STYLE="font-size:20px; color:#0077DD;">TEMPLOT &nbsp;Version &nbsp;'+round_str(program_version/100,2)+version_build+'</P>'
            +'<P CLASS="centerbold">a  &nbsp;shared   &nbsp;hobby  &nbsp;project  &nbsp;from &nbsp;MARTIN &nbsp;WYNNE</P>'
            +'<P CLASS="centerbold"><A HREF="go_to_templot_com.85a">85a.uk / templot</A></P>'
            +'<P CLASS="center"><SPAN STYLE="font-size:12px; color:#777777;">&copy; 2024 &nbsp;all rights reserved</SPAN></P>'
            +'<HR NOSHADE>'
            +'<TABLE WIDTH="90%" ALIGN="CENTER"><TR>'
            +'<TD CLASS="notes" VALIGN="TOP" WIDTH="35%">This program includes<BR>these code elements,<BR>with thanks to authors:&nbsp;</TD>'
            +'<TD CLASS="notes">THtmlViewer &copy; David Baldwin'
            +'<BR>TGIFImage &copy; Anders Melander'
            +'<BR>Resample &copy; Anders Melander'   // 227
            +'<BR>TPNGObject &copy; Gustavo Daud'
            +'<BR>DtpDocuments &copy; Nils Haeck'
            +'<BR>TRsRuler &copy; Hans Roos'
            +'<BR>TRichView &copy; Sergey Tkachenko'
            +'<BR>wPDF &copy; WPCubed GmbH'
            +'<BR>Graphics32 &copy; Alex Denisov'
            +'<BR>TAcquireImage &copy; Roberto Zarrelli, Achim Metzen'
            +'<BR>Synapse &copy; Lukas Gebauer'                         // 227
            +'</TD></TR></TABLE>';


  no_new_help_sizes:=True;      // don't change the user's default sizes.

  if Screen.PixelsPerInch>121      // 211b
     then begin
            help_form.ClientWidth:=742;     //  *1.4 ..
            help_form.ClientHeight:=994;
          end
     else begin
            help_form.ClientWidth:=530;
            help_form.ClientHeight:=710;
          end;
                                ...
                                if dtp_form.dtp_document.Modified=True
                                   then begin
                                          i:=alert(7,'php/501    save  sketchboard ?',
                                                     ' |Save sketchboard?'
                                                    +'||There are unsaved changes on the sketchboard which will be lost if not saved before you quit Templot.'
 ...
 *)
                      ...
                      if CanClose=True
                         then begin
                                ...
                                info_form.Close;      //  !!! bug in Delphi - address violation if child forms overlapping the bottom of the pad on quit, so close first.
                                grid_form.Close;
                                shove_timber_form.Close;
                                heave_chairs_form.Close;  // 244a
                                ...
                                gaps_form.Close;         // 227a
                                lib_form.Close;          // 227a
                                intersect_form.Close;    // 227a

                                brick_form.Close;       // 235b

// OT2024                                dtp_form.Close;  // 0.93.a !!! needed to prevent AV ???
                              end;
...
const
  ...
  +'||Clicking the green  " = "  buttons in Templot brings up this "chat box",'
...
const
  help_str:string='            Program  Panel.'
   ...
   +' The program panel can be kept in view by resizing the trackpad window. Closing the program panel will quit Templot.'

   +'||The `0PROGRAM`1 menu includes various options for experienced users to customise the way Templot works.'

   +'||green_panel_begin tree.gif Until you become an experienced user of Templot you will probably have little use for the program panel, as most of the menu items are repeated elsewhere in Templot.green_panel_end';
...
begin
  ...
  trackpad_groupbox.Show;  // 227a

  pad_form.Show;

  cancel_adjusts(False);

  if beginner_counter=0 then beginner_counter:=1;      // 0.93.a start it running
...
const
 ...
 +' use this form to set a custom number of terms which Templot will use in expanding the Ideal Transition Spiral.'
 ...
 +' This will not cause any harm, but may give unpredictable results, or cause Windows to terminate the running of Templot.'
 +' For this reason, please save your previous work before experimenting. To minimise the possibility of this happening,'
 +' Templot actually performs the transition calculations in kilometres, and does its best to recover gracefully from'
...
const
  slow_str:string='      Slow  Running'
    ...
    +'||A suitable count for your computer will need to be established by trial and error. Higher counts cause Templot to run more slowly.'
...
const
  max_spiral_str:string='    Expert  Help  -  Max  Spiral'
    ...
    +'||Templot therefore limits transitions to a maximum "gentleness", i.e. a maximum value for the "spiral constant" showing in the INFORMATION panel.'
    ...
    +'||CAUTION: Setting too high a limit could cause Templot to appear to "hang" when adjusting transition curves by mouse action. A figure in excess of 2000 is unlikely to produce a usable template.'
...
const
  help_str:string='      Expert  Help  -  Step  Size'
           +'||Templot calculates the alignment of the rails by stepping along them. The size of each step can be changed, although there is normally'
...
const
  help_str:string='      Expert  Help  -  Re-origination'
           ...
           +' from Templot. For more information select the PROGRAM > EXPERT > DATA DISTORTIONS > ? HELP menu item.'
...
const
  zero_help_str:string='   Trailing  Zeros'
                      +'||For the drawing calculations Templot always uses the maximum precision available from your processor, but normally rounds results to two decimal places'
...
begin
  i:=alert(4,'   negative  numbers',
             '|Templot encloses negative values within square brackets as [ -000.0 ] when displaying them,'
...
begin
  ...
  chat('You have clicked the TEST menu item which I use for experimental items and for testing Templot during development.'
...
begin
  ...
  with timb_len_list do begin                                                       // 227a
    if Count>0 then for index:=0 to Count-1 do Ttimber_count_object(Objects[index]).Free;
    Free;
  end;//with

  with box_timber_lengths_list do begin                                             // 227a
    if Count>0 then for index:=0 to Count-1 do Ttimber_count_object(Objects[index]).Free;
    Free;
  end;//with
...
const
 ...
 +' longer be accurate - so please heed the warnings about not using these data distortions when printing directly from Templot.'
...
begin
  ...
  try
    repeat
      ...
      if n<>0 then EXIT;
      if getdims('Y - scaling  factor',distortion_help_str,pad_form,0,od)=True
         then begin
      ...
      if y_distortion_factor=1 then begin
                                    ...
                                    i:=alert(7,'        Y - scaling  distortion ?',
                                              ...
                                              +' construction templates to be unusable when printed. This function is intended as'
                                              +' a means of adjustment when the output will be elsewhere, such as on the sketchboard.'
                                              +'||If your reason is that your printer is not producing accurate results, do a printer calibration instead.'
                                              +'||Are you sure you want to distort your template data ?',
                                               '','','yes  -  use  '+round_str(temp*100,2)+' %   Y - scaling  distortion','?  help','no  -  cancel  distortion',
                                               'no  -  do  printer  calibration  instead',4);
                                    if i=3 then begin
                                                  if keeps_list.Count>0
                                                     then begin
                                                            if alert(4,'        restore  wiped  templates ?',
                                                                       'Do you now want to restore previously wiped templates to the background ?'
                                                                      +'||They will reflect to your distortion settings.' 
                                                                      +'||(This will include all unused templates.)',
                                                                       '','','','no  thanks','','yes  please  -  restore  templates',0)=6

                                                               then keep_form.all_to_bgnd_menu_entry.Click;    // 226e
                                                          end;
                                                  EXIT;
                                                end
                                           else begin
...
const
 ...
 +'||There should NEVER be any need to apply an aspect-distortion factor when printing directly from Templot - leave the factor setting at 100%'
 ...
 +' longer be accurate - so please heed the warnings above about not using data distortions when printing directly from Templot.'
...
const
  ...
  +'||The present version of Templot does all calculations on unit angles using RAM (Right-Angle Measure),'
  ...
  +'||Later versions of Templot will permit a global change of unit measure (here in this menu), and the converter tool will then'
...
const
  ...
  +'||The minimum dimension which Templot will accept is 0.5 mm.'
  +'|The maximum dimension which Templot will accept is 1000 mm.'
  +'|The pre-set dimension is 6 mm.'  // 229c  was 10mm   0.93.a was 15mm
  +'||Setting a figure greater than about 150 mm will handicap some of Templot''s functions to no advantage.'
  +'|------------------'
  +'|Caution:'
  ...
  +' with dimensions less than 6 mm. There is seldom any practical need to zoom in more than this.'
  ...
  +' They will re-appear as you zoom out again. This is a feature of the old Windows graphics system, not Templot, and does not apply to later versions of Windows.'
  +'||The recommended minimum settings are 3 mm for working at 4mm/ft scale, and 5 mm for 7mm/ft scale.';
...
begin
  ...
  if getdims('maximum  explode  ( zoom-in )','',control_room_form,n,od)=True
     then begin
            m:=od[0];
            if m=def_req then m:=6;    // default 6mm. 229c was 10mm  0.93.a was 15mm
...
const
  ...
  +'||Enter the required arm length in mm. The pre-set arm length is equivalent to a scale length of 9".'
...
begin
  ...
  if getdims('peg  arm  length','',control_room_form,n,od)=True
     then begin
            m:=od[0];
            if m=def_req then m:=9*inscale;    // 227a default 9"    was 3ft.
            peg_arm_length:=m;
          end;
  redraw(True);
end;
...
begin
  ClientWidth:=750;    // 208g  was 626
  ClientHeight:=442;   // must do these in this order to get neat result..   // 0.82.a was 330
  AutoScroll:=True;
  ...
  timb_len_list:=TStringList.Create;               // 227a
  box_timber_lengths_list:=TStringList.Create;     // 227a
  ...
  about_templot_version_menu_entry.Caption:='about  &Templot    ( v : '+FormatFloat('0.00',program_version/100)+version_build+' )';
...
begin
  backup_wanted:=True;
  intersect_form.find_intersect_button.Tag:=-1;   // 227a  recorded index no longer valid
end;
//_______________________________________________________________________________________
...
const
  ...
  +'||These options allow you to control the amount of cpu processor time allocated to Templot.'

  +'||The 100% ALWAYS (FAST) option causes Templot to use all of the processor time permitted by the Windows operating system.'
  ...
  +'||The ALLOW FULL IDLE (SLOW) option causes Templot to relinquish control of the cpu whenever there is no further processing to be done. This option will give a slower response to mouse activity and keyboard keys,'
  ...
  +'||n.b. In all versions of Templot prior to 0.73.a the only option available was 100% ALWAYS (FAST).';
...
const
  ...
  +'For <B>Windows 95 / 98 / ME</B> the setting is fixed at 16-bit and cannot be changed. This is a limitation in Windows, not Templot.'
...
const
  url_str:string='https://85a.uk/templot/';
...
const
  url_str:string='https://85a.uk/templot/club/';
...
const
  url_str:string='https://85a.uk/templot/companion/';    // 0.93.a
...
begin
  url_str:='https://85a.uk/templot/companion/'+dest_page_str;  // needs to be local
...
const
  url_str:string='https://85a.uk/templot/templot_donate.php';
...
const
  url_str:string='https://85a.uk/templot/club/index.php?threads/zoom-meetings-access-links.620/latest';
...
procedure go_to_upgrade;

begin
  go_to_templot_companion_page('manual_update.php');      // 227a
end;
//______________________________________________________________________________
...
const
  url_str:string='https://85a.uk/templot/companion/online_video_list.php';
...
procedure Tcontrol_room_form.session_menuClick(Sender: TObject);
...
procedure Tcontrol_room_form.FormDeactivate(Sender: TObject);

begin
  if (pad_button.Visible=True) and (beginner_counter=0) then beginner_counter:=1;      // 0.93.a start it running
end;
//______________________________________________________________________________
...
procedure exes_running_warn;

var
  exe_file_str:string;

begin
  exe_file_str:=ExtractFilePath(Application.ExeName)+'flashback_player.exe';     // player
  if FileExists(exe_file_str)=True
     then begin
            if DeleteFile(exe_file_str)=False                                    // must be running
               then ShowMessage('The Templot Video Player is running.'
                       +#13+#13+'Please close it now before clicking OK.'
                       +#13+#13+'The update cannot be installed while the Templot Video Player is running.'
                           +#13+'(You will see error messages.)');
          end;
  ...
  if i=3
     then begin
            ...
            if ShellExecute(0,'open',PChar(folder_str),nil,nil,SW_SHOWNORMAL)<=32
               then ShowMessage('Sorry, unable to open the PDF file.')
               else external_window_showing:=True;
          end;

  if i=4
     then begin
            folder_str:=ExtractFilePath(path_str);
...
begin
  ...
  if pad_form.show_bgnd_keeps_in_rect_menu_entry.Checked=False
     then begin
            ...
            pad_form.top_toolbar_panel.Color:=$00D0E0FF;          // shows yellowish
          end;

  pad_form.second_toolbar_panel.ParentColor:=False;     // 217a
  pad_form.second_toolbar_panel.Color:=$00D0E0FF;      // shows yellowish
end;
//______________________________________________________________________________
...
const
  ...
  +'||Change to the `0images as files`1 option if your system is low on RAM memory, or you have many hundreds of .box files in a single folder, or you have many other programs running concurrently with Templot.'
  +'||The screenshot images will then be saved as image files on the disk drive, instead of being stored in memory.'
  +' Be aware that this option will cause the viewer to work more slowly, and the option to export files in MECBOX format will not be available.';
...
procedure Tcontrol_room_form.screen_scaling_menu_entryClick(Sender: TObject);
...
begin
             // 211b allow manifest modify...

  if FileExists(exe_str+'dpi_yes.txt')=False

     then alert_str:='Some systems now have a high-definition display screen. Your current setting is: '+IntToStr(Screen.PixelsPerInch)+' dpi.'
            +'||This version of Templot has adjusted its display to make the best use of this with crisp fine-line drawing on the trackpad.'
            +'||This means that on high-definition systems you may find some of the buttons and text too small for comfortable use.'
            +'||You can if you prefer revert to the previous screen scaling method using Windows dpi-virtualization, which was used in earlier versions of Templot.'
            +'||This will make the text and controls larger, but the trackpad drawing may appear fuzzy or smudged. Also the `0sketchboard`3 functions will not be available.'


     else alert_str:='You may have previously used a high-definition display screen.'
                    +'||You can if you prefer revert to the previous high-definition screen scaling method for Templot.';


  if alert(2,'php/970    screen  scaling',alert_str
            +'||If you decide to change it will be necessary to re-start Templot to see the changes.'
            +'||You can return here at anytime by clicking the|`0program > screen scaling method...`1 menu item on the program panel. Each time you answer yes will swap to the opposite method.'
            +'||Do you want to change the screen scaling method?'
            +'||If you answer yes, Templot will close immediately, so make sure that you have saved your work.'
            +'||On older systems these changes will have no effect.',

             '','','','yes,  close  Templot  and  change  screen  scaling','','no  change  -  continue',0)=4

     then begin

            if alert(7,'php/970    change  screen  scaling',
                      'Are you sure? Templot will close immediately.',
                      '','','','','cancel','yes  -  change  screen  scaling  method',0)=5 then EXIT;

            folder_str:=exe_str+'dpi_change.exe';

            if ShellExecute(0,'open',PChar(folder_str),nil,nil,SW_SHOWNORMAL)<=32
               then begin
                      alert(2,'    function  failed',
                              'Sorry, unable to change the screen scaling method.',
                              '','','','','','continue',0);
                      EXIT;
                    end;
...
const
  popup_help_str:string='Enter a screen position for the pop-up menu which appears when you click on a background template.'
    ...
    +'||These co-ordinates refer to your full computer display screen for Windows, the position of Templot on the screen (if it is not full-screen) is not relevant.'
...
const
  trackpad_help_str:string='Enter a screen position for the trackpad window when it is not maximized.'
    ...
    +'||These co-ordinates are in screen dots from the top left corner of the screen: X across from the left, Y down from the top.'
    +'||These co-ordinates refer to your full computer display screen for Windows, the current position of Templot on the screen is not relevant.'
...
begin
  ...
  SHGetSpecialFolderLocation(Application.Handle,folder_id,PIDL);
...
procedure open_sk5_file;   // 216b

var
  sk5_str,loader_str,file_str:string;

begin
  with pad_form.sk5_file_open_dialog do begin

    InitialDir:=get_path_to_Windows_folder(CSIDL_PERSONAL);

    Filter:= ' SK5  files  ( .sk5)|*.sk5';
    Filename:='*.sk5';
    DefaultExt:='sk5';

    if Execute=True                  // get the file name.
       then sk5_str:=FileName
       else EXIT;
  end;//with

  loader_str:=exe_str+'templot_video_loader.exe';

  file_str:='"'+sk5_str+'"';

  if ShellExecute(0,'open',PChar(loader_str),PChar(file_str),nil,SW_SHOWNORMAL)<=32
     then begin
            alert(2,'    function  failed',
                  'Sorry, unable to open the SK5 file.',
                    '','','','','','continue',0);
            EXIT;
          end;
...
const
  ...
  +'||This setting controls the image quality when saving image files in JPG format from Templot.'
  ...
  +'||green_panel_begin tree.gif Generally it is better to use PNG format rather than JPG when saving image files from Templot.'
...
procedure Tcontrol_room_form.restore_t55_startup_menu_entryClick(Sender: TObject);   // 224a

begin
  copy_keep(org_template);
  show_and_redraw(True,True);          // redraw pad when ready.
  current_name_str:='';
  current_memo_str:=' your memo notes for this template ...|';
  info_form.ref_name_label.Caption:=current_name_str;
end;
//______________________________________________________________________________

procedure Tcontrol_room_form.bgs3_compact_menu_entryClick(Sender: TObject);

begin
  bgs3_compact_menu_entry.Checked:=True;   // radio item
end;
//_________________________________

procedure Tcontrol_room_form.bgs3_readable_menu_entryClick(Sender: TObject);

begin
  bgs3_readable_menu_entry.Checked:=True;  // radio item
end;
//______________________________________________________________________________

procedure Tcontrol_room_form.maps_buttonClick(Sender: TObject);
...
procedure Tcontrol_room_form.shapes_buttonClick(Sender: TObject);   // 226e
...
procedure Tcontrol_room_form.pad_maximized_radioMouseDown(Sender:TObject; Button:TMouseButton; Shift:TShiftState; X,Y:Integer);   // 227a

begin
  pad_form.WindowState:=wsMaximized;

  BringToFront;
end;
//______________________________________________________________________________

procedure Tcontrol_room_form.pad_downsized_radioMouseDown(Sender:TObject; Button:TMouseButton; Shift:TShiftState; X,Y:Integer);   // 227a

begin
  if pad_form.WindowState<>wsMaximized then EXIT;  // already draggable

  pad_form.WindowState:=wsNormal;

  pad_form.Left:=Screen.DesktopLeft+10;
  pad_form.Top:=Screen.DesktopTop+10;
  pad_form.Width:=Screen.DesktopWidth-20;    // margins for easier dragging/resizing.
  pad_form.Height:=Screen.DesktopHeight-80;  // allow for taskbar.

  BringToFront;
end;
//______________________________________________________________________________

procedure Tcontrol_room_form.search_site_for_menu_entryClick(Sender: TObject);   // 233c

begin
  pad_form.search_for_menu_entry.Click;
end;
//______________________________________________________________________________

procedure Tcontrol_room_form.enableDXFexportedfillerbelowkey1Click(Sender: TObject);  // 239a

begin
  dxf_form.fill_below_key_checkbox.Enabled:=True;
end;
//______________________________________________________________________________
