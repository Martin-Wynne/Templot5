...
uses
  pad_unit,math_unit,math2_unit,keep_select,bgkeeps_unit,alert_unit, info_unit, control_room, shove_timber,
  switch_select, wait_message, help_sheet, rail_options_unit;
...
var
  ...
  dummy_notch1,dummy_notch2,dummy_notch3:Tnotch;
                ...
                function get_k3n(increment:extended):boolean;   // get xing angle to match intersection of turnout curves
                  ...
                  // enter with very short k3n (e.g. 0.95) and starting increment (e.g. 0.1) and initial a3
...
begin
  ...
  rail_options_form.reset_all_exit_joints_button.Click;  // 227a  reset MVJP and TVJP on defaults
  ...
  if jump_forward=False        // not re-entering
     then begin                // create first 2 turnouts ...
            ...
            turnout_road_i:=0;  // 223a normal exit

            if retpar_i=1
               then begin
                        // changing from parallel
                      retpar_i:=0;
  ...
  try
    ...
    if alert(4,'    make  3-way  tandem  turnout',
       ...
       then begin
              ...
              +'||Templot cannot make these final adjustments for you because it requires a human eye to see what is needed.'
                ...
                begin
                  ...
                  try
                    repeat
                      ...
                      if hdkn>250 then EXIT;   // 226d  go try gaunt for more sensible result
...
begin
  ...
  try
    ...
    if try_hd=True
       then begin
              k3n:=k3n+0.0001;      // restore last decrement
    ...
    startx:=tvjp_ncx-get_notch_distance(far_notch,isol_notch)-4*inscale;    // 4" arbitrary
