...
uses
  ...
  gauge_unit,colour_unit,preview_unit,

  bgkeeps_unit, startup_unit, action_unit, make_slip_unit, print_unit,
  export_unit, entry_sheet, brick_unit, pdf_laz_unit;
...
begin
  ...
  with user_prefs_list do begin
    ...
    Values['templot_user_preferences']:=str_prefix+'from Templot version '+round_str(program_version/100,2)+version_build;
    ...
    Values['00-22f']:=FloatToStr(screenx_min);        // 229c  max-explode  (zoom-in)
    ...
    Values['00-86b']:=IntToStr(integer(data_entry_form.show_selected_checkbox.Checked));    // 229c

    Values['00-87b']:=IntToStr(integer(retain_prefix_pref));  // 213b

    Values['00-88i']:=IntToStr(chosen_colour_scheme);         // 224a

    Values['00-89b']:=IntToStr(integer(hide_name_labels));    // 224a
  ...
  with cdvi do begin    // dummy vehicle dims  0.98.a
    ...
    Values['01-04b']:=IntToStr(integer(bgnd_form.pad_shapes_linewidth_4_radiobutton.Checked));    // 234d

    Values['01-10i']:=IntToStr(shapes_colour);  // on pad

    Values['01-20i']:=IntToStr(save_psc);       // on output

    Values['01-22i']:=IntToStr(twist_corner_colour);   // 232a picture shapes ..
    Values['01-24i']:=IntToStr(combined_patch_colour);
    ...
    Values['01-41b']:=IntToStr(integer(bgnd_form.pad_brick_boundary_1_radio.Checked));   // 234d ...
    Values['01-42b']:=IntToStr(integer(bgnd_form.pad_brick_boundary_2_radio.Checked));
    Values['01-43b']:=IntToStr(integer(bgnd_form.pad_brick_boundary_3_radio.Checked));
    Values['01-44b']:=IntToStr(integer(bgnd_form.pad_brick_boundary_4_radio.Checked));
    Values['01-45b']:=IntToStr(integer(bgnd_form.pad_brick_boundary_5_radio.Checked));
    Values['01-46b']:=IntToStr(integer(bgnd_form.pad_brick_boundary_6_radio.Checked));


    Values['01-50f']:=FloatToStr(brick_boundary_rectangle_x);       // 234d ...
    Values['01-51f']:=FloatToStr(brick_boundary_rectangle_y);
    Values['01-52f']:=FloatToStr(brick_boundary_rectangle_w);
    Values['01-53f']:=FloatToStr(brick_boundary_rectangle_h);
    Values['01-54f']:=FloatToStr(brick_boundary_rectangle_k);
    ...
    Values['03-83b']:=IntToStr(integer(bgkeeps_form.guide_marks_checkbox.Checked));
    ...
    Values['03-95b']:=IntToStr(integer(bgkeeps_form.radial_centre_marks_checkbox.Checked));
    ...
    Values['03-98b']:=IntToStr(integer(bgkeeps_form.chair_outlines_checkbox.Checked));          // 221a
    Values['03-99b']:=IntToStr(integer(bgkeeps_form.dropper_ridge_marks_checkbox.Checked));     // 243b
...
(* // OT2024
    ...
    Values['08-10f']:=FloatToStr(track_bgnd_width_in);   // 206a
...
*)
    ...
    with print_settings_form do begin        // 223d

      Values['08-49b']:=IntToStr(integer(output_timb_id_prefix_checkbox.Checked));
      Values['08-50b']:=IntToStr(integer(output_rail_joints_checkbox.Checked));
      Values['08-51b']:=IntToStr(integer(output_timber_centres_checkbox.Checked));
      Values['08-52b']:=IntToStr(integer(output_timber_extensions_checkbox.Checked));
      Values['08-53b']:=IntToStr(integer(output_timber_numbers_checkbox.Checked));
      Values['08-54b']:=IntToStr(integer(output_guide_marks_checkbox.Checked));
      Values['08-55b']:=IntToStr(integer(output_switch_drive_checkbox.Checked));

      Values['08-56b']:=IntToStr(integer(output_rails_checkbox.Checked));
      Values['08-57b']:=IntToStr(integer(output_centrelines_checkbox.Checked));
      Values['08-58b']:=IntToStr(integer(output_timbering_checkbox.Checked));
      Values['08-59b']:=IntToStr(integer(output_chairs_checkbox.Checked));
      Values['08-60b']:=IntToStr(integer(output_fb_foot_lines_checkbox.Checked));
      Values['08-61b']:=IntToStr(integer(output_radial_ends_checkbox.Checked));
      Values['08-62b']:=IntToStr(integer(output_switch_labels_checkbox.Checked));
      Values['08-63b']:=IntToStr(integer(output_xing_labels_checkbox.Checked));
      Values['08-64b']:=IntToStr(integer(output_radial_centres_checkbox.Checked));
      Values['08-65b']:=IntToStr(integer(output_platforms_checkbox.Checked));
      Values['08-66b']:=IntToStr(integer(output_trackbed_edges_checkbox.Checked));

      Values['08-71b']:=IntToStr(integer(output_bgnd_shapes_checkbox.Checked));
      Values['08-72b']:=IntToStr(integer(output_sketchboard_items_checkbox.Checked));

    end;//with

    Values['08-81b']:=IntToStr(integer(print_form.page_ident_checkbox.Checked));
    Values['08-82b']:=IntToStr(integer(pdf_laz_form.page_ident_checkbox.Checked));
    Values['08-83b']:=IntToStr(integer(print_form.warnings_checkbox.Checked));
    Values['08-84b']:=IntToStr(integer(pad_form.show_corner_info_menu_entry.Checked));

    Values['08-85b']:=IntToStr(integer(pdf_laz_form.page_ident_checkbox.Checked));    // 245a



    Values['08-86i']:=IntToStr(printgrid_i);      // print grid on/off

    Values['08-90i']:=IntToStr(save_ptic);        // timber infill colour
    Values['08-91i']:=IntToStr(save_priccu);      // control template rail infill colour
    Values['08-92i']:=IntToStr(save_pricbg);      // bgnd template rail infill colour
    Values['08-93i']:=IntToStr(save_grc);         // printed grid colour
    Values['08-94i']:=IntToStr(save_psc);         // bgnd shapes colour
    Values['08-95i']:=IntToStr(save_priplatedge); // platform edge colour
    Values['08-96i']:=IntToStr(save_priplatfill); // platform infill
...
(* // OT2024
    ...
    Add('--------  sketchboard:  --------');
    Values['09-05f']:=FloatToStr(freehand_tracking);

    Values['09-10b']:=IntToStr(integer(dtp_settings_form.design_quality_radiobutton.Checked));
...
*)
    ...
    Values['10-10b']:=IntToStr(integer(keep_form.use_normal_colours_menu_entry.Checked));     // 227a radio item
    Values['10-11b']:=IntToStr(integer(keep_form.use_inherited_colours_menu_entry.Checked));  // 227a radio item
    ...
    Add('--------  PDF export:  --------');                 // 227a
    Values['14-10b']:=IntToStr(integer(export_form.pdf_size_inside_trim_margins_checkbox.Checked));

    Values['14-12s']:=prepare_string_for_prefs(export_form.pdf_long_mm_edit.Text);
    Values['14-13s']:=prepare_string_for_prefs(export_form.pdf_short_mm_edit.Text);
    Values['14-14s']:=prepare_string_for_prefs(export_form.pdf_dpi_edit.Text);

    Values['14-16b']:=IntToStr(integer(export_form.pdf_end_run_radio.Checked));      // radio
    Values['14-17b']:=IntToStr(integer(export_form.pdf_side_run_radio.Checked));     // radio
  ...
  if startup_msg_prefs=True
     then begin
            ...
            try
              ...
              screenx_min:=      get_pref_float('00-22f',screenx_min);    // max-explode (zoom-in)
              ...
              if data_entry_form.show_selected_checkbox.Checked<>get_pref_bool('00-86b',data_entry_form.show_selected_checkbox.Checked)  // 213b
                 then begin
                        data_entry_form.show_selected_checkbox.Checked:=get_pref_bool('00-86b',data_entry_form.show_selected_checkbox.Checked);
                        set_autoselect;
                      end;

              retain_prefix_pref:=get_pref_bool('00-87b',retain_prefix_pref);  // 213b

                  // 224a  chosen hi-colour scheme...

              chosen_colour_scheme:=get_pref_int('00-88i',chosen_colour_scheme);     // 224a

              if (chosen_colour_scheme<1) or (chosen_colour_scheme>6) then chosen_colour_scheme:=4;  // sky blue startup
              if chosen_colour_scheme<>5 then do_hi_colour_schemes(chosen_colour_scheme);


              hide_name_labels:=get_pref_bool('00-89b',hide_name_labels);  // 224a

              pad_form.hide_name_labels_menu_entry.Checked:=hide_name_labels;       // radio items
              pad_form.show_name_labels_menu_entry.Checked:= NOT hide_name_labels;
              ...
              with bgnd_form.pad_shapes_linewidth_4_radiobutton do Checked:=get_pref_bool('01-04b',Checked);

                      shapes_colour:=get_pref_int('01-10i',shapes_colour);  // on pad
                           save_psc:=get_pref_int('01-20i',save_psc);       // on output

                twist_corner_colour:=get_pref_int('01-22i',twist_corner_colour);   // 232a picture shapes ..
              combined_patch_colour:=get_pref_int('01-24i',combined_patch_colour);
              ...
              with bgnd_form.pad_brick_boundary_1_radio do Checked:=get_pref_bool('01-41b',Checked);      // 234d ...
              with bgnd_form.pad_brick_boundary_2_radio do Checked:=get_pref_bool('01-42b',Checked);
              with bgnd_form.pad_brick_boundary_3_radio do Checked:=get_pref_bool('01-43b',Checked);
              with bgnd_form.pad_brick_boundary_4_radio do Checked:=get_pref_bool('01-44b',Checked);
              with bgnd_form.pad_brick_boundary_5_radio do Checked:=get_pref_bool('01-45b',Checked);
              with bgnd_form.pad_brick_boundary_6_radio do Checked:=get_pref_bool('01-46b',Checked);

              brick_boundary_rectangle_x:=get_pref_float('01-50f',brick_boundary_rectangle_x);  // 234d ...
              brick_boundary_rectangle_y:=get_pref_float('01-51f',brick_boundary_rectangle_y);
              brick_boundary_rectangle_w:=get_pref_float('01-52f',brick_boundary_rectangle_w);
              brick_boundary_rectangle_h:=get_pref_float('01-53f',brick_boundary_rectangle_h);
              brick_boundary_rectangle_k:=get_pref_float('01-54f',brick_boundary_rectangle_k);
 ...
 with pad_form.snap_always_on_background_templates_menu_entry do Checked:=get_pref_bool('03-72b',Checked);     // 218a radio item
             ...
             with bgkeeps_form.timber_outlines_checkbox do Checked:=get_pref_bool('03-80b',Checked);
              with bgkeeps_form.timber_centres_checkbox do Checked:=get_pref_bool('03-81b',Checked);
                 ...
                 with bgkeeps_form.guide_marks_checkbox do Checked:=get_pref_bool('03-83b',Checked);
         ...
         with bgkeeps_form.radial_centre_marks_checkbox do Checked:=get_pref_bool('03-95b',Checked);
        ...
        with bgkeeps_form.bold_timber_outlines_checkbox do Checked:=get_pref_bool('03-97b',Checked);    // 219a
              with bgkeeps_form.chair_outlines_checkbox do Checked:=get_pref_bool('03-98b',Checked);    // 221a

              brick_form.bgnd_chairs_checkbox.Checked:=bgkeeps_form.chair_outlines_checkbox.Checked;    // 244d

         with bgkeeps_form.dropper_ridge_marks_checkbox do Checked:=get_pref_bool('03-99b',Checked);    // 243b
...
*)
         ...
         print_platform_infill_style:=get_pref_int('08-43i',print_platform_infill_style);
              ...
              with print_settings_form do begin        // 223d
               ...
               with output_timb_id_prefix_checkbox    do Checked:=get_pref_bool('08-49b',Checked);
               with output_rail_joints_checkbox       do Checked:=get_pref_bool('08-50b',Checked);
               with output_timber_centres_checkbox    do Checked:=get_pref_bool('08-51b',Checked);
               with output_timber_extensions_checkbox do Checked:=get_pref_bool('08-52b',Checked);
               with output_timber_numbers_checkbox    do Checked:=get_pref_bool('08-53b',Checked);
               with output_guide_marks_checkbox       do Checked:=get_pref_bool('08-54b',Checked);
               with output_switch_drive_checkbox      do Checked:=get_pref_bool('08-55b',Checked);

               with output_rails_checkbox             do Checked:=get_pref_bool('08-56b',Checked);
               with output_centrelines_checkbox       do Checked:=get_pref_bool('08-57b',Checked);
               with output_timbering_checkbox         do Checked:=get_pref_bool('08-58b',Checked);
               with output_chairs_checkbox            do Checked:=get_pref_bool('08-59b',Checked);
               with output_fb_foot_lines_checkbox     do Checked:=get_pref_bool('08-60b',Checked);
               with output_radial_ends_checkbox       do Checked:=get_pref_bool('08-61b',Checked);
               with output_switch_labels_checkbox     do Checked:=get_pref_bool('08-62b',Checked);
               with output_xing_labels_checkbox       do Checked:=get_pref_bool('08-63b',Checked);
               with output_radial_centres_checkbox    do Checked:=get_pref_bool('08-64b',Checked);
               with output_platforms_checkbox         do Checked:=get_pref_bool('08-65b',Checked);
               with output_trackbed_edges_checkbox    do Checked:=get_pref_bool('08-66b',Checked);

               with output_bgnd_shapes_checkbox       do Checked:=get_pref_bool('08-71b',Checked);
               with output_sketchboard_items_checkbox do Checked:=get_pref_bool('08-72b',Checked);

              end;//with

              with print_form.page_ident_checkbox       do Checked:=get_pref_bool('08-81b',Checked);
              with pdf_laz_form.page_ident_checkbox     do Checked:=get_pref_bool('08-82b',Checked);
              with print_form.warnings_checkbox         do Checked:=get_pref_bool('08-83b',Checked);
              with pad_form.show_corner_info_menu_entry do Checked:=get_pref_bool('08-84b',Checked);

              with pdf_laz_form.page_ident_checkbox     do Checked:=get_pref_bool('08-85b',Checked);     // 245a

                  printgrid_i:=get_pref_int('08-86i',printgrid_i);      // print grid on/off

                    save_ptic:=get_pref_int('08-90i',save_ptic);        // timber infill colour
                  save_priccu:=get_pref_int('08-91i',save_priccu);      // control template rail infill colour
                  save_pricbg:=get_pref_int('08-92i',save_pricbg);      // bgnd template rail infill colour
                     save_grc:=get_pref_int('08-93i',save_grc);         // printed grid colour
                     save_psc:=get_pref_int('08-94i',save_psc);         // bgnd shapes colour
             save_priplatedge:=get_pref_int('08-95i',save_priplatedge); // platform edge colour
             save_priplatfill:=get_pref_int('08-96i',save_priplatfill); // platform infill
...
*)
              ...
              with keep_form.use_normal_colours_menu_entry    do Checked:=get_pref_bool('10-10b',Checked); // 227a radio item
              with keep_form.use_inherited_colours_menu_entry do Checked:=get_pref_bool('10-11b',Checked); // 227a radio item
  ...
  with export_form.pdf_size_inside_trim_margins_checkbox do Checked:=get_pref_bool('14-10b',Checked);

                        with export_form.pdf_long_mm_edit do Text:=get_pref_string('14-12s',Text);
                       with export_form.pdf_short_mm_edit do Text:=get_pref_string('14-13s',Text);
                            with export_form.pdf_dpi_edit do Text:=get_pref_string('14-14s',Text);

                with export_form.pdf_end_run_radio do Checked:=get_pref_bool('14-16b',Checked); // radio item
               with export_form.pdf_side_run_radio do Checked:=get_pref_bool('14-17b',Checked); // radio item
               ...
               then RESULT:=True                // beginner user, not using prefs, or prefs since abandoned.
                      ...
                      repeat
                        i:=alert(4,'      save  program  preferences ?',
                                  ...
                                  +'||You are seeing this message because you started this Templot session without using your previously saved program preferences.'
                                  ...
                                  +'||If you choose to abandon your preferences, you won''t see the option to use your preferences when you start the next Templot session.'
  ...
  if FileExists(exe_str+str+'.sk1')=True
     then begin
            if alert(7,'      delete  program  preferences ?',
                         '||Your are about to delete your saved program preferences in file: '+str
                        +'||OK?| ',
    ...
    with cgs_list do begin
      ...
      for n:=0 to 3 do begin    // custom A to D ...
        ...
        with gauge[index] do begin
           ...
           name_str_glist:=Trim(get_pref_string_cgs(cgs_list,'11-'+IntToStr(n)+'0s',name_str_glist));   // gauge designation: 9 chars max    227a
