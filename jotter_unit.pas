...
begin
  ...
  show_modal_message('The jotter read-outs are now in the top-right corner of the screen.');
end;
//__________________________________________________________________________________________
...
(*
  ...
  try
    ...
    with Printer.Canvas do begin
      ...
      print_memo.ClientWidth:=Round(text_width_dots*PixelsPerInch/nom_width_dpi);     // width between margins in screen pixels.
    ...
    print_memo.Clear;
    print_memo.Font.Assign(printer_text_font);
    print_memo.Text:=AdjustLineBreaks(jotter_memo.Text);

    WriteLn(pf,'');
    WriteLn(pf,left_margin_str+'            JOTTER  TEXT      at  '+TimeToStr(Time)+'    on  '+DateToStr(Date)+'  :');
    WriteLn(pf,'');
    for line_now:=0 to print_memo.Lines.Count-1 do WriteLn(pf,left_margin_str+print_memo.Lines[line_now]);
  finally
    CloseFile(pf);
...
const
  ...
  +'||While using the Templot data-entry form, clicking the JOT LINE / JOT ALL button adds a copy of the current input line or all the input lines onto the jotter.'

  +'||The Templot HELP texts can be added to the jotter by right-button clicking on them and them clicking the JOT ALL menu item. They include long text lines, so you will probably want to have the WRAP LINES option selected for the jotter.'

  +'||These functions work regardless of whether the jotter window is currently visible.'

  +'||Clicking RESTORE PREVIOUS TEXT on the jotter right-click menu restores the jotter text from your previous Templot working session. In this way a complete journal of your design work can be maintained if you wish.'
