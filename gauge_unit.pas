...
begin
  ...
  with gauge[i] do begin
           listbox_str:=' 00-XF    16.0  mm    4 mm/ft     1:76.2    00 Extreme Fine UK      ';  // 239a
           scale_glist:=4;            // mm per ft.
           gauge_glist:=16.0;         // mm.
           fw_glist:=0.8;             // mm flangeway.
           old_fwe_glist:=1.38;       // mm flangeway end (flangeway+flare).
           min_radius_glist:=1000;    // mm minimum radius for check (39" approx).
                   end;{with}

           t_00XF_i:=i;            // for gauge checks and sleeper size.

                   gauge_form.gauge_listbox.Items.Add(listbox_str);    Inc(i);if i>gauge_indexmax_c then run_error(39);


  with gauge[i] do begin
           listbox_str:=' 00-SF    16.2  mm    4 mm/ft     1:76.2    00 EMGS-derived UK(4-SF)';      // 227a name change
           scale_glist:=4;            // mm per ft.
           gauge_glist:=16.2;         // mm.
           fw_glist:=1.0;             // mm flangeway.
           old_fwe_glist:=1.7;        // mm flangeway end (flangeway+flare).
           ...
           t_00SF_i:=i;            // for gauge checks and sleeper size.

                   gauge_form.gauge_listbox.Items.Add(listbox_str);    Inc(i);if i>gauge_indexmax_c then run_error(39);

  with gauge[i] do begin
           listbox_str:=' 00-MF    16.3  mm    4 mm/ft     1:76.2    00 Modified Fine UK     ';      // 233a
           scale_glist:=4;            // mm per ft.
           gauge_glist:=16.3;         // mm.
           fw_glist:=1.1;             // mm flangeway.
           old_fwe_glist:=1.8;        // mm flangeway end (flangeway+flare).
  ...
  with gauge[i] do begin
           listbox_str:=' 00-IF    16.4  mm    4 mm/ft     1:76.2    00 RTR Improved UK      ';  // added 234a   31/3/2022
           scale_glist:=4;            // mm per ft.
           gauge_glist:=16.4;         // mm.
           ...
           trtscent_glist:=50;        // mm track centres, turnout side.
           trmscent_glist:=50;        // mm ditto, main side.
           min_radius_glist:=675;     // mm minimum radius for check (26.5" approx).

           t_00IF_i:=i;         // for sleeper sizes.
  ...
  with gauge[i] do begin
           ...
           trtscent_glist:=50;        // mm track centres, turnout side.
           trmscent_glist:=50;        // mm ditto, main side.
           min_radius_glist:=650;     // mm minimum radius for check (26" approx).
  ...
  with gauge[i] do begin
           ...
           trtscent_glist:=50;        // mm track centres, turnout side.
           trmscent_glist:=50;        // mm ditto, main side.
           min_radius_glist:=625;     // mm minimum radius for check (25" approx).
  ...
  with gauge[i] do begin
           listbox_str:=' 00-BF    16.5  mm    4 mm/ft     1:76.2    00 RTR Standard UK      ';   // 212a fw was 1.25
           scale_glist:=4;            // mm per ft.
           gauge_glist:=16.5;         // mm.
           fw_glist:=1.3;             // mm flangeway.
           old_fwe_glist:=1.9;        // mm flangeway end (flangeway+flare).
           ...
           t_00BF_i:=i;         // for gauge quick-set menu and sleeper size.

                   end;{with}   gauge_form.gauge_listbox.Items.Add(listbox_str);    Inc(i);if i>gauge_indexmax_c then run_error(39);

  with gauge[i] do begin
           listbox_str:=' 00/H0    16.5  mm    4 mm/ft     1:76.2    00/H0 Universal OLD UK  ';
           scale_glist:=4;            // mm per ft.
           gauge_glist:=16.5;         // mm.
           fw_glist:=1.5;             // mm flangeway.
           old_fwe_glist:=2.2;        // mm flangeway end (flangeway+flare).
           trtscent_glist:=50;        // mm track centres, turnout side.
           trmscent_glist:=50;        // mm ditto, main side.
           min_radius_glist:=550;     // mm minimum radius for check (22" approx).
   ...
   with gauge[i] do begin
           listbox_str:=' EM-SF    18.0  mm    4 mm/ft     1:76.2    EM Special (Manchester) ';   // added 212a
           scale_glist:=4;            // mm per ft.
           gauge_glist:=18.0;         // mm.
           fw_glist:=0.8;             // mm flangeway.
           old_fwe_glist:=1.38;       // mm flangeway end (flangeway+flare).
  ...
  with gauge[i] do begin
           ...
           gauge_glist:=18.0;         // mm.
           fw_glist:=1.0;             // mm flangeway.
           old_fwe_glist:=1.58;       // mm flangeway end (flangeway+flare).
           min_radius_glist:=900;     // mm minimum radius for check (36" approx).
                   end;{with}   gauge_form.gauge_listbox.Items.Add(listbox_str);    Inc(i);if i>gauge_indexmax_c then run_error(39);
  ...
  with gauge[i] do begin
           listbox_str:=' EM4      18.8  mm    4 mm/ft     1:76.2    EM Extra Fine (EEM)     ';
           scale_glist:=4;            // mm per ft.
           gauge_glist:=18.8;         // mm.
           fw_glist:=0.8;             // mm flangeway.
           old_fwe_glist:=1.38;       // mm flangeway end (flangeway+flare).
  ...
  with gauge[i] do begin
           ...
           fw_glist:=0.71;            // mm flangeway.   .028"
           old_fwe_glist:=1.40;           // mm flangeway end (flangeway+flare).
           trtscent_glist:=55.7609;   // mm track centres, turnout side.  227a    140.5" centres for 6ft way
           trmscent_glist:=55.7609;   // mm ditto, main side.
           min_radius_glist:=1300;    // mm minimum radius for check (51" approx).
  ...
  with gauge[i] do begin
           listbox_str:=' T-55      1.000"     5.5 mm/ft   1:55.42   Templot startup         ';
  ...
  with gauge[i] do begin
           listbox_str:=' 0-EURO   32.00 mm    6.77 mm/ft  1:45      0 Europe                ';      // 221a
           scale_glist:=304.8/45;     // mm per ft.
           gauge_glist:=32.0;         // mm.
           fw_glist:=2.0;             // mm flangeway.
           old_fwe_glist:=3.0;        // mm flangeway end (flangeway+flare).
           trtscent_glist:=114.3;   // mm track centres, turnout side.        (Lenz catalogue)
           trmscent_glist:=114.3;   // mm ditto, main side.
           min_radius_glist:=1000;    // mm minimum radius for check (66" approx).
                   end;{with}   gauge_form.gauge_listbox.Items.Add(listbox_str);    Inc(i);if i>gauge_indexmax_c then run_error(39);
  ...
  with gauge[i] do begin
           ...
           gauge_glist:=31.2;         // mm.
           fw_glist:=1.2;            // mm flangeway.  221a    was 1.25mm
  ...
  with gauge[i] do begin
           listbox_str:=' 0-US-OLD  1.25"      9/32"/ft    1:42.67   0 US old dimensions     ';
  ...
  with gauge[i] do begin
           listbox_str:=' 1C        1.75"      10 mm/ft    1:30.48   Gauge 1 Coarse          ';
  ...
  with gauge[i] do begin
           listbox_str:=' 1F       45.0  mm    10 mm/ft    1:30.48   Gauge 1 G1MRA Fine      ';
           scale_glist:=10;           // mm per ft.
           gauge_glist:=45;           // mm.
           fw_glist:=1.75;            // mm flangeway.
           old_fwe_glist:=3.5;        // mm flangeway end (flangeway+flare).
  ...
  with gauge[i] do begin
           listbox_str:=' 1-MF     45.0  mm    10 mm/ft    1:30.48   Gauge 1 Modified Fine   ';
           scale_glist:=10;           // mm per ft.
           gauge_glist:=45;           // mm.
           fw_glist:=2.5;             // mm flangeway.
           old_fwe_glist:=3.5;        // mm flangeway end (flangeway+flare).
           trtscent_glist:=111.67;    // mm track centres, turnout side.
           trmscent_glist:=111.67;    // mm ditto, main side.
           min_radius_glist:=2500;    // mm minimum radius for check (98" approx).
                   end;{with}   gauge_form.gauge_listbox.Items.Add(listbox_str);    Inc(i);if i>gauge_indexmax_c then run_error(39);

  with gauge[i] do begin
           listbox_str:=' No.3      2.50"     17/32"/ft    1:22.59   Gauge 3                 ';
           scale_glist:=25.4*17/32;   // mm per ft.
           gauge_glist:=25.4*2.5;     // mm.
           fw_glist:=3.0;             // mm flangeway.
           old_fwe_glist:=5.0;        // mm flangeway end (flangeway+flare).
  ...
  with gauge[i] do begin
           listbox_str:=' No.4      3.50"       3/4"/ft    1:16      model engineers / trams ';
           scale_glist:=25.4*3/4;     // mm per ft.
           gauge_glist:=25.4*3.5;     // mm.
  ...
  with gauge[i] do begin
           listbox_str:=' No.5-A    4.75"         1"/ft    1:12      model engineers  USA    ';
           scale_glist:=25.4;         // mm per ft.
           gauge_glist:=25.4*4.75;    // mm.
  ...
  with gauge[i] do begin
           listbox_str:=' No.5      5.00"     17/16"/ft    1:11.29   model engineers         ';
           scale_glist:=25.4*17/16;   // mm per ft.
           gauge_glist:=25.4*5;       // mm.
           fw_glist:=5.5;             // mm flangeway.
           old_fwe_glist:=10.0;       // mm flangeway end (flangeway+flare).
           trtscent_glist:=350;       // mm track centres, turnout side.
           trmscent_glist:=350;       // mm ditto, main side.
  ...
  for j:=0 to i do begin
    ...
    gauge[j].name_str_glist:=Trim(Copy(temp_str,1,9));  // and crop to max 9 if nec.     227a
...
const
  help_all_str:string='||With most standard gauge tracks you will probably only need to set the track gauge and flangeway gaps.'
   ...
   +'||The existing dimensions showing in the input box are Templot''s best guess based on the current gauge/scale combination, so it is'
...
begin
  ...
  if gauge[index].scale_glist>minfp
     then begin
            ...
            gauge[index].name_str_glist:=Trim(Copy(name_str,1,9));  // add the name        227a
       ...
       then begin
              ...
              with info_listbox.Items do begin
                with gauge[i] do begin
                  ...
                  Add(space_lead(FormatFloat('00000.00',min_radius_glist))+'  mm  suggested small radius warning limit.');
                end;//with
              end;//with
            end;
...
begin
  ...
  do_open_source_bang('PRINT GAUGE LIST');  // OT2024
