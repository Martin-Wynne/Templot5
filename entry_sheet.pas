...
type
  ...
  Tdata_entry_form = class(TForm)
    ...
    procedure beans_on_toast(Sender:TObject);
...
procedure modify_prompt_font(i,h:integer);   // 241b
...
const
  ...
  +'||This is Templot''s main data-entry form, which appears whenever dimensional information needs to be entered.'
  ...
  +'||If you enter a slash ( / ) at the left of the white panel, Templot will in most cases ignore anything else'
  ...
  +'||If you enter a completely blank value, Templot will assume that you mean zero ( 0 ). Note that in some cases,'
  +' zero is not a valid entry (as a radius dimension, for example). If the figures you enter are unacceptable,'
  +' Templot will ask you to try again.'
  ...
  +' This corresponds to Templot''s normal display of negative numbers within square brackets.'
  ...
  +' asked for millimetres, Templot will ignore the "ft" and assume that you mean 2 mm.'
  ...
  +' Templot jotter for future reference. To show or hide the jotter, press the `0CTRL+J`2 keys.'
    ...
    if Showing=True
       then begin
              ...
              if n<>7 then with input_panel7 do begin BorderStyle:=bsNone; ParentColor:=True; Left:=8; Top:=Round(276*global_factor); help_button7.Visible:=False; input_box7.Color:=$00ECECEC; input_spacer_label7.Color:=$00ECECEC; end;
                        ...
                        //  extra PostMessage needed to ensure SetFocus with flashing caret ...
...
begin
  ...
  with dims[n] do begin
    ...
    input_prompt_label.Hint:=' '+prompt_str+' ';
...
procedure Tdata_entry_form.FormDeactivate(Sender: TObject);
...
begin
  if (stay_visible_form.Showing=True) and ( stay_visible_form.Left<(Left+Width-8) )
     then begin
            Left:=0;
            stay_visible_form.Left:=Width-8;  // make sure it doesn't obscure us
          end;
end;
//______________________________________________________________________________
...
procedure Tdata_entry_form.beans_on_toast(Sender:TObject);
...
const
  ...
  +' are frequently in decimal or fractional inches, Templot also provides "input conversion factors".'
 ...
 +'||| r  prototype chains   scale model mm        radius:     r10   =   2640   mm     ( 10 chains )'
  +'|| u  prototype chains  prototype inches      distance:    u1    =    792  inches  ( 1 chain )'
 ...
 +'||| k    degrees         unit angle (RAM)   crossing angle: k9.5  =  1:5.98 RAM     (9.5 deg)'
  ...
  +'|For example Templot expects the sleeper length to be entered in full-size prototype inches.'
  ...
  +' Templot makes no check that the code letters used are appropriate for the units requested, since only you know what your figures represent.'
    ...
    with dims[n] do begin
                ...
                repeat                  // do all conversions
                  ...
                  if conversions_checkbox.Checked=True
                     then case in_char1 of
                           ...
                           'r','R': begin                     //  r = chains (66ft) to model mm      227d
                                      Delete(intxt,1,1);
                                      conv_factor:=conv_factor*66*scale;
                                    end;

                           'u','U': begin                     //  u = chains (66ft) to inches       227d
                                      Delete(intxt,1,1);
                                      conv_factor:=conv_factor*792;
                                    end;
      ...
      if ok_flag=True     // set ouput data...
         then begin
                ...
                if (zeroterm=True) and (newdim=0)      //  227a    terminate input on zero
                   then begin
...
begin
  with data_entry_form do begin
    ...
    case i of
       ...
       0: input_prompt_label0.Font.Height:=input_prompt_label0.Font.Height+h;  // font height negative
       1: input_prompt_label1.Font.Height:=input_prompt_label1.Font.Height+h;
       2: input_prompt_label2.Font.Height:=input_prompt_label2.Font.Height+h;
       3: input_prompt_label3.Font.Height:=input_prompt_label3.Font.Height+h;
       4: input_prompt_label4.Font.Height:=input_prompt_label4.Font.Height+h;
       5: input_prompt_label5.Font.Height:=input_prompt_label5.Font.Height+h;
       6: input_prompt_label6.Font.Height:=input_prompt_label6.Font.Height+h;
       7: input_prompt_label7.Font.Height:=input_prompt_label7.Font.Height+h;
